...
/**
 * \file config_feature_base.h
 *
 * \brief OpenHarmony MbedTLS feature-ized base configuration (non-module macros).
 *
 * Contains the NON-module configuration macros (platform, cipher modes, ECP
 * curves, SSL parameters, key exchanges, etc.) shared by mini and small/standard.
 *
 * Module macros (MBEDTLS_xxx_C) are NOT defined here; they are injected by GN
 * via MBEDTLS_FEATURE_DEFINES, driven by the mbedtls_feature_xxx flags declared
 * in third_party/mbedtls/mbedtls.gni. This realizes compile-level trimming:
 * a feature off => the .c is not compiled AND MBEDTLS_xxx_C is not defined.
 *
 * Derived from the legacy config_liteos_m.h with all 52 MBEDTLS_xxx_C module
 * defines removed. Contains NO #undef of module macros, so it can never override
 * GN-injected -D defines.
 */

#ifndef MBEDTLS_CONFIG_FEATURE_BASE_H
#define MBEDTLS_CONFIG_FEATURE_BASE_H

#define RESTRICT_MBEDTLS_DEFINES
 ...
 */
#ifndef RESTRICT_MBEDTLS_DEFINES
 ...
 */
/* ECP curve macros (MBEDTLS_ECP_DP_*_ENABLED) are GN-injected via
 * mbedtls_feature_ecp_curve_* sub-features (see mbedtls.gni).
 * Short Weierstrass: secp192r1..secp521r1, secp192k1..secp256k1, bp256r1..bp512r1
 ...
 */
/* entropy poll： BUILD.gn insource switch on wheninject -DMBEDTLS_NO_PLATFORM_ENTROPY（ws63 no FS）。
 has FS use --gn-args mbedtls_platform_entropy=true enable platform poll。*/
#ifndef RESTRICT_MBEDTLS_DEFINES
...
/** Default minimum number of bytes required for the hardware entropy source mbedtls_hardware_poll() before entropy
 *  is released */
...
#endif /* MBEDTLS_CONFIG_FEATURE_BASE_H */
