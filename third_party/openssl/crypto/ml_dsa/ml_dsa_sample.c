...
{
    int j = 0;
    uint8_t blocks[SHAKE128_BLOCKSIZE * 10], *b, *end = blocks + sizeof(blocks);
    ...
    const uint8_t *seed, size_t seed_len, POLY *out)
{
    int j = 0;
    uint32_t z0, z1;
    uint8_t blocks[SHAKE256_BLOCKSIZE * 10], *b, *end = blocks + sizeof(blocks);
...
int ossl_ml_dsa_poly_sample_in_ball(POLY *out_c, const uint8_t *seed, int seed_len,
    ...
    uint32_t tau)
{
    uint8_t block[SHAKE256_BLOCKSIZE * 10];
    uint64_t signs;
    int offset = 8;
    size_t end;
