...
void wpa_auth_set_ptk_rekey_timer(struct wpa_state_machine *sm)
{
	if (sm && sm->wpa_auth->conf.wpa_ptk_rekey) {
		wpa_printf(MSG_DEBUG, "WPA: Start PTK rekeying timer for "
			   MACSTR_SEC " (%d seconds)",
			   MAC2STR_SEC(wpa_auth_get_spa(sm)),
...
void wpa_auth_sta_deinit(struct wpa_state_machine *sm)
{
	...
	if (sm->in_step_loop) {
		...
		wpa_printf(MSG_DEBUG,
			   ...
			   MACSTR_SEC, MAC2STR_SEC(wpa_auth_get_spa(sm)));
		sm->pending_deinit = 1;
	} else
		wpa_free_sta_sm(sm);
}
		 ...
		 u8 *data, size_t data_len)
{
	...
	wpa_printf(MSG_INFO, "WPA: Received EAPOL-Key from " MACSTR_SEC
		   ...
		   MAC2STR_SEC(wpa_auth_get_spa(sm)), key_info, key->type,
...
static u8 * wpa_auth_ml_group_kdes(struct wpa_state_machine *sm, u8 *pos)
{
	...
	wpa_printf(MSG_DEBUG, "RSN: MLO Group KDE len = %ld", (long)(pos - start));
	return pos;
}
...
static u8 * wpa_auth_ml_kdes(struct wpa_state_machine *sm, u8 *pos)
{
	...
	wpa_printf(MSG_DEBUG, "RSN: MLO Link KDE len = %ld", (long)(pos - start));
...
int wpa_auth_sta_set_vlan(struct wpa_state_machine *sm, int vlan_id)
{
	...
	wpa_printf(MSG_DEBUG, "WPA: Moving STA " MACSTR_SEC
		   ...
		   MAC2STR_SEC(wpa_auth_get_spa(sm)), vlan_id);
				  ...
				  struct wpa_state_machine *sm, int ack)
{
	...
	wpa_printf(MSG_DEBUG, "WPA: EAPOL-Key TX status for STA " MACSTR_SEC
		   " ack=%d", MAC2STR_SEC(wpa_auth_get_spa(sm)), ack);
