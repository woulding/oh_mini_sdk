			...
			int rssi, int from_queue)
{
	...
	wpa_printf(MSG_DEBUG, "authentication: STA=" MACSTR_SEC " auth_alg=%d "
		   ...
		   "seq_ctrl=0x%x%s%s",
		   MAC2STR_SEC(sa), auth_alg, auth_transaction,
	...
	if (hapd->conf->no_auth_if_seen_on) {
		...
		if (other) {
			...
			wpa_printf(MSG_DEBUG, "%s: Reject authentication from "
				   ...
				   hapd->conf->iface, MAC2STR_SEC(sa),
				   hapd->conf->no_auth_if_seen_on);
		       ...
		       const u8 *link_addr)
{
	...
	wpa_printf(MSG_DEBUG, "OWE: Update status %d, ie len %d for peer "
			      ...
			      MAC2STR_SEC(link_addr ? link_addr : sta->addr));
			     ...
			     size_t len, int ok)
{
	...
	if (ok && len >= IEEE80211_HDRLEN + 2 &&
		...
		wpa_printf(MSG_DEBUG, "HS 2.0: Deauthenticate STA " MACSTR_SEC
