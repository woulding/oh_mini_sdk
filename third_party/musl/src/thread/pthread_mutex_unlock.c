...
#endif
int __pthread_mutex_unlock_slow(pthread_mutex_t *m)
{
	pthread_t self;
...
__attribute__((noinline))
#endif
int __pthread_mutex_wake_slow(pthread_mutex_t *m)
{
	int priv = (m->_m_type & 128) ^ 128;
	__wake(&m->_m_lock, 1, priv);
	return 0;
...
int __pthread_mutex_unlock(pthread_mutex_t *m)
{
	...
	if (UNLIKELY(type != PTHREAD_MUTEX_NORMAL))
	    return __pthread_mutex_unlock_slow(m);

	int cont = a_swap(&m->_m_lock, 0);
#ifdef USE_MUTEX_WAIT_OPT
    __asm__ volatile ("sev" : : : "memory");
#endif

    if (UNLIKELY(waiters || cont < 0))
	    return __pthread_mutex_wake_slow(m);
