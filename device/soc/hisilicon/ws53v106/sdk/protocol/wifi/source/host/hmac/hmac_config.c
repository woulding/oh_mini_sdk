...
OAL_STATIC osal_u8 g_channel_scoring_enable = OSAL_FALSE;
#endif
...
osal_s32 hmac_config_cali_online_cmd(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    if (OAL_UNLIKELY((hmac_vap == OAL_PTR_NULL) || (msg == OAL_PTR_NULL) || (msg->data == OAL_PTR_NULL))) {
        return OAL_FAIL;
    }
    hal_online_cali_cmd(*(msg->data));
    return OAL_SUCC;
}
#endif
/*****************************************************************************************
 功能描述  : 获取芯片温度
******************************************************************************************/
osal_s32 hmac_config_get_temp(hmac_vap_stru *hmac_vap, const osal_s8 *param)
{
    osal_s8 temp = 0;
    osal_s32 ret;
...
{
    hmac_device_stru   *hmac_device;
    mac_cfg_mode_param_stru prot_param_etc;
    ...
    prot_param_etc.band = prot_param->band;
    prot_param_etc.en_bandwidth = prot_param->en_bandwidth;
    return hmac_config_check_bw_and_band(hmac_vap, &prot_param_etc, hmac_device);
}
/*****************************************************************************
...
OAL_STATIC osal_u32 hmac_config_set_mode_flag(const hmac_vap_stru *hmac_vap, mac_cfg_channel_param_stru *channel_param,
...
OAL_STATIC osal_s32 hmac_config_offline_cali_finish(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if (wlan_close_flag == 0) {
        ...
        if (ret != OAL_SUCC) {
            oam_warning_log1(0, OAM_SF_RRM, "hmac_config_offline_cali_finish: frw_asyn_host_post_msg failed[%d]", ret);
        }
    }
...
OAL_STATIC osal_s32 hmac_config_set_sta_send_probe_req_max(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    if ((hmac_vap == OSAL_NULL) || (msg == OSAL_NULL) || (msg->data == OSAL_NULL) || (msg->data_len == 0)) {
        return OAL_FAIL;
    }

    hmac_vap->linkloss_info.send_probe_req_max = *(osal_u8 *)msg->data;
...
osal_void hmac_cfg_init(osal_void)
{
...
#endif
#ifdef _PRE_WLAN_ONLINE_IQ_CALI
    frw_msg_hook_register(WLAN_MSG_D2H_ONLINE_IQ_CALI_COMPLETE,    online_iq_cali_complete);
#endif
#ifdef _PRE_WLAN_CFGID_DEBUG
...
#endif
    frw_msg_hook_register(WLAN_MSG_W2H_CFG_SET_PROBE_REQ_MAX, hmac_config_set_sta_send_probe_req_max);
    ...
    hmac_ccpriv_register((const osal_s8 *)"get_temp", hmac_config_get_temp);
#if defined(_PRE_WLAN_FEATURE_TEMP_PROTECT) && defined(_PRE_WLAN_FEATURE_TEMP_PROTECT_ACTION)
...
static void hmac_del_mgmt_timer(hmac_vap_stru *hmac_vap)
{
    if (((hmac_vap->vap_state == MAC_VAP_STATE_STA_WAIT_AUTH_SEQ2) ||
        (hmac_vap->vap_state == MAC_VAP_STATE_STA_WAIT_AUTH_SEQ4)) &&
        (hmac_vap->mgmt_timer.is_registerd == OAL_TRUE)) {
...
osal_s32 hmac_config_external_auth(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if ((ext_auth.status != MAC_SUCCESSFUL_STATUSCODE) && (hmac_user->user_asoc_state != MAC_USER_STATE_ASSOC)) {
        ...
        if (ext_auth.status != MAC_ANTI_CLOGGING) {
            hmac_handle_disconnect_rsp_etc(hmac_vap, hmac_user, ext_auth.status + HMAC_EXT_AUTH_FAIL);
        }
        /* 删除用户 */
...
osal_s32 hmac_config_send_custom_pkt(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    /* 如果发送的任意帧类型为RTS并且长度为16，就认为是CSI触发的RTS帧，加上RTS标记 */
    if (tx_ctl->frame_header_length == MAC_CONTROL_FRAME_LEN &&
        (mac_get_frame_type_and_subtype((osal_u8 *)mac_header) == (WLAN_FC0_SUBTYPE_RTS | WLAN_FC0_TYPE_CTL))) {
        tx_ctl->is_custom_rts = OSAL_TRUE;
    }
    /* 调用发送管理帧接口 */
    ret = hmac_tx_mgmt(hmac_vap, netbuf, (osal_u16)len, OSAL_TRUE);
    if (ret != OAL_SUCC) {
        oal_netbuf_free(netbuf);
    }
    return (osal_s32)ret;
...
#ifndef _PRE_WLAN_FEATURE_POWERSAVE // 使能低功耗特性，需要配置dtim period，不使能暗屏dtim2策略
    /* 暗屏实施dtim2策略 */
...
WIFI_TCM_TEXT WIFI_HMAC_TCM_TEXT_SMALLER osal_s32 hmac_config_tx_comp_schedule(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    osal_s32 ret;
...
osal_s32 hmac_config_get_tid_info(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    hmac_tid_stru *tid_queue = OAL_PTR_NULL;
    hmac_user_stru *hmac_user = OAL_PTR_NULL;
    osal_u32 idx;
    tid_statistics_stru tid_stat[WLAN_TID_MAX_NUM];
    osal_u16 usr_idx;
    ...
    if (hmac_vap_find_user_by_macaddr_etc(hmac_vap, (osal_u8 *)msg->data, &usr_idx) != OAL_SUCC) {
        return OAL_FAIL;
    }

    hmac_user = (hmac_user_stru *)mac_res_get_hmac_user_etc(usr_idx);
    if (hmac_user == OSAL_NULL) {
        return OAL_FAIL;
    }

    for (idx = 0; idx < WLAN_TID_MAX_NUM; idx++) {
        tid_queue = &(hmac_user->tx_tid_queue[idx]);
        tid_stat[idx].tid_num = idx;
        tid_stat[idx].mpdu_num = tid_queue->mpdu_num;
        tid_stat[idx].tid_buffer_frame_len = tid_queue->tid_buffer_frame_len;
    }

    memcpy_s(msg->rsp, msg->rsp_buf_len, tid_stat, sizeof(tid_stat));
    msg->rsp_len = (osal_u16)sizeof(tid_stat);
    return OAL_SUCC;
}

osal_s32 hmac_config_set_dscp_tid_map(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    osal_u8 idx;
    mac_dscp_tid_map_stru *map = (mac_dscp_tid_map_stru *)(msg->data);
    osal_u8 dscp;
    osal_u8 tid;

    if (map->len > HMAC_MAX_DSCP_VALUE_NUM) {
        return OAL_FAIL;
    }

    for (idx = 0; idx < map->len; idx++) {
        hmac_vap->dscp_tid_map[idx] = map->dscp_tid_map[idx];
    }
...
#ifdef _PRE_WLAN_FEATURE_STA_CHANNEL_SCORING
osal_s32 hmac_config_set_channel_scoring_enable(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    g_channel_scoring_enable = (osal_u8)(*(msg->data));
    return OAL_SUCC;
}
 
osal_u8 hmac_get_channel_scoring_enable(osal_void)
{
    return g_channel_scoring_enable;
}
#endif
