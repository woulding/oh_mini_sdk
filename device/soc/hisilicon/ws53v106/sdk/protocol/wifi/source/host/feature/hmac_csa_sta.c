...
OSAL_STATIC osal_void hmac_sta_csa_fsm_init_entry(osal_void *ctx)
{
    ...
    csa_fsm_info->first_compare_flag = OSAL_TRUE;
...
OSAL_STATIC osal_void hmac_sta_csa_fsm_trans_to_state(hmac_vap_stru *hmac_vap, osal_u8 state)
{
    ...
    osal_void *csa_pm_cb = hmac_get_feature_fhook(HMAC_FHOOK_PM_CSA_SLEEP_ALLOW);
    ...
    /* CSA切换过程中暂时不允许深睡，切换完成后再放开 */
    if (csa_pm_cb != OSAL_NULL) {
        ((hmac_sta_pm_csa_flag_set_cb)csa_pm_cb)(now_next_status);
    }
    ...
    const oal_ieee80211_channel_sw_ie * const csa_info)
{
    ...
    if (csa_info->count >= csa_fsm_info->sta_csa_last_cnt && csa_info->new_ch_num == csa_vap_info->new_channel) {
        /* 只有在第一次比较并且计数相等时才不触发invalid状态防攻击，因为此时很可能是接收到了第一个beacon的csa ie */
        /* 该值与action帧中携带的是完全一致的 */
        if (csa_fsm_info->first_compare_flag == OSAL_TRUE && csa_info->count == csa_fsm_info->sta_csa_last_cnt) {
            oam_warning_log0(0, OAM_SF_ANY,
                "{hmac_sta_csa_fsm_start_get_csa_ie::The first two CSA IE are consistent, skipping the check}");
        } else {
    ...
    /* 生效一次后，只要后来的计数大于等于之前的计数，就都认为是异常 */
    csa_fsm_info->first_compare_flag = OSAL_FALSE;
    csa_fsm_info->sta_csa_last_cnt = csa_info->count;
    csa_vap_info->new_channel = csa_info->new_ch_num;
