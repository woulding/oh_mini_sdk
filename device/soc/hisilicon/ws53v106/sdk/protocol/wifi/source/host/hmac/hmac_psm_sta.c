...
*****************************************************************************/
osal_void hmac_psm_update_bcn_rf_chain(const hmac_vap_stru *hmac_vap, osal_s8 rssi)
{
...
osal_s32 hmac_set_psm_param(hmac_vap_stru *hmac_vap, mac_cfg_ps_param_stru *ps_param)
{
    ...
    osal_u8 init_pm_info = OSAL_FALSE;
    osal_s32 ret;
    ...
    if (init_pm_info == OSAL_TRUE) {
#ifdef _PRE_WLAN_FEATURE_POWERSAVE
        hal_init_pm_info_sync(hmac_vap->hal_vap);
#endif

        /* 缺省配置beacon超时，同步到device，重置自适应调整的beacon超时时间 */
        if (ps_param->beacon_timeout == 0 && hmac_vap->beacon_timeout != 0) {
            ps_param->beacon_timeout = hmac_vap->beacon_timeout;
        }
