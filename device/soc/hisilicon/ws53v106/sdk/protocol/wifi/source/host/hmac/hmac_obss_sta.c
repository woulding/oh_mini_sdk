...
/*****************************************************************************
 功能描述  : 准备obss扫描参数
*****************************************************************************/
OAL_STATIC osal_void hmac_prepare_obss_scan_base_param(mac_scan_req_stru *scan_params, const hmac_vap_stru *hmac_vap)
{
    ...
    is_random_mac_addr_scan = wlan_customize_etc->random_mac_addr_scan;
#else
    is_random_mac_addr_scan = hmac_device->scan_mgmt.is_random_mac_addr_scan;
#endif
    ...
    /* 4.OBSS扫描设置Source MAC ADDRESS */
    if ((is_random_mac_addr_scan != 0) && ((hmac_device->mac_oui[0] != 0) ||
        (hmac_device->mac_oui[1] != 0) || (hmac_device->mac_oui[2] != 0))) { /* 确保第2位不为0 */
        oal_random_ether_addr(scan_params->sour_mac_addr, WLAN_MAC_ADDR_LEN); /* 随机生成MAC地址 */
...
OAL_STATIC osal_u32 hmac_scan_prepare_obss_scan_params(mac_scan_req_stru *scan_params, const hmac_vap_stru *hmac_vap)
{
    ...
    /* 准备OBSS扫描的基本参数 */
    hmac_prepare_obss_scan_base_param(scan_params, hmac_vap);
...
OAL_STATIC osal_u32 hmac_scan_obss_timeout_fn(osal_void *arg)
{
    ...
    if (scan_now == OSAL_TRUE) {
        if (g_obss_sta_scan_disable == OSAL_TRUE) {
            oam_warning_log0(0, OAM_SF_SCAN, "{OBSS SCAN ABORT}");
            return OAL_SUCC;
        }
        /* 准备OBSS扫描参数，准备发起扫描 */
