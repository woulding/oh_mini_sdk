    ...
    hmac_disasoc_misc_stru *disasoc_misc_stru)
{
    ...
    /* 抛事件上报内核，已经去关联某个STA */
    hmac_handle_disconnect_rsp_ap_etc(hmac_vap, hmac_user, OAL_TRUE, disasoc_reason_code);
    /* 发去关联帧 */
...
#endif
        frw_util_notifier_notify(WLAN_UTIL_NOTIFIER_EVENT_RX_BEACON, hmac_vap);
    }
    ...
    oal_netbuf_stru *action_frame)
{
    ...
    /* 管理帧后部保存相关信息 */
    action_info_offset = tx_ctl->mpdu_payload_len - (osal_u16)sizeof(hmac_ctx_action_event_stru);
