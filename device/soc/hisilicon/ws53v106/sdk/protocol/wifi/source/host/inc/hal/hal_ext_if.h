...
#else
#define HAL_PM_DEF_EXT_TSF_OFFSET           3550       /* 外置tsf唤醒提前量单位us */
#endif
#endif
...
#define hal_m2s_check_rssi_on(_pst_handler)   \
    ((((*(osal_u8*)&((_pst_handler)->hal_m2s_fsm.m2s_mode)) & WLAN_M2S_TRIGGER_MODE_RSSI) != 0) ?    \
        OSAL_TRUE : OSAL_FALSE)
...
#define hal_m2s_check_test_on(_pst_handler)   \
    ((((*(osal_u8*)&((_pst_handler)->hal_m2s_fsm.m2s_mode)) & WLAN_M2S_TRIGGER_MODE_TEST) != 0) ?    \
        OSAL_TRUE : OSAL_FALSE)
...
#define hal_m2s_check_spec_on(_pst_handler)   \
    ((((*(osal_u8*)&((_pst_handler)->hal_m2s_fsm.m2s_mode)) & WLAN_M2S_TRIGGER_MODE_SPEC) != 0) ?    \
        OSAL_TRUE : OSAL_FALSE)
#ifdef _PRE_WLAN_FEATURE_M2S
...
static INLINE__ osal_void hal_reg_read16_tbl(hal_reg16_cfg_stru *reg_tbl, osal_u8 tbl_len)
{
    osal_u8 index;
    for (index = 0; index < tbl_len; index++) {
        reg_tbl[index].value = hal_reg_read16(reg_tbl[index].address);
    }
}
