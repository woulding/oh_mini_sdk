...
osal_void hmac_psm_debug_info_dump(osal_void)
{
    pm_tsf_cali_res_stru *tsf_cali = OSAL_NULL;
    osal_u16 *sleep_debug_cnt;
    osal_u16 *tsf_diff;
    osal_u32 cnt_size;
    osal_u32 tsf_size;
    dft_pm_debug_info_stru psm_debug_info;
    frw_msg msg_to_device = {0};
    osal_u8 flag = PM_DEBUG_GET_DEBUG_INFO | PM_DEBUG_CLEAR_ALL_DEBUG_INFO;

    (osal_void)memset_s(&psm_debug_info, sizeof(dft_pm_debug_info_stru), 0, sizeof(dft_pm_debug_info_stru));
    frw_msg_init((osal_u8 *)&flag, sizeof(osal_u8), (osal_u8 *)&psm_debug_info, sizeof(dft_pm_debug_info_stru),
        &msg_to_device);
    frw_send_msg_to_device(0, WLAN_MSG_H2D_C_CFG_GET_PM_DEBUG_INFO, &msg_to_device, OSAL_TRUE);

    tsf_cali = (pm_tsf_cali_res_stru *)&psm_debug_info.tsf_cali;
    sleep_debug_cnt = psm_debug_info.sleep_debug_cnt;
    tsf_diff = psm_debug_info.tsf_diff;
    if (sleep_debug_cnt == OSAL_NULL || tsf_diff == OSAL_NULL || tsf_cali == OSAL_NULL) {
        return;
    }

    wifi_printf_always("flag %d frw %d hcc %d scan %d txq %d t %d wtxq %d tx %d bo %d\n",
        sleep_debug_cnt[HAL_PM_NOT_SLEEP_FLAG], sleep_debug_cnt[HAL_PM_NOT_SLEEP_FRW],
        sleep_debug_cnt[HAL_PM_NOT_SLEEP_HCC], sleep_debug_cnt[HAL_PM_NOT_SLEEP_SCAN],
        sleep_debug_cnt[HAL_PM_NOT_SLEEP_TX_QUE], sleep_debug_cnt[HAL_PM_NOT_SLEEP_TIME],
        sleep_debug_cnt[HAL_PM_NOT_SLEEP_WAIT_TX_QUE], sleep_debug_cnt[HAL_PM_NOT_SLEEP_TX],
        sleep_debug_cnt[HAL_PM_NOT_SLEEP_BCN_TIMEOUT]);
    wifi_printf_always("hhcc %d hfrw %d htxq %d hto %d vote %d bc %d uc %d wk %d\n",
        sleep_debug_cnt[HAL_PM_DISALLOW_HOST_HCC], sleep_debug_cnt[HAL_PM_DISALLOW_HOST_FRW],
        sleep_debug_cnt[HAL_PM_DISALLOW_HOST_TX_QUE], sleep_debug_cnt[HAL_PM_DISALLOW_HOST_BCN_TIMEOUT],
        sleep_debug_cnt[HAL_PM_VOTE_SLEEP_FLAG], sleep_debug_cnt[HAL_PM_NOT_SLEEP_BCAST],
        sleep_debug_cnt[HAL_PM_NOT_SLEEP_UCAST], sleep_debug_cnt[HAL_PM_NOT_SLEEP_WK]);
    wifi_printf_always("po tsf diff %d %d %d %d, ne tsf diff %d %d %d %d\n",
        tsf_diff[HAL_PM_TSF_BEGIN], tsf_diff[HAL_PM_TSF_NOR], tsf_diff[HAL_PM_TSF_WAR], tsf_diff[HAL_PM_TSF_ERR],
        tsf_diff[HAL_PM_TSF_END + HAL_PM_TSF_BEGIN], tsf_diff[HAL_PM_TSF_END + HAL_PM_TSF_NOR],
        tsf_diff[HAL_PM_TSF_END + HAL_PM_TSF_WAR], tsf_diff[HAL_PM_TSF_END + HAL_PM_TSF_ERR]);
    wifi_printf_always("ne cnt %d ratio %d po cnt %d ratio %d\n",
        tsf_cali->negetive_cnt, tsf_cali->negetive_ratio, tsf_cali->positive_cnt, tsf_cali->positive_ratio);
}
...
#ifdef _PRE_WLAN_FEATURE_NET_DIAGNOSIS
    /* 开关低功耗通知维测更新统计 */
    hmac_net_diagnosis_update_bcn_stat(hmac_vap);
#endif
...
OSAL_STATIC osal_u32 hmac_config_set_sta_pm_on_check(hmac_vap_stru *hmac_vap, const mac_cfg_ps_open_stru *pm_cfg)
{
#ifdef _PRE_WLAN_FEATURE_POWERSAVE
    hmac_sta_pm_ctrl_info *sta_pm_ctrl = hmac_sta_pm_get_ctrl();
#endif

    if (osal_unlikely(pm_cfg == OSAL_NULL)) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_config_set_sta_pm_on_check, pm_cfg is null}");
        return OAL_FAIL;
    }

    /* p2p模式不支持低功耗 */
    if (hmac_vap->vap_mode != WLAN_VAP_MODE_BSS_STA || !is_legacy_vap(hmac_vap)) {
        oam_warning_log2(0, 0, "vap_id[%d] {hmac_config_set_sta_pm_on_check::vap[%d] isn't sta or legacy vap}",
            hmac_vap->vap_id, hmac_vap->vap_mode);
        return OAL_FAIL;
    }
#ifdef _PRE_WLAN_FEATURE_POWERSAVE
    if (sta_pm_ctrl->ps_enable_ctrl == OSAL_FALSE && pm_cfg->pm_enable != OSAL_FALSE) {
        return OAL_FAIL;
    }
#endif
...
OSAL_STATIC osal_s32 hmac_config_set_sta_pm_on(hmac_vap_stru *hmac_vap, mac_cfg_ps_open_stru *pm_cfg)
{
    ...
    if (hmac_config_set_sta_pm_on_check(hmac_vap, pm_cfg) != OAL_SUCC) {
        return OAL_FAIL;
    }
...
#ifdef _PRE_WLAN_FEATURE_POWERSAVE
    sta_pm_ctrl->ps_enable_ctrl = (sta_pm_open->pm_enable == MAC_STA_PM_SWITCH_OFF) ? OSAL_FALSE : OSAL_TRUE;
#endif
...
osal_void hmac_config_mvap_set_sta_pm_on(hmac_vap_stru *hmac_vap, oal_bool_enum mvap_up)
{
    ...
    if (mvap_up == OAL_TRUE && !(hmac_vap->vap_state == MAC_VAP_STATE_UP || hmac_vap->vap_state == MAC_VAP_STATE_PAUSE
...
OAL_STATIC osal_s32 hmac_ccpriv_sta_psm_param(hmac_vap_stru *hmac_vap, const osal_s8 *param)
{
    ...
    if (!is_legacy_sta(hmac_vap)) {
        oam_warning_log0(0, OAM_SF_ANY, "{hmac_ccpriv_sta_psm_param:: not sta, disallow cfg psm para!}");
        return OAL_FAIL;
    }
...
#ifdef _PRE_WLAN_FEATURE_POWERSAVE
    sta_pm_ctrl->ps_enable_ctrl = (sta_pm_open == MAC_STA_PM_SWITCH_OFF) ? OSAL_FALSE : OSAL_TRUE;
#endif
...
/*****************************************************************************
 ...
 功能描述  : hmac低功耗状态同步功能开启/关闭
*****************************************************************************/
OSAL_STATIC osal_s32  hmac_ccpriv_sta_system_psm_set(hmac_vap_stru *hmac_vap, const osal_s8 *pc_param)
{
    osal_s32                          ret;
...
*****************************************************************************/
OSAL_STATIC osal_s32 hmac_config_set_ext_offset(hmac_vap_stru *hmac_vap, osal_u16 ext_tbtt_offset)
{
...
*****************************************************************************/
OSAL_STATIC osal_s32  hmac_ccpriv_set_ext_offset(hmac_vap_stru *hmac_vap, const osal_s8 *pc_param)
{
    osal_s32                          ret;
    ...
    return hmac_config_set_ext_offset(hmac_vap, (osal_u16)ext_tbtt_offset);
}

/*****************************************************************************
 功能描述  : hmac配置外置tbtt offset uapi接口W2H消息回调
*****************************************************************************/
OSAL_STATIC osal_s32 hmac_config_set_ext_offset_cb(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
...
/*****************************************************************************
 功能描述  : hmac系统低功耗维测
*****************************************************************************/
OSAL_STATIC osal_u32 hmac_system_psm_debug(hmac_vap_stru *hmac_vap, osal_u8 psm_debug_cmd, osal_u8 psm_debug_switch)
{
    ...
    if (psm_debug_cmd == STA_SYS_PM_STATE) { /* 参数范围是1/0 */
        ...
        oam_warning_log1(0, OAM_SF_CFG, "hal_pm_debug plt all wkup cnt[%lu]", sleep_debug.pm_all_wkup_count);
#endif
#endif
    }

    if (psm_debug_cmd == STA_NOT_SLEEP_REASON_DUMP) {
        wifi_printf_always("{hmac_system_psm_debug::not_sleep_reason scan[%lu] vap_num[%lu] ps_mode[%lu] app[%lu]"
            " hcc[%lu] frw[%lu] dev[%lu] wkup[%lu]}\n",
            g_host_sleep_check_debug[HMAC_NOT_SLEEP_SCAN], g_host_sleep_check_debug[HMAC_NOT_SLEEP_VAP_NUM],
            g_host_sleep_check_debug[HMAC_NOT_SLEEP_PS_MODE], g_host_sleep_check_debug[HMAC_NOT_SLEEP_APP],
            g_host_sleep_check_debug[HMAC_NOT_SLEEP_HCC], g_host_sleep_check_debug[HMAC_NOT_SLEEP_FRW],
            g_host_sleep_check_debug[HMAC_NOT_SLEEP_DEV_SLEEP], g_host_sleep_check_debug[HMAC_NOT_SLEEP_HOST_WAKEUP]);
        wifi_printf_always("{tx cnt[%lu] dev allow[%lu] disallow[%lu] wkup fail[%lu],"
            " host tid_tx[%lu] read reg[%lu]}\n",
            g_host_sleep_check_debug[HMAC_NOT_SLEEP_TX_CNT], g_host_sleep_check_debug[HMAC_NOT_SLEEP_DEV_ALLOW_SLEEP],
            g_host_sleep_check_debug[HMAC_NOT_SLEEP_DEV_DISALLOW_SLEEP],
            g_host_sleep_check_debug[HMAC_NOT_SLEEP_HOST_WAKEUP_FAIL],
            g_host_sleep_check_debug[HMAC_NOT_SLEEP_HOST_TID], g_host_sleep_check_debug[HMAC_NOT_SLEEP_READ_REG]);
    ...
    msg.data_len = (osal_u16)sizeof(psm_debug_info);
    ret = frw_send_msg_to_device(hmac_vap->vap_id, WLAN_MSG_H2D_C_CFG_GET_SYS_PM_STAT, &msg, OSAL_TRUE);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_PWR, "{hmac_system_psm_debug::frw_send_msg_to_device failed[%d].}", ret);
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
...
/*****************************************************************************
 ...
 功能描述  : hmac维测功能开启/关闭
*****************************************************************************/
OSAL_STATIC osal_s32  hmac_ccpriv_system_psm_debug(hmac_vap_stru *hmac_vap, const osal_s8 *pc_param)
{
    osal_s32                          ret;
    ...
    return (osal_s32)hmac_system_psm_debug(hmac_vap, (osal_u8)psm_debug_cmd, (osal_u8)psm_debug_switch);
}
#endif

OSAL_STATIC osal_s32 hmac_config_psm_debug(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
#ifndef _PRE_FEATURE_WS53_DEVICE_MODE
    osal_u32 psm_debug_cfg;

    if (osal_unlikely((hmac_vap == OSAL_NULL) || (msg->data == OSAL_NULL))) {
        oam_warning_log0(0, OAM_SF_PWR, "{hmac_config_psm_debug::hmac_vap / param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    psm_debug_cfg = *((osal_u32 *)msg->data);

    return (osal_s32)hmac_system_psm_debug(hmac_vap,
        (osal_u8)(psm_debug_cfg & 0xFFFF), (osal_u8)(psm_debug_cfg >> BIT_OFFSET_16));
#else
    unref_param(hmac_vap);
    unref_param(msg);
    return OAL_SUCC;
...
*****************************************************************************/
WIFI_HMAC_TCM_TEXT_SMALLER hmac_sta_pm_ctrl_info *hmac_sta_pm_get_ctrl(osal_void)
{
...
osal_void hmac_sta_pm_csa_flag_set(osal_u8 csa_running_flag)
{
    hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl = hmac_sta_pm_get_ctrl();

    /* CSA在运行中，需要设置状态，用于阻止深睡 */
    hmac_sta_pm_ctrl->csa_is_waiting_flag = csa_running_flag;
    oam_warning_log1(0, OAM_SF_PWR, "hmac_sta_pm_csa_flag_set:: csa_is_waiting_flag[%d]",
        hmac_sta_pm_ctrl->csa_is_waiting_flag);
    return;
}
...
*****************************************************************************/
OSAL_STATIC osal_s32 hmac_sta_pm_wait_ack_check(const osal_void *para)
{
...
*****************************************************************************/
OSAL_STATIC osal_u32 hmac_sta_pm_ipc_cmd_send(hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl, osal_u8 hcc_ch_id)
{
    osal_u8 rsv = 0;
...
*****************************************************************************/
OSAL_STATIC osal_u32 hmac_sta_pm_cmd_send(hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl)
{
...
*****************************************************************************/
OSAL_STATIC osal_u32 hmac_sta_pm_wakeup_wait_ack(hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl)
{
    osal_s32 ret;
...
/*****************************************************************************
 ...
 功能描述  : 唤醒失败情况下的处理
*****************************************************************************/
OSAL_STATIC osal_void hmac_sta_pm_wakeup_fail_proc(hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl)
{
    ...
    hmac_sleep_check_debug_cnt_incr(HMAC_NOT_SLEEP_HOST_WAKEUP_FAIL);
}
...
*****************************************************************************/
WIFI_HMAC_TCM_TEXT_SMALLER OSAL_STATIC osal_u32 hmac_sta_pm_wakeup_check(hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl,
    osal_u32 datatype)
{
    /* 低功耗未使能不进行唤醒 */
...
*****************************************************************************/
WIFI_HMAC_TCM_TEXT_SMALLER osal_u32 hmac_sta_pm_wakeup_dev(uintptr_t *buf, osal_u32 len, osal_u32 data_type)
{
    ...
    /* 检查是否需要唤醒操作 */
    if (hmac_sta_pm_wakeup_check(hmac_sta_pm_ctrl, data_type) == OSAL_FALSE) {
    ...
    osal_mutex_unlock(&hmac_sta_pm_ctrl->sleep_state_mutex);
    hmac_sleep_check_debug_cnt_incr(HMAC_NOT_SLEEP_HOST_WAKEUP);
    return OAL_SUCC;
}
...
******************************************************************************/
WIFI_HMAC_TCM_TEXT_SMALLER osal_u32 hmac_wifi_pm_get_host_allow_sleep()
{
    hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl = hmac_sta_pm_get_ctrl();
    osal_u8 ret = 0;
    ...
    if (ret == OSAL_TRUE) {
        hmac_sleep_check_debug_cnt_incr(HMAC_NOT_SLEEP_READ_REG);
    }
    return ret;
}
...
*****************************************************************************/
OSAL_STATIC osal_u32 hmac_sta_pm_wakeup_done_callback(osal_u8 *cb_data)
{
    hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl = hmac_sta_pm_get_ctrl();
    unref_param(cb_data);
...
*****************************************************************************/
OSAL_STATIC osal_u32 hmac_sta_pm_sleep_wait_ack(hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl)
{
    osal_s32 ret;
...
*****************************************************************************/
OSAL_STATIC osal_u32 hmac_sta_pm_check_hcc_empty(osal_void)
{
    osal_u8 hcc_ch_id = HCC_CHANNEL_AP;
    hcc_handler *hcc = hcc_get_handler(hcc_ch_id);
    osal_u8 i;
...
OSAL_STATIC osal_u32 hmac_sta_pm_check_up_vap_ps_enable(hmac_device_stru *hmac_device)
{
    osal_u32 vap_idx;
...
OSAL_STATIC osal_u8 hmac_sta_pm_app_allow_sleep(osal_void)
{
#ifdef _PRE_FEATURE_WS53_DEVICE_MODE
    return OSAL_FALSE;
#else
#ifndef BUILD_UT
    pm_veto_t *pm_veto_info = uapi_pm_veto_get_info();
    osal_u32 veto_id = PM_IPERF_VETO_ID;

    /* IPERF启动，禁止A核进入休眠 */
    for (; veto_id < PM_VETO_ID_MAX; veto_id++) {
        if (pm_veto_info->veto_counts.sub_counts[veto_id] != 0) {
            return OSAL_FALSE;
        }
    }
#endif
...
OSAL_STATIC osal_bool hmac_sta_check_tid_is_empty(hmac_device_stru *hmac_device)
{
    hmac_vap_stru *mac_vap_temp = OSAL_NULL;
    hmac_user_stru *hmac_user = OSAL_NULL;
    osal_u32 total_mpdu_num;

    if (mac_device_find_up_sta_etc(hmac_device, &mac_vap_temp) != OAL_SUCC) {
        return OSAL_TRUE;
    }
    hmac_user = (hmac_user_stru *)mac_res_get_hmac_user_etc(mac_vap_temp->assoc_vap_id);
    total_mpdu_num = hmac_psm_tid_mpdu_num(hmac_user);
    if (total_mpdu_num < WLAN_TID_MPDU_NUM_LIMIT && total_mpdu_num > 0) {
        return OSAL_FALSE;
    }
    return OSAL_TRUE;
}
...
*****************************************************************************/
OSAL_STATIC osal_u32 hmac_sta_pm_sleep_check(hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl)
{
    hmac_device_stru *hmac_device = hmac_res_get_mac_dev_etc(0);
    ...
    if (hmac_sta_pm_ctrl->wlan_dev_state == HMAC_ALLOW_SLEEP) {
        hmac_sleep_check_debug_cnt_incr(HMAC_NOT_SLEEP_DEV_SLEEP);
        return OSAL_FALSE;
    }

    if (hmac_sta_pm_app_allow_sleep() == OSAL_FALSE) {
        hmac_sleep_check_debug_cnt_incr(HMAC_NOT_SLEEP_APP);
        return OSAL_FALSE;
    }
    ...
    /* 单STA模式才判断tid队列 */
    if (hmac_sta_check_tid_is_empty(hmac_device) == OSAL_FALSE) {
        hmac_sleep_check_debug_cnt_incr(HMAC_NOT_SLEEP_HOST_TID);
        return OSAL_FALSE;
    }
    ...
    if (hmac_sta_pm_ctrl->csa_is_waiting_flag == OSAL_TRUE) {
        hmac_sleep_check_debug_cnt_incr(HMAC_NOT_SLEEP_CSA_SWTICH_RUNNING);
        return OSAL_FALSE;
    }
    return OSAL_TRUE;
}
...
*****************************************************************************/
OSAL_STATIC osal_void hmac_sta_pm_sleep_fail_proc(hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl)
{
...
*****************************************************************************/
OSAL_STATIC osal_void hmac_sta_pm_sleep_stage_proc(hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl)
{
    if (hmac_sta_pm_ctrl->pm_stage == HMAC_SLEEP_ALLOW_RCV) {
        ...
        hmac_sleep_check_debug_cnt_incr(HMAC_NOT_SLEEP_DEV_ALLOW_SLEEP);
    }
    ...
    if (hmac_sta_pm_ctrl->pm_stage == HMAC_SLEEP_DISALLOW_RCV) {
        ...
        hmac_sta_pm_ctrl->wlan_dev_state = HMAC_DISALLOW_SLEEP;
        hmac_sta_pm_feed_wdg(hmac_sta_pm_ctrl);
        hmac_sleep_check_debug_cnt_incr(HMAC_NOT_SLEEP_DEV_DISALLOW_SLEEP);
    }
...
/*****************************************************************************
 函 数 名  : hmac_pm_sta_wakeup_ipc
 功能描述  : sta类型VAP主动唤醒ipc
*****************************************************************************/
osal_void hmac_pm_sta_wakeup_ipc(osal_void)
{
    hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl = hmac_sta_pm_get_ctrl();
    ...
    /* 上报TBTT中断,说明device未睡眠,但state为HMAC_ALLOW_SLEEP,状态不同步,需要主动唤醒ipc */
    hmac_sta_pm_wakeup_dev(OSAL_NULL, 0, HMAC_PM_CFG_WKUP);
}
...
*****************************************************************************/
OSAL_STATIC osal_u32 hmac_sta_pm_sleep_cmd(osal_void)
{
    hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl = hmac_sta_pm_get_ctrl();
    osal_u32 ret = OAL_FAIL;
    osal_u8 i = 0;
    ...
    /* 需要在设置allow sleep并释放锁后调用，否则会出现死锁 */
    if (hmac_sta_pm_ctrl->wlan_dev_state == HMAC_ALLOW_SLEEP) {
        hmac_ba_sta_force_send_reorder();
    }
    return OAL_SUCC;
}
...
*****************************************************************************/
OSAL_STATIC osal_u32 hmac_sta_pm_allow_sleep_callback(osal_u8 *cb_data)
{
    hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl = hmac_sta_pm_get_ctrl();
    unref_param(cb_data);
...
*****************************************************************************/
OSAL_STATIC osal_u32 hmac_sta_pm_disallow_sleep_callback(osal_u8 *cb_data)
{
    hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl = hmac_sta_pm_get_ctrl();
    unref_param(cb_data);
...
/*****************************************************************************
 ...
 功能描述  : 睡眠定时器超时的处理函数
*****************************************************************************/
OSAL_STATIC osal_u32 hmac_sta_pm_wdg_timeout(osal_void *timeout_arg)
{
...
osal_u32 hmac_sta_pm_ctrl_set(osal_u32 flag)
{
    ...
    if (flag == OSAL_TRUE) {
        ...
        if (hmac_sta_pm_wakeup_dev(OSAL_NULL, 0, HMAC_PM_CFG_WKUP) == OAL_FAIL) {
            return OAL_FAIL;
        }
        hmac_sta_pm_stop_wdg(hmac_sta_pm_ctrl);
    }
    hmac_sta_pm_ctrl->wlan_pm_enable = (osal_u8)flag;
    return OAL_SUCC;
}

OSAL_STATIC osal_bool hmac_sta_pm_set_on(osal_void *notify_data)
{
    hmac_device_stru *hmac_device = hmac_res_get_mac_dev_etc(0);
    hmac_vap_stru *hmac_vap = OSAL_NULL;
    ...
    if (mac_device_find_up_sta_etc(hmac_device, &hmac_vap) != OAL_SUCC) {
        return OSAL_FALSE;
    }
...
osal_u32 hmac_sta_pm_ctrl_init(osal_void)
{
    ...
    hmac_sta_pm_ctrl->fail_sleep_count = 0;
    hmac_sta_pm_ctrl->ignor_linkloss = OSAL_FALSE;
    hmac_sta_pm_ctrl->ps_enable_ctrl = OSAL_TRUE;
    ...
    hmac_feature_hook_register(HMAC_FHOOK_PM_CSA_SLEEP_ALLOW, hmac_sta_pm_csa_flag_set);

    /* 注册dhcp完成后自动开启WiFi协议低功耗的回调 */
    frw_util_notifier_register(WLAN_UTIL_NOTIFIER_EVENT_INETADDR_NOTIFIER_UP, hmac_sta_pm_set_on);

    frw_msg_hook_register(WLAN_MSG_W2H_CFG_SET_EXT_TBTT_OFFSET, hmac_config_set_ext_offset_cb);
    frw_msg_hook_register(WLAN_MSG_W2H_CFG_PSM_DEBUG, hmac_config_psm_debug);
...
/*****************************************************************************
 ...
 功能描述  : PM模块退出
*****************************************************************************/
OSAL_STATIC osal_u32 hmac_sta_pm_ctrl_exit(osal_void)
{
    hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl = hmac_sta_pm_get_ctrl();
    osal_u8 hcc_ch_id = HCC_CHANNEL_AP;
    ...
    frw_msg_hook_unregister(WLAN_MSG_W2H_CFG_SET_EXT_TBTT_OFFSET);
    frw_msg_hook_unregister(WLAN_MSG_W2H_CFG_PSM_DEBUG);
