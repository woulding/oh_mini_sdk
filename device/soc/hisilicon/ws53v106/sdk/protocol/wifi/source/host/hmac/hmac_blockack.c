...
*****************************************************************************/
WIFI_HMAC_TCM_TEXT WIFI_TCM_TEXT OSAL_STATIC hmac_rx_buf_stru *hmac_ba_buffer_frame_in_reorder(
    hmac_ba_rx_stru *rx_hdl, osal_u16 seq_num, mac_rx_ctl_stru *cb_ctrl)
{
    osal_u16       buf_index;
...
*****************************************************************************/
WIFI_HMAC_TCM_TEXT WIFI_TCM_TEXT osal_u32 hmac_ba_filter_serv_etc(hmac_vap_stru *hmac_vap,
    hmac_user_stru *hmac_user, mac_rx_ctl_stru *cb_ctrl, oal_netbuf_head_stru *netbuf_header,
...
OAL_STATIC osal_u32  hmac_ba_send_reorder_timeout(hmac_ba_rx_stru *rx_ba, hmac_vap_stru *hmac_vap,
    osal_u16 *pus_timeout, hmac_tid_stru *tx_tid_info)
{
    osal_u32                  time_diff;
    osal_u32                  rx_timeout;
    oal_netbuf_head_stru        netbuf_head;
...
osal_void hmac_ba_sta_force_send_reorder(osal_void)
{
    hmac_device_stru *hmac_device = hmac_res_get_mac_dev_etc(0);
    hmac_vap_stru *hmac_vap = OSAL_NULL;
    hmac_user_stru *hmac_user = OSAL_NULL;
    hmac_ba_rx_stru *ba_rx_stru = OSAL_NULL;
    osal_u16 timeout = 0;
    osal_u8 tid = 0;
    ...
    for (tid = 0; tid < WLAN_TID_MAX_NUM; tid++) {
        ...
        if (ba_rx_stru == OSAL_NULL) {
            continue;
        }
        hmac_ba_send_reorder_timeout(ba_rx_stru, hmac_vap, (osal_u16 *)&timeout, &(hmac_user->tx_tid_queue[tid]));
    }
}
#endif
...
osal_u32  hmac_ba_timeout_fn_etc(osal_void *p_arg)
{
    ...
    if (direction == MAC_RECIPIENT_DELBA) {
        ...
        if (rx_ba->mpdu_cnt > 0) {
            hmac_ba_send_reorder_timeout(rx_ba, hmac_vap, &timeout, &(hmac_user->tx_tid_queue[tid]));
        }
...
/*****************************************************************************
 ...
 功能描述  : 过滤ampdu的每一个mpdu 有未确认报文需要入重传队列
*****************************************************************************/
WIFI_HMAC_TCM_TEXT WIFI_TCM_TEXT osal_u32 hmac_ba_filter_serv(hmac_vap_stru *hmac_vap,
    hmac_user_stru *hmac_user, const dmac_rx_ctl_stru *cb_ctrl, const mac_ieee80211_frame_stru *frame_hdr,
...
#ifdef _PRE_WLAN_FEATURE_POWERSAVE
    /* A核Wi-Fi业务睡眠，报文不进重排序队列 */
    if (pm_stat_cb != OSAL_NULL && ((hmac_sta_pm_get_wlan_dev_state_cb)pm_stat_cb)() == HMAC_ALLOW_SLEEP) {
        return OAL_SUCC;
    }
#endif
    return OAL_CONTINUE;
