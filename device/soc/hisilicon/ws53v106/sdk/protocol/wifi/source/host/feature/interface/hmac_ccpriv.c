...
osal_spinlock g_hmac_ccpriv_lock;
...
osal_void hmac_ccpriv_register(const osal_s8 *cmd_name, hmac_ccpriv_cmd_func func)
{
    ...
    cmd_node->func = func;
    osal_spin_lock(&g_hmac_ccpriv_lock);
    osal_list_add_tail(&cmd_node->entry_list, &g_ccpriv_cmd);
    osal_spin_unlock(&g_hmac_ccpriv_lock);
    return;
}
...
OSAL_STATIC osal_void hmac_ccpriv_exit(osal_void)
{
    ...
    osal_spin_lock_destroy(&g_hmac_ccpriv_lock);
    return;
}
...
OSAL_STATIC osal_s32 hmac_ccpriv_do_cmd(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    hmac_ccpriv_cmd_func cmd_func = OSAL_NULL;
    ...
    cmd_func = cmd_node->func;
    osal_spin_unlock(&g_hmac_ccpriv_lock);

    if (cmd_func == OSAL_NULL) {
        return OAL_ERR_CODE_INVALID_CONFIG;
    }
    param = info->param;
    return cmd_func(hmac_vap, param);
}
...
{
    /* 链表初始化 */
    osal_spin_lock_init(&g_hmac_ccpriv_lock);
