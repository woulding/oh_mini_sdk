...
#if defined(_PRE_OS_VERSION_LINUX) && defined(_PRE_OS_VERSION) && (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    if (type == WLAN_DATA_BASICTYPE && is_multicast != 0) {
    ...
    const mac_ieee80211_frame_stru *frame_hdr, const mac_rx_ctl_stru *rx_info, osal_s8 rssi, oal_netbuf_stru *netbuf)
{
    ...
    if (hmac_promisc_send_data_to_lan(hmac_vap, netbuf, type, is_multicast) != OAL_CONTINUE) {
        return OAL_ERR_CODE_CFG80211_BASE;
    }
...
OAL_STATIC osal_void hmac_promisc_pa_on(hmac_vap_stru *hmac_vap)
{
    hmac_device_stru *hmac_device = hmac_res_get_mac_dev_etc(0);
    ...
    if (hmac_vap->hal_device->hal_dev_fsm.oal_fsm.cur_state == HAL_DEVICE_IDLE_STATE) {
        ...
        wifi_printf("hmac_promisc_pa_on turned on.\r\n");
    }
}

OAL_STATIC osal_void hmac_promisc_pa_off(hmac_vap_stru *hmac_vap)
{
    hmac_device_stru *hmac_device = hmac_res_get_mac_dev_etc(0);
    ...
    if ((hmac_vap->vap_state != MAC_VAP_STATE_UP) && (hmac_vap->vap_state != MAC_VAP_STATE_PAUSE) &&
        ...
        wifi_printf("hmac_promisc_pa_on turned off.\r\n");
    }
}
...
OAL_STATIC osal_s32 hmac_config_set_monitor_switch(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if (promisc_switch == 1) {
        ...
        hmac_promisc_pa_on(hmac_vap);
    } else {
        /* 混杂关闭时注册 */
        hmac_promis_feature_hook_unregister();
        hmac_promisc_pa_off(hmac_vap);
    }
