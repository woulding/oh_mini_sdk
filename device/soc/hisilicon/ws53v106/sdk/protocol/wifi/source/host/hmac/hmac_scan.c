...
OAL_STATIC osal_void hmac_scan_encap_probe_req_frame_header(oal_netbuf_stru *mgmt_buf, const osal_u8 *bssid)
{
    ...
    hmac_device_stru *hmac_device = hmac_res_get_mac_dev_etc(0);
    hmac_encap_da_sa_bssid_fragment(mac_header, WLAN_PROTOCOL_VERSION | WLAN_FC0_TYPE_MGT | WLAN_FC0_SUBTYPE_PROBE_REQ,
        BROADCAST_MACADDR, hmac_device->scan_params.sour_mac_addr, bssid);
}
...
{
    osal_void *fhook = OSAL_NULL;
    mac_channel_stru radar_work_channel = {0};
    if (hal_scan_params->need_switch_back_home_channel == OSAL_TRUE) {
        ...
        if (fhook != OSAL_NULL) {
            radar_work_channel.chan_number = ((hmac_radar_sensor_get_work_ch_num_cb)fhook)();
            if (radar_work_channel.chan_number != 0) {
                radar_work_channel.band = WLAN_BAND_2G;
                radar_work_channel.en_bandwidth = WLAN_BAND_WIDTH_20M;
                radar_work_channel.chan_idx = radar_work_channel.chan_number - 1;
                hmac_mgmt_switch_channel(hal_device, &radar_work_channel, OSAL_TRUE);
                hmac_scan_switch_channel_notify_alg(hal_device, hmac_vap, &radar_work_channel);
            }
        }
    }
...
OAL_STATIC osal_u32 hmac_scan_send_bcast_probe(const hmac_device_stru *hmac_device, osal_u8 band, osal_u8 index)
{
    ...
    */
    if (hmac_vap->mib_info == OSAL_NULL) {
        oam_error_log3(0, 0, "{hmac_scan_send_bcast_probe:vap mib info is null,vap[%d], p_fn_cb[%p],scan_func[%d].}",
            hmac_device->scan_params.vap_id, (uintptr_t)hmac_device->scan_params.fn_cb,
    ...
    for (loop = 0; loop < send_times; loop++) {
        ...
        if (ret != OAL_SUCC) {
            oam_warning_log2(0, 0, "vap[%d] {hmac_scan_send_bcast_probe:hmac_mgmt_send_probe_request failed[%d]}",
                hmac_vap->vap_id, ret);
            break;
        }
    }
...
osal_void hmac_scan_begin(hmac_device_stru *hmac_device, hal_to_dmac_device_stru *hal_device)
{
    ...
    hmac_vap = mac_res_get_hmac_vap(scan_params->vap_id);
    if (hmac_vap == OSAL_NULL) {
        oam_error_log1(0, OAM_SF_SCAN, "vap[%d] {hmac_scan_begin::hmac_vap null.}", scan_params->vap_id);
        return;
    }
...
osal_void hmac_scan_prepare_end(hmac_device_stru *hmac_device, hal_to_dmac_device_stru *hal_device)
{
    ...
    if (hmac_vap == OSAL_NULL) {
        oam_warning_log1(0, OAM_SF_SCAN,
            "vap[%d] {hmac_scan_prepare_end::mac_res_get_hmac_vap fail}", hmac_device->scan_params.vap_id);
        return;
    }
...
osal_void hmac_scan_abort(hmac_device_stru *hmac_device)
{
    ...
    hmac_vap = mac_res_get_hmac_vap(hmac_device->scan_params.vap_id);
    if (hmac_vap == OSAL_NULL) {
        oam_error_log0(0, OAM_SF_SCAN, "{hmac_scan_abort::vap is null.}");
        return;
    }
...
OAL_STATIC osal_u32 hmac_scan_switch_home_channel_work_timeout(osal_void *arg)
{
    ...
    if (hmac_device->curr_scan_state != MAC_SCAN_STATE_RUNNING) {
        oam_warning_log1(0, 0, "vap[%d] {hmac_scan_switch_home_channel_work_timeout::scan aborted.}",
            hmac_device->scan_params.vap_id);
        return OAL_SUCC;
    }
    ...
    hmac_vap = mac_res_get_hmac_vap(hmac_device->scan_params.vap_id);
    if (hmac_vap == OSAL_NULL) {
        oam_error_log1(0, 0, "vap {hmac_scan_switch_home_channel_work_timeout::hmac_vap[%d] null.}",
            hmac_device->scan_params.vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
...
osal_s32 hmac_scan_channel_statistics_complete(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if (hmac_device->curr_scan_state != MAC_SCAN_STATE_RUNNING &&
        msg->data_len == sizeof(alg_intf_det_ch_stats_result)) {
...
*****************************************************************************/
OAL_STATIC oal_bool_enum_uint8 hmac_scan_is_hidden_ssid(osal_u8 vap_id, const hmac_scanned_bss_info *new_bss,
