...
static const cmd_one_int_args g_one_int_args[] = {
    ...
    {"arp_offload_enable", CCPRIV_SWITCH_OFF, CCPRIV_U8_MAX, WLAN_MSG_W2H_C_CFG_ENABLE_ARP_OFFLOAD},
...
osal_u32  uapi_ccpriv_set_ampdu_amsdu(oal_net_device_stru *net_dev, osal_s8 *param)
{
    ...
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{uapi_ccpriv_set_ampdu_amsdu:wal_get_cmd_one_arg return err_code [%d]!}", ret);
        return ret;
    }
    param_tmp += off_set;
    ...
    ret = wal_get_cmd_one_arg(param_tmp, ac_name, OAL_SIZEOF(ac_name), &off_set);
    if (ret != OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_ANY, "{uapi_ccpriv_set_ampdu_amsdu::force_tx_on set default param 0!}");
        force_tx_on = 0;
    } else {
        param_tmp += off_set;
        force_tx_on = (osal_s8)oal_atoi((const osal_s8 *)ac_name);
    }

    cfg_ampdu_amsdu.tx_switch   = (tx > 0) ? OSAL_TRUE : OSAL_FALSE;
    cfg_ampdu_amsdu.rx_switch   = (rx > 0) ? OSAL_TRUE : OSAL_FALSE;
    cfg_ampdu_amsdu.force_tx_on = (force_tx_on > 0) ? OSAL_TRUE : OSAL_FALSE;

    oam_warning_log3(0, OAM_SF_ANY, "{uapi_ccpriv_set_ampdu_amsdu:: tx_switch[%d] rx_switch[%d] force_tx_on[%d]!}",
                     cfg_ampdu_amsdu.tx_switch, cfg_ampdu_amsdu.rx_switch, cfg_ampdu_amsdu.force_tx_on);
    /***************************************************************************
                             抛事件到wal层处理
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(l_ret != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_ccpriv_set_tlv_cmd:wal_sync_post2hmac_no_rsp err_code [%d]!}", l_ret);
...
osal_u32 uapi_ccpriv_set_wow(oal_net_device_stru *netdev, osal_s8 *pc_param)
{
    ...
    /***************************************************************************
                                抛事件到wal层处理
    ***************************************************************************/
    ret = (td_u32)wal_sync_post2hmac_no_rsp(wal_util_get_vap_id(netdev), WLAN_MSG_W2H_CFG_SET_WOW,
...
osal_u32 uapi_ccpriv_set_wow_pattern(oal_net_device_stru *netdev, osal_s8 *pc_param)
{
    ...
    ***************************************************************************/
    ...
    retval = (td_u32)wal_sync_post2hmac_no_rsp(wal_util_get_vap_id(netdev), WLAN_MSG_W2H_CFG_SET_WOW_PATTERN,
