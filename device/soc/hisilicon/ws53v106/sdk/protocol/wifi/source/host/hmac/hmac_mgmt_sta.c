...
*****************************************************************************/
/* 与dmac层的同步修改 */
const hmac_data_rate_stru g_st_data_rates[DATARATES_80211G_NUM] = {
...
osal_u32 hmac_sta_wait_auth(hmac_vap_stru *hmac_vap, osal_void *msg)
{
    ...
    if ((mac_mib_get_authentication_mode(hmac_vap) == WLAN_WITP_AUTH_SAE) &&
        ...
        hmac_vap->mgmt_timetout_param.user_index = user_index;
        hmac_vap->mgmt_timetout_param.vap_id = hmac_vap->vap_id;
        frw_create_timer_entry(&hmac_vap->mgmt_timer, hmac_mgmt_timeout_sta, ((hmac_auth_req_stru *)msg)->timeout,
...
OAL_STATIC osal_void hmac_sta_sae_auth_proc(hmac_vap_stru *hmac_vap, oal_netbuf_stru *auth_req)
{
    ...
    status = mac_get_auth_status(oal_netbuf_header(auth_req));
    oam_warning_log2(0, OAM_SF_AUTH, "{hmac_sta_sae_auth_proc::[WPA3]auth seq [%d], status[%d]}", auth_seq, status);
    /* 获取STA的地址 */
    ...
    /* 收到AUTH第一帧，取消hmac_sta_wait_auth中起的定时器 */
    ...
    if (hmac_vap->vap_state == MAC_VAP_STATE_STA_WAIT_AUTH_SEQ2) {
        if ((status != MAC_ANTI_CLOGGING) && (status != MAC_FINITE_CYCLIC_GROUP_NOT_SUPPORTED)) {
            /* 切换STA 到MAC_VAP_STATE_STA_WAIT_AUTH_SEQ4 */
            hmac_fsm_change_state_etc(hmac_vap, MAC_VAP_STATE_STA_WAIT_AUTH_SEQ4);
            hmac_vap->mgmt_timetout_param.state = MAC_VAP_STATE_STA_WAIT_AUTH_SEQ4;
        } else {
        ...
        /* 当前还处于MAC_VAP_STATE_STA_WAIT_AUTH_SEQ4 状态，需等待wpa_supplicant 返回external auth status */
        hmac_vap->mgmt_timetout_param.state = MAC_VAP_STATE_STA_WAIT_AUTH_SEQ4;
    }
    frw_create_timer_entry(&hmac_vap->mgmt_timer, hmac_mgmt_timeout_sta, hmac_vap->mgmt_timer.timeout,
        &hmac_vap->mgmt_timetout_param, OAL_FALSE);
}
#endif
...
static osal_u32 hmac_sta_wait_auth_check(hmac_vap_stru *hmac_vap, osal_u8 *mac_hdr)
{
    osal_u16 err_code;
    ...
    if ((mac_get_frame_type_and_subtype(mac_hdr) == (WLAN_FC0_SUBTYPE_DEAUTH | WLAN_FC0_TYPE_MGT)) ||
        ...
        oam_warning_log1(0, OAM_SF_RX, "{hmac_sta_wait_auth_check:recv deauth or disassoc err[%d]}", err_code);
        return OAL_SUCC;
    }
...
osal_u32 hmac_sta_wait_auth_seq2_rx_etc(hmac_vap_stru *hmac_vap, osal_void *msg)
{
    ...
    ret = hmac_sta_wait_auth_check(hmac_vap, mac_hdr);
    if (ret != OAL_CONTINUE) {
        return ret;
    }
#ifdef _PRE_WLAN_FEATURE_WPA3
    ...
    ret = hmac_sta_wait_auth_seq2_rx_process(hmac_vap, mac_hdr);
    return ret;
}
...
#ifdef _PRE_WLAN_FEATURE_WPA3
    /* VAP时收到认证请求帧且认证类型为SAE 时直接发送给WPA_SUPPLICANT处理,不在此处组Auth Response帧 */
    if (auth_alg == WLAN_WITP_AUTH_SAE) {
        hmac_sta_sae_auth_proc(hmac_vap, (oal_netbuf_stru *)crx_event->netbuf);
        return OAL_SUCC;
    }
#endif
    /* 下面是WEP WLAN_WITP_AUTH_SHARED_KEY加密的流程 */
