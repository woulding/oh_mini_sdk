...
osal_void hmac_rx_ba_session_decr_etc(hmac_vap_stru *hmac_vap, osal_u8 tidno)
{
    if (mac_mib_get_RxBASessionNumber(hmac_vap) == 0) {
        oam_warning_log3(0, OAM_SF_BA, "vap_id[%d] {hmac_rx_ba_session_decr_etc::tid[%d] tx_session[%d] remove.}",
            hmac_vap->vap_id, tidno, mac_mib_get_RxBASessionNumber(hmac_vap));
        return;
    }
...
osal_void hmac_tx_ba_session_decr_etc(hmac_vap_stru *hmac_vap, osal_u8 tidno)
{
    oam_warning_log3(0, OAM_SF_BA, "vap_id[%d] {hmac_tx_ba_session_decr_etc::tid[%d] tx_session[%d] remove.}",
        hmac_vap->vap_id, tidno, mac_mib_get_TxBASessionNumber(hmac_vap));
...
osal_void hmac_encap_da_sa_bssid_fragment(osal_u8 *data, osal_u16 type_subtype,
    const osal_u8* da, const osal_u8* sa, const osal_u8* bssid)
{
    mac_hdr_set_frame_control(data, type_subtype);

    /* DA is address of STA requesting association */
    oal_set_mac_addr(data + WLAN_HDR_ADDR1_OFFSET, da);

    /* SA的值为dot11MACAddress的值 */
    oal_set_mac_addr(data + WLAN_HDR_ADDR2_OFFSET, sa);

    oal_set_mac_addr(data + WLAN_HDR_ADDR3_OFFSET, bssid);
    ...
    hmac_ba_tx_hdl_stru *tx_ba, osal_u8 tid)
{
    ...
    hmac_encap_da_sa_bssid_fragment(data, WLAN_PROTOCOL_VERSION | WLAN_FC0_TYPE_MGT | WLAN_FC0_SUBTYPE_ACTION,
        tx_ba->dst_addr, mac_mib_get_station_id(hmac_vap), hmac_vap->bssid);
    ...
    hmac_ba_rx_stru *addba_rsp, osal_u8 tid, osal_u8 status)
{
    ...
    /*************************************************************************/
    hmac_encap_da_sa_bssid_fragment(data, WLAN_PROTOCOL_VERSION | WLAN_FC0_TYPE_MGT | WLAN_FC0_SUBTYPE_ACTION,
        addba_rsp->transmit_addr, mac_mib_get_station_id(hmac_vap), hmac_vap->bssid);
    ...
    osal_u8 tid, mac_delba_initiator_enum_uint8 initiator, osal_u8 reason)
{
    ...
    /*************************************************************************/
    hmac_encap_da_sa_bssid_fragment(data, WLAN_PROTOCOL_VERSION | WLAN_FC0_TYPE_MGT | WLAN_FC0_SUBTYPE_ACTION,
        addr, mac_mib_get_station_id(hmac_vap), hmac_vap->bssid);
    ...
    hmac_ba_rx_stru *ba_rx_info, osal_u8 tid, osal_u8 status)
{
    ...
    return hmac_mgmt_tx_addba_rsp(hmac_vap, hmac_user, ba_rx_info, tid, status);
}
/*****************************************************************************
...
OAL_STATIC osal_u32 hmac_sa_query_del_user(hmac_vap_stru *hmac_vap, hmac_user_stru  *hmac_user)
{
    ...
    switch (vap_mode) {
        case WLAN_VAP_MODE_BSS_AP:
            /* 抛事件上报内核，已经去关联某个STA */
            hmac_handle_disconnect_rsp_ap_etc(hmac_vap, hmac_user, OAL_TRUE, MAC_DEAUTH_LV_SS);
            break;
...
OSAL_STATIC osal_void hmac_mgmt_rx_addba_rsp_debug(const mac_mgmt_rx_addba_rsp_ba_sync *ba_sync_stru)
{
