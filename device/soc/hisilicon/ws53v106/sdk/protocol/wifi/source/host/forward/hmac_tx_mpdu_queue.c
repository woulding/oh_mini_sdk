    ...
    hmac_tid_stru *tid_queue, osal_u8 mpdu_num)
{
    ...
    if (oal_netbuf_list_empty(&buff_head) != 0) {
        ...
        hmac_device->total_mpdu_num -= tid_queue->mpdu_num;
...
OSAL_STATIC osal_u32 hmac_tx_schedule_get_mpdu_num_left(osal_void)
{
...
WIFI_HMAC_TCM_TEXT WIFI_TCM_TEXT osal_u32 hmac_tx_schedule(hal_to_dmac_device_stru *hal_device, osal_u8 ac_num)
{
    ...
    hmac_user_stru *hmac_user = OSAL_NULL;
    hmac_device_stru *hmac_device = hmac_res_get_mac_dev_etc(0);
    osal_u32 mpdu_num_left = hmac_tx_schedule_get_mpdu_num_left();
...
#ifdef CONFIG_HMAC_SUPPORT_FLOW_CONTRL
/* 线程唤醒判断 */
OSAL_STATIC osal_s32 hmac_tid_tx_enqueue_wait_condition(const void *param)
{
    tid_queue_mpdu_num *queue_mpdu_num = (tid_queue_mpdu_num *)param;
    return ((queue_mpdu_num->single_mpdu_num < WLAN_SINGLE_TID_MPDU_NUM_LIMIT &&
        queue_mpdu_num->total_mpdu_num < WLAN_TID_MPDU_NUM_LIMIT) || (hcc_get_tx_sched_num() > 0));
}
#endif
...
*****************************************************************************/
WIFI_TCM_TEXT OSAL_STATIC osal_u32 hmac_tid_tx_queue_enqueue(hmac_device_stru *hmac_device,
    const hmac_vap_stru *hmac_vap, hmac_tid_stru *tid_queue, osal_u8 mpdu_num, oal_netbuf_stru *netbuf)
{
    /* 更新device结构体下的统计信息 */
    if (osal_unlikely((hmac_device->total_mpdu_num) >= WLAN_TID_MPDU_NUM_LIMIT) ||
        osal_unlikely((tid_queue->mpdu_num) >= WLAN_SINGLE_TID_MPDU_NUM_LIMIT)) {
        oam_warning_log3(0, OAM_SF_TX, "vap_id[%d] tid_tx_queue_enqueue:total_mpdu_num[%d], single_tid_mpdu_num[%d]",
            hmac_vap->vap_id, hmac_device->total_mpdu_num, tid_queue->mpdu_num);
        return OAL_FAIL;
    }
...
#ifdef CONFIG_HMAC_SUPPORT_FLOW_CONTRL
        tid_queue_mpdu_num queue_mpdu_num = {0};
        queue_mpdu_num.total_mpdu_num = hmac_device->total_mpdu_num;
        queue_mpdu_num.single_mpdu_num = tid_queue->mpdu_num;
        hcc_tx_sched_wait(hmac_tid_tx_enqueue_wait_condition, &queue_mpdu_num, TID_TX_ENQUEUE_WAIT_TIME_MS);
