...
*****************************************************************************/
WIFI_HMAC_TCM_TEXT OAL_STATIC OAL_INLINE oal_bool_enum_uint8 hmac_tx_amsdu_is_overflow(
    const hmac_amsdu_stru *amsdu, osal_u32 frame_len)
...
*****************************************************************************/
WIFI_HMAC_TCM_TEXT OAL_STATIC OAL_INLINE osal_u32 hmac_amsdu_alloc_netbuf(hmac_amsdu_stru *amsdu,
    oal_netbuf_stru *buf)
{
    oal_netbuf_stru *dest_buf;
    mac_tx_ctl_stru *cb;
...
/*****************************************************************************
 ...
 功能描述  : amsdu聚合发送处理函数
*****************************************************************************/
WIFI_HMAC_TCM_TEXT OSAL_STATIC osal_u32  hmac_amsdu_tx_process_etc(hmac_vap_stru *hmac_vap,
    ...
    hmac_user_stru *hmac_user, const mac_ether_header_stru *ether_header, osal_u8 tid_no)
{
    ...
    oal_ipv6hdr_stru *pst_ip6 = OAL_PTR_NULL;
    ...
    if (hmac_user_is_amsdu_support(hmac_user, tid_no) == OAL_FALSE &&
        hmac_vap->force_amsdu_active == OAL_FALSE) {
        return HMAC_TX_PASS;
    }
    ...
    /* ICMP6不聚合 */
    if (ether_header->ether_type == oal_host2net_short(ETHER_TYPE_IPV6)) {
        pst_ip6 = (oal_ipv6hdr_stru *)(ether_header + 1);
        if (oal_netbuf_is_icmp6_etc(pst_ip6) == OAL_TRUE) {
            return HMAC_TX_PASS;
        }
    }
