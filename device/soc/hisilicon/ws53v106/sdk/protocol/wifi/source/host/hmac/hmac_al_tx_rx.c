...
OSAL_STATIC osal_void hmac_al_txrx_config_netpool(osal_void)
{
...
osal_s32 hmac_config_set_always_rx(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if (al_rx_flag == HAL_ALWAYS_RX_RESERVED) {
        ...
        hal_device_handle_event(hal_device_base, HAL_DEVICE_EVENT_VAP_UP, size, (osal_u8 *)(hmac_vap->hal_vap));
         ...
         // 开常收，则8个大包阈值
        (osal_void)hmac_ccpriv_alg_cfg_etc(hmac_vap, (const osal_s8 *)"traffic_dscr_min_thres 8");
         // 开常收，则4个小包阈值
        (osal_void)hmac_ccpriv_alg_cfg_etc(hmac_vap, (const osal_s8 *)"traffic_small_dscr_min_thres 4");
#endif
...
#ifdef _PRE_WLAN_SUPPORT_CCPRIV_CMD
         // 关常收，则2个大包阈值
        (osal_void)hmac_ccpriv_alg_cfg_etc(hmac_vap, (const osal_s8 *)"traffic_dscr_min_thres 2");
         // 关常收，则4个小包阈值
        (osal_void)hmac_ccpriv_alg_cfg_etc(hmac_vap, (const osal_s8 *)"traffic_small_dscr_min_thres 4");
#endif
    }
    ...
    if (hal_device_base->current_11ax_working == OSAL_TRUE && al_rx_flag != HAL_ALWAYS_RX_DISABLE) {
        ...
        hal_set_machw_tx_suspend();
        /* 存一个包时，type只能使用BE，因为DMAC从0开始存1个netbuf，取的是第q->num个netbuf，因此q->num（type）只能为0 */
...
osal_s32 hmac_config_set_always_tx(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    hmac_hal_device_sync(hal_device);
    hmac_config_al_tx_low_mcs(hal_device);
...
OAL_STATIC osal_s32 hmac_ccpriv_set_frequency(hmac_vap_stru *hmac_vap, const osal_s8 *param)
{
    unref_param(hmac_vap);
    unref_param(param);
    hal_set_frequency();
...
osal_u32 hmac_al_tx_rx_init(osal_void)
{
    hmac_ccpriv_register((const osal_s8 *)"set_frequency", hmac_ccpriv_set_frequency);
    return OAL_SUCC;
}

osal_void hmac_al_tx_rx_deinit(osal_void)
{
    hmac_ccpriv_unregister((const osal_s8 *)"set_frequency");
    return;
}
