...
osal_u32 hmac_sdp_init(hmac_vap_stru *hmac_vap, osal_bool switch_channel, osal_u16 peroid, osal_u8 *input_ratio)
{
    ...
    if (ret == OAL_SUCC) {
        ...
        hmac_config_set_pm_by_module_etc(hmac_vap, MAC_STA_PM_CTRL_TYPE_SDP, MAC_STA_PM_SWITCH_OFF);
#endif
    }
    return ret;
}
...
osal_u32 hmac_sdp_deinit(hmac_vap_stru *hmac_vap)
{
    ...
    if (ret == OAL_SUCC) {
        ...
        hmac_config_set_pm_by_module_etc(hmac_vap, MAC_STA_PM_CTRL_TYPE_SDP, pm_switch);
#endif
    }
