    ...
    oal_netbuf_stru *buffer, mac_color_area_stru *dth_color_area)
{
    ...
    index += (osal_ulong)sizeof(hmac_color_event_rpt_elements_stru);
...
OSAL_STATIC osal_void hmac_report_collision(hmac_vap_stru *hmac_vap)
{
    ...
    hmac_sta_sr_stru *sr = (hmac_sta_sr_stru *)g_hmac_sta_sr_info[hmac_vap->vap_id];
 
    collision_handler = &sr->collision_handler;
...
OSAL_STATIC osal_bool hmac_sta_clear_spatial_reuse_timer(osal_void *notify_data)
{
    hmac_sta_sr_stru *sr = OSAL_NULL;
    mac_sta_collision_handler *collision_handler = OSAL_NULL;
    hmac_vap_stru *hmac_vap = (hmac_vap_stru *)notify_data;
 
    if (hmac_vap == OSAL_NULL || hmac_vap->vap_id >= WLAN_VAP_SUPPORT_MAX_NUM_LIMIT) {
        oam_warning_log0(0, OAM_SF_11AX, "{hmac_sta_clear_spatial_reuse_timer::hmac_vap null.}");
        return OSAL_FALSE;
    }
    sr = (hmac_sta_sr_stru *)g_hmac_sta_sr_info[hmac_vap->vap_id];
    if (sr == OSAL_NULL) {
    ...
    collision_handler = &sr->collision_handler;
    if (collision_handler->collision_timer.is_registerd == OSAL_TRUE) {
...
osal_u32 hmac_sta_sr_init(osal_void)
{
    ...
    frw_util_notifier_register(WLAN_UTIL_NOTIFIER_EVENT_DOWN_VAP, hmac_sta_clear_spatial_reuse_timer);
    frw_util_notifier_register(WLAN_UTIL_NOTIFIER_EVENT_DEL_USER, hmac_sta_clear_spatial_reuse_timer);
...
osal_void hmac_sta_sr_deinit(osal_void)
{
    ...
    frw_util_notifier_unregister(WLAN_UTIL_NOTIFIER_EVENT_DOWN_VAP, hmac_sta_clear_spatial_reuse_timer);
    frw_util_notifier_unregister(WLAN_UTIL_NOTIFIER_EVENT_DEL_USER, hmac_sta_clear_spatial_reuse_timer);
