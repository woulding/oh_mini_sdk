...
OSAL_STATIC osal_void hmac_tx_eapol_key_proc(const hmac_vap_stru *hmac_vap,
...
OAL_STATIC osal_u8 hmac_tx_coustom_tid_map_update(hmac_vap_stru *hmac_vap,
    osal_u8 tos, osal_u8 *tid)
{
    osal_u8 dscp = tos >> WLAN_DSCP_PRI_SHIFT;
    if (hmac_vap->dscp_tid_map[dscp] == HMAC_DSCP_VALUE_INVALID) {
        *tid = tos >> WLAN_IP_PRI_SHIFT;
        return OAL_CONTINUE;
    }
    ...
    mac_tx_ctl_stru *tx_ctl, mac_ether_header_stru *ether_header, osal_u8 *tid)
{
    ...
    if (hmac_tx_coustom_tid_map_update(hmac_vap, ip_hdr->tos, tid) != OAL_CONTINUE) {
        return;
    }
#ifdef _PRE_WLAN_FEATURE_TX_CLASSIFY_LAN_TO_WLAN
...
/*****************************************************************************
 ...
 返 回 值  : osal_u32
*****************************************************************************/
WIFI_HMAC_TCM_TEXT_SMALLER OAL_STATIC osal_u32 hmac_tx_filter_security(hmac_vap_stru *hmac_vap,
...
WIFI_HMAC_TCM_TEXT OAL_STATIC osal_u32 oal_netbuf_reserve_header_pk(oal_netbuf_stru *netbuf)
{
    ...
    if (header_len < (osal_u32)OAL_HDR_TOTAL_LEN) {
        if (oal_netbuf_expand_head(netbuf, (OAL_HDR_TOTAL_LEN - header_len), 0, 0) != 0) {
...
WIFI_HMAC_TCM_TEXT OAL_STATIC osal_void hmac_bridge_vap_xmit_proc(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf)
{
    ...
    if ((fhook_wkup_dev != OSAL_NULL) &&
        (((hmac_sta_pm_wakeup_dev_cb)fhook_wkup_dev)(OSAL_NULL, 0, HMAC_PM_TX_WKUP) != OAL_SUCC)) {
        oam_warning_log0(0, OAM_SF_FRW, "hmac_bridge_vap_xmit_proc: sta_pm");
        oal_netbuf_free(netbuf);
        return;
    }
#endif
...
OAL_STATIC osal_void hmac_tx_from_lan_pkts_stat(hmac_vap_stru *hmac_vap)
{
#ifdef _PRE_WLAN_FEATURE_NET_DIAGNOSIS
    hmac_net_diagnosis_tx_from_lan_pkts_stat(hmac_vap->vap_id, 1);
#endif
...
WIFI_HMAC_TCM_TEXT osal_s32 hmac_bridge_vap_xmit_etc(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    mac_vap_intrrupt_enable();
    hmac_tx_from_lan_pkts_stat(hmac_vap);
    /* user引用计数加1 */
