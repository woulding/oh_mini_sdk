...
WIFI_HMAC_TCM_TEXT osal_s32 hmac_rx_process_data_msg(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if (hook_ret != OAL_CONTINUE) {
        /* 目前Linux的monitor流程中可能会将报文上报到内核，返回OAL_ERR_CODE_CFG80211_BASE，无需进行报文free */
        if (hook_ret != OAL_ERR_CODE_CFG80211_BASE) {
            oal_netbuf_free(netbuf);
        }
        hook_ret = (hook_ret == OAL_ERR_CODE_CFG80211_BASE) ? OAL_SUCC : hook_ret;
