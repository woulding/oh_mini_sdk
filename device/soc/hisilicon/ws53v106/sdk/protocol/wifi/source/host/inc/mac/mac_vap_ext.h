...
typedef struct {
    ...
    osal_u8 send_probe_req_cnt;     /* 记录linkloss之前发送probe req个数，随linkloss计数一起清零 */
    osal_u8 send_probe_req_max;     /* linkloss之前发送probe req最大个数 */
    osal_u8 resv[2];
...
typedef struct hmac_vap_tag {
    ...
    osal_u8 force_amsdu_active : 1; /* 强制开启amsdu聚合 */
