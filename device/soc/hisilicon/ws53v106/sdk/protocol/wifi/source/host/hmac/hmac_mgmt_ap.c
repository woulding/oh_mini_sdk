...
osal_void hmac_handle_disconnect_rsp_ap_etc(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, osal_u8 is_tx,
    osal_u16 reason_code)
{
    osal_s32 ret;
    frw_msg msg_info = {0};
    hmac_ap_disconnect_event_stru ap_disconnect_param = {0};

    ap_disconnect_param.is_tx = is_tx;
    ap_disconnect_param.reason_code = reason_code;
    (osal_void)memcpy_s(ap_disconnect_param.user_mac_addr, WLAN_MAC_ADDR_LEN, (osal_u8 *)hmac_user->user_mac_addr,
        WLAN_MAC_ADDR_LEN);

    msg_info.data = (osal_u8 *)&ap_disconnect_param;
    msg_info.data_len = (osal_u8)sizeof(hmac_ap_disconnect_event_stru);
    /* 2、3表示mac地址位 */
    ...
    oal_bool_enum_uint8 is_protected)
{
    ...
    /* 抛事件上报内核，已经去关联某个STA */
    hmac_handle_disconnect_rsp_ap_etc(hmac_vap, hmac_user, OAL_FALSE, err_code);
...
#endif
    {
        ...
        if (status_code == MAC_SUCCESSFUL_STATUSCODE) {
            ...
            if ((hmac_user->cap_info.use_wpa3 == OAL_TRUE) && (hmac_vap->cap_flag.wpa3 == OAL_TRUE)) {
                ...
                hmac_handle_disconnect_rsp_ap_etc(hmac_vap, hmac_user, OAL_TRUE, status_code);
            }
#endif
            hmac_user_del_etc(hmac_vap, hmac_user);
        }
    ...
    osal_u32 mac_hdr_len, osal_u8 *payload, osal_u32 payload_len, oal_bool_enum_uint8 is_protected)
{
    ...
    err_code = *((osal_u16 *)(mac_hdr + MAC_80211_FRAME_LEN));
    oam_warning_log1(0, OAM_SF_CONN, "{DISASSOC rx, err_code[%d].}", err_code);
    /* 0、1、2、3表示MAC相应位数 */
...
osal_u32  hmac_mgmt_timeout_ap_etc(osal_void *p_param)
{
    ...
    /* 抛事件上报内核，已经去关联某个STA */
    hmac_handle_disconnect_rsp_ap_etc(hmac_vap, hmac_user, OAL_TRUE, MAC_AUTH_NOT_VALID);
