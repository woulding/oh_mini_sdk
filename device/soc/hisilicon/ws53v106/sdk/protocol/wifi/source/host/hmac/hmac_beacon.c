...
OSAL_STATIC oal_bool_enum_uint8 hmac_sta_edca_is_changed(const hmac_vap_stru *hmac_vap, osal_u8 *frame_body,
    osal_u16 frame_len);
...
/*****************************************************************************
 函 数 名  : hmac_linkloss_send_probe_req_check
 功能描述  : 非背景扫描阶段，LinkLoss计数器超过一半，判断是否满足发送probe_req条件之一
*****************************************************************************/
OSAL_STATIC osal_s32 hmac_linkloss_send_probe_req_check(hmac_vap_stru *hmac_vap)
{
    if ((hmac_vap->linkloss_info.send_probe_req_max == 0) ||
        (hmac_vap->linkloss_info.send_probe_req_cnt < hmac_vap->linkloss_info.send_probe_req_max)) {
        return OAL_SUCC;
    }
...
OSAL_STATIC osal_void hmac_linkloss_send_probe_req(hmac_vap_stru *hmac_vap, const hmac_device_stru *hmac_device)
{
    ...
    if ((get_current_linkloss_cnt(hmac_vap) >= get_current_linkloss_send_probereq_threshold(hmac_vap)) &&
        (send_prob_period == 0) && (hmac_linkloss_send_probe_req_check(hmac_vap) == OAL_SUCC)) {
        ...
        hmac_vap->linkloss_info.send_probe_req_cnt++;
    }
}
...
WIFI_TCM_TEXT osal_void hmac_vap_linkloss_clean(hmac_vap_stru *hmac_vap)
{
    ...
    hmac_vap->linkloss_info.send_probe_req_cnt = 0;
...
/*****************************************************************************
 ...
 功能描述  : 上报linkloss统计信息
*****************************************************************************/
OSAL_STATIC osal_void  hmac_report_linkloss_info(const hmac_vap_stru *hmac_sta)
{
    osal_void *fhook = OSAL_NULL;
    osal_u64 last_active_time;
    ...
    oam_warning_log4(0, OAM_SF_ANY,
        "linkloss_info: link_loss_cnt =%d, linkloss_threshold = %d, last_active_time =%llu, current_time = %llu",
        get_current_linkloss_cnt(hmac_sta), get_current_linkloss_int_threshold(hmac_sta),
        last_active_time, osal_get_time_stamp_ts());
}
...
OSAL_STATIC osal_void hmac_sta_tbtt_prior_linkloss_handler(hmac_vap_stru *hmac_vap)
{
    ...
    if ((get_current_linkloss_cnt(hmac_vap) % WLAN_LINKLOSS_REPORT == 0) &&
        ...
        (hmac_device->curr_scan_state != MAC_SCAN_STATE_RUNNING)) {
        /* 打印link_loss计数 */
        hmac_report_linkloss_info(hmac_vap);
    }
}
...
osal_void hmac_vap_linkloss_channel_clean(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf)
{
    ...
    osal_u8 *payload;
    osal_u16 msg_len;
    static osal_u8 diff_channel_frame_num = 0;
    osal_void *fhook = hmac_get_feature_fhook(HMAC_FHOOK_CSA_STA_FSM_IS_WAITING);
    ...
    if ((hmac_vap->channel.chan_number == frame_channel) && (frame_channel != 0)) {
        hmac_vap_linkloss_clean(hmac_vap);
        diff_channel_frame_num = 0;
    } else if (hmac_vap->vap_state == MAC_VAP_STATE_UP &&
        (fhook == OSAL_NULL || ((hmac_sta_csa_fsm_is_waiting_cb)fhook)(hmac_vap) == OSAL_FALSE)) {
        diff_channel_frame_num++;
    } else {
        diff_channel_frame_num = 0;
    }
    /* 如果收到连续3个beacon的信道号都不对，说明AP切走了，但是我们没有感知，直接触发linkloss流程 */
    if (diff_channel_frame_num >= 3 && hmac_vap->vap_state == MAC_VAP_STATE_UP) {
        wifi_printf("Oh no,LinkLoss!! [%02X:%02X:%02X:%02X:XX:XX] wrong_channel\r\n", hmac_vap->bssid[0],
            /* bssid index 1, bssid index 2, bssid index 3 */
            hmac_vap->bssid[1], hmac_vap->bssid[2], hmac_vap->bssid[3]);
        hmac_vap_linkloss_clean(hmac_vap);
        hmac_send_disassoc_misc_event(hmac_vap, hmac_vap->assoc_vap_id,
