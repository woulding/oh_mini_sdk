    ...
    mac_reason_code_enum_uint16  disasoc_reason)
{
    ...
    switch (hmac_vap->vap_mode) {
        case WLAN_VAP_MODE_BSS_AP:
            /* 抛事件上报内核，已经去关联某个STA */
            hmac_handle_disconnect_rsp_ap_etc(hmac_vap, hmac_user, OAL_TRUE, disasoc_reason);
            break;
...
/*****************************************************************************
  2 全局变量定义
*****************************************************************************/
const oal_fsm_state_info g_sta_bw_switch_fsm_info[] = {
    {
    ...
    hmac_vap_stru *down_vap, hmac_vap_stru *up_vap)
{
    ...
    for (q_num = HAL_TX_QUEUE_BUTT - 1; q_num >= 0; q_num--) {
        ...
        for (netbuf = oal_netbuf_peek(head); netbuf != OSAL_NULL; netbuf = netbuf_next) {
            netbuf_next = oal_netbuf_peek_next(netbuf, head);
...
{
    wlan_phy_protocol_enum_uint8 protocol_mode = rx_ctrl->rx_statistic.nss_rate.legacy_rate.protocol_mode;
    /* AMPDU最后一个mpdu才更新snr信息，否则上报值为0 */
    if (rx_ctrl->rx_status.last_mpdu_flag == OSAL_TRUE && protocol_mode != WLAN_11B_PHY_PROTOCOL_MODE) {
...
#else
        /* 转换后snr单位为0.5db */
        hal_phy_rx_get_snr_info(hal_chip_get_hal_device(), (uint8_t)protocol_mode,
...
OSAL_STATIC osal_s32 hmac_config_del_vap_sync_device(hmac_vap_stru *hmac_vap, frw_msg *msg,
    const hmac_cfg_del_vap_param_stru *del_vap_param)
{
    osal_s32 ret;
    ...
    hmac_device_stru *hmac_device, mac_cfg_start_vap_param_stru *start_param)
{
    ...
    if (hmac_vap->vap_mode == WLAN_VAP_MODE_BSS_AP) {
        ...
        if (is_legacy_sta(hmac_vap)) {
            if (hmac_init_channel_scoring_record() != OAL_SUCC) {
                return OAL_FAIL;
            }
        }
#endif
...
OSAL_STATIC osal_u32 hmac_config_down_vap_pre(hmac_vap_stru *hmac_vap)
{
    ...
    osal_list_for_each_safe(entry, dlist_tmp, &(hmac_vap->mac_user_list_head)) {
        ...
        /* 删除用户事件上报给上层 */
        if (hmac_vap->vap_mode == WLAN_VAP_MODE_BSS_AP) {
            hmac_handle_disconnect_rsp_ap_etc(hmac_vap, hmac_user, OAL_TRUE, MAC_DISAS_LV_SS);
        } else if (hmac_vap->vap_mode == WLAN_VAP_MODE_BSS_STA) {
    ...
    const osal_u8 *param, osal_u8 vap_id)
{
    ...
    if (fhook != OSAL_NULL) {
        ((hmac_csi_exit_cb)fhook)(hmac_vap);
    }
