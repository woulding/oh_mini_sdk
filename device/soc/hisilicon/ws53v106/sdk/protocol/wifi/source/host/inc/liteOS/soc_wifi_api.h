 ...
 * @ingroup soc_wifi_basic
 *
 * function definition of wifi event.CNcommment:根据帧类型设置phy发送速率函数接口定义.CNend
 */
 ...
 * @ingroup soc_wifi_basic
 *
 * function definition of wifi event.CNcommment:设置报文发包时每档发送次数接口定义.CNend
 */
...
* @since
*/
int uapi_wifi_set_wow_switch(td_u8 enable);
...
* @since
*/
int uapi_wifi_set_wow_pattern(td_u8 type, td_u8 index, td_u8 *pattern);
...
/**
* @ingroup  soc_wifi_basic
* @brief    config wifi power save debug info.CNcomment:配置和查询WiFi低功耗调试信息.CNend
*
* @par Description:
*           Open/close wifi power save debug info.CNcomment:开启/关闭WiFi低功耗调试信息.CNend
*
* @attention  NULL
* @param  psm_debug_cmd    [IN] Type  #unsigned char, cmd value 0~5.CNcomment:支持配置的命令字范围0~5.CNend
* @param  psm_debug_switch [IN] Type  #unsigned int,enable(1) or disable(0). CNcomment:开启或关闭调试项.CNend
*
* @retval #EXT_WIFI_OK  Excute successfully
* @retval #Other           Error code
* @par Dependency:
...
* @since
*/
unsigned int uapi_wifi_nd_offload_setting(const td_char *ifname, td_u8 en, td_u8 *ip6);
...
/**
* @ingroup  soc_wifi_basic
* @brief  配置apf过滤规则及下发暗屏状态.CNcomment:配置apf过滤规则及下发暗屏状态.CNend
*
* @par Description:
*           Set apf filter program and suspend mode.CNcomment:配置apf过滤规则及下发暗屏状态.CNend
*
* @attention  NULL
* @param  program          [IN]     Type #const char *, identity string.CNcomment:过滤规则.CNend
*         len              [IN]     Type #unsigned short, identity string length.CNcomment:过滤规则长度.CNend
*         flag             [IN]     Type #unsigned char, suspend mode, should be 0 or 1.CNcomment:暗屏状态.CNend
*
* @retval #EXT_WIFI_OK  Excute successfully
* @retval #Other        Error code
*
...
* @since
*/
int uapi_wifi_apf_set_list(const char *program, uint16_t len, uint8_t flag);

/**
* @ingroup  soc_wifi_basic
* @brief  config arp offload & apf .CNcomment:配置arp offload & apf使能、去使能.CNend
*
* @par Description:
*           Set apf & arp offload .CNcomment:配置arp offload & apf使能、去使能.CNend
*
* @attention  NULL
* @param  ifname         [IN]     Type #const char *, identity string.CNcomment:设备名称.CNend
*         en             [IN]     Type #unsigned short, packet fiter switch.CNcomment:使能过滤.CNend
*         ip             [IN]     Type #unsigned int, ip config.CNcomment:过滤IP.CNend
*
* @retval #EXT_WIFI_OK  Excute successfully
* @retval #Other        Error code
*
...
/**
* @ingroup  soc_wifi_basic
* @brief  Get VAP TSF by netdev type.CNcomment:根据网络设备类型获取TSF.CNend
*
* @par Description:
*            Get tsf by netdev type.CNcomment:根据网络设备类型获取TSF.CNend
*
* @attention  NULL
* @param  iftype [IN]     Type #uint8_t, netdev type.CNcomment:网络设备类型.CNend
*         tsf    [IN]     Type #uint64_t *, tsf value.CNcomment:获取的TSF值.CNend
*
* @retval #EXT_WIFI_OK  Excute successfully
* @retval #Other        Error code
*
...
* @since
*/
td_s32 uapi_wifi_get_vap_tsf(uint8_t iftype, uint64_t *tsf);

/**
* @ingroup  soc_wifi_basic
* @brief  Config wifi sta send probe request max times after beacon miss.
*         CNcomment:配置sta在beacon miss之后发送probe request的最大次数.CNend
* @par Description:
*         Set wifi sta send probe request max times after beacon miss.
*         CNcomment:配置sta在beacon miss之后发送probe request的最大次数.CNend
* @attention  NULL
* @param  ifname         [IN]     Type #td_u8, max times.CNcomment:最大次数.CNend
*
* @retval #EXT_WIFI_OK  Excute successfully
* @retval #Other        Error code
*
...
/**
* @ingroup  soc_wifi_basic
* @brief  获取不同tid队列当前报文数量，字节数量.CNcomment:获取不同tid队列当前报文数量，字节数量.CNend
*
* @par Description:
*           Get Tid Info.CNcomment:获取不同tid队列当前报文数量，字节数量.CNend
*
* @attention  NULL
* @param  iftype                [IN]     Type #uint8_t, unsigned short.CNcomment:网络设备类型.CNend
*         mac_addr              [IN]     Type #uint8_t *, identity string length.CNcomment:对应user的mac地址.CNend
*         tid_stat              [IN]     Type #tid_info_stru *, identity string length.CNcomment:tid统计数组.CNend
*         len                   [IN]     Type #uint8_t, unsigned short, tid统计数组长度.CNcomment:表长.CNend
*
* @retval #EXT_WIFI_OK  Excute successfully
* @retval #Other        Error code
*
...
* @since
*/
int uapi_wifi_get_tid_info(uint8_t iftype, uint8_t *mac_addr, tid_info_stru *tid_stat, uint8_t len);

/**
* @ingroup  soc_wifi_basic
* @brief  配置dscp和tid的对应关系.CNcomment:配置dscp和tid的对应关系.CNend
*
* @par Description:
*           Mapping from dscp to tid.CNcomment:配置dscp和tid的对应关系.CNend
*
* @attention  NULL
* @param  iftype                [IN]Type #uint8_t, unsigned short.CNcomment:网络设备类型.CNend
*         dscp_to_tid_value     [IN]Type #uint8_t *, identity string length.CNcomment:dscp到tid的映射表.CNend
*         len                   [IN]Type #uint8_t, unsigned short, generally, the value is 64.CNcomment:表长.CNend
*
* @retval #EXT_WIFI_OK  Excute successfully
* @retval #Other        Error code
*
...
* @since
*/
int uapi_wifi_set_dscp_tid_map(uint8_t iftype, uint8_t *dscp_to_tid_value, uint8_t len);

typedef struct {
    osal_u32 rx_total_num;
    osal_u32 rx_data_num;
    osal_u32 rx_mgmt_num;
    osal_u32 rx_rts_num;
    osal_u32 tx_total_data_num;
    osal_u32 tx_succ_data_num;
    osal_u32 tx_mgmt_num;
    osal_u32 tx_drop_data_num;
    osal_u32 tx_rts_num;
} trx_info_stru;

/**
* @ingroup  soc_wifi_basic
* @brief  清除trx统计结果.CNcomment:清除trx统计结果.CNend
*
* @par Description:
*           Clean packets tx/rx statistics.CNcomment:清除trx统计结果.CNend
*
* @attention  1、获取统计结果前，需要先调用该接口清除/使能部分统计
* @param  NULL
*
* @retval #EXT_WIFI_OK  Excute successfully
* @retval #Other        Error code
...
* @since
*/
int uapi_wifi_trx_info_clear(void);

/**
* @ingroup  soc_wifi_basic
* @brief  获取报文trx统计结果.CNcomment:获取报文trx统计结果.CNend
*
* @par Description:
*           Get packets tx/rx statistics.CNcomment:获取报文trx统计结果.CNend
*
* @attention  1、获取统计结果前，需要先调用clear接口清除/使能部分统计
* @param  trx_info_stru         [IN]     Type #trx_info_stru.CNcomment:统计结果CNend
*
* @retval #EXT_WIFI_OK  Excute successfully
* @retval #Other        Error code
*
...
* @since
*/
int uapi_wifi_get_trx_info(trx_info_stru *info);

typedef osal_void (*wifi_ap_disconnect_reason_report_cb)(osal_u8 is_tx, osal_u8 reason_code,
    const osal_u8 *user_mac_addr);
