...
osal_void frw_timer_restart_all_timer(osal_void)
{
    ...
    for (core_id = 0; core_id < WLAN_FRW_MAX_NUM_CORES; core_id++) {
        ...
        if (osal_list_empty(&g_timer_list_head[core_id]) == OAL_FALSE) {
            ret = osal_adapt_timer_mod(&g_os_timer[core_id], FRW_TIMER_DEFAULT_TIME);
            if (ret != OSAL_SUCCESS) {
                oam_error_log1(0, OAM_SF_FRW, "{frw_timer_restart_all_timer:: osal_timer_start fail 0x%x!}", ret);
            }
