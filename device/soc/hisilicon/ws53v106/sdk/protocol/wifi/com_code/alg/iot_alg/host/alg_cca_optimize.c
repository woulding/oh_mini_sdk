...
osal_void alg_cca_opt_radar_sw(osal_bool radar_switch)
{
    ...
    if (radar_switch == OSAL_TRUE) {
        ...
        if (cca_opt->ed_high_20th_reg < RADAR_ENABLE_CCA_ED_RSSI) {
            hal_set_ed_high_th(RADAR_ENABLE_CCA_ED_RSSI, RADAR_ENABLE_CCA_ED_RSSI, OSAL_TRUE);
        }
        hal_set_cca_prot_th(RADAR_ENABLE_CCA_ED_RSSI, RADAR_ENABLE_CCA_ED_RSSI);
    } else {
        cca_opt->cca_opt_en = WLAN_CCA_OPT_ENABLE;
        hal_set_ed_high_th(cca_opt->ed_high_20th_reg, cca_opt->ed_high_40th_reg, OSAL_TRUE);
        hal_set_cca_prot_th(cca_opt->sd_cca_20th_dsss, cca_opt->sd_cca_20th_ofdm);
