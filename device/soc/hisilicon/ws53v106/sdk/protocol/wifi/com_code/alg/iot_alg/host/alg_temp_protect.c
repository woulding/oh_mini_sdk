...
static const oal_fsm_state_info  g_alg_temp_protect_fsm_info[] = {
    {
...
osal_u32 alg_temp_protect_fsm_trans_to_state(alg_temp_protect_fsm_stru *temp_protect_fsm, osal_u8 state)
{
    ...
    alg_temp_protect_stru *temp_protect = (alg_temp_protect_stru *)(temp_protect_fsm->oal_fsm.p_oshandler);
    hmac_temp_pri_custom_stru *temp_pri = hmac_get_pri_custom();
    osal_u32 ret;

    wifi_printf_always("temp %d state %hhu th:%d %d %d opt:%hhu %hhu %hhu\r\n",
        temp_protect->ave_temp, state, temp_pri->temp_pro_safe_th, temp_protect->over_th_set,
        temp_pri->temp_pro_pa_off_th, temp_protect->reduce_pwr_enable, temp_protect->schedule_reduce,
        temp_protect->is_pa_off);
    ret = oal_fsm_trans_to_state(oal_fsm, state);
#ifdef _PRE_WLAN_FEATURE_TEMP_PROTECT_ACTION
    hmac_alg_temp_protect_notify(temp_protect->ave_temp, state);
#endif
    return ret;
}
/******************************************************************************
...
#ifdef _PRE_WLAN_FEATURE_TEMP_PROTECT_ACTION
    if (temp_protect->temp_pro_fsm.oal_fsm.cur_state == ALG_TEMP_PROTECT_STATE_OVER) {
        timer_cnt++;
        if (timer_cnt >= 5) { /* 5:统计 */
            hmac_alg_temp_protect_notify(temp_protect->ave_temp, temp_protect->temp_pro_fsm.oal_fsm.cur_state);
            timer_cnt = 0;
        }
    }
#endif
