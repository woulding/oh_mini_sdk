...
/* 解决低功耗下存在短时间突发RX流量场景，dscr_min_thres描述符不足导致AP降速问题 */
OSAL_STATIC osal_void alg_traffic_ctl_customize_dscr_min_thres(alg_param_sync_traffic_ctl_stru *traffic_ctl_sync_data)
{
    osal_u8 nv_val = 0;
    osal_u16 nv_val_len = 0;

    if (uapi_nv_read(NV_ID_TRAFFIC_CTL_RX_RESTORE, sizeof(nv_val), &nv_val_len, &nv_val) == OAL_SUCC) {
        if (nv_val == 1) {
            traffic_ctl_sync_data->rx_ctl[HAL_RX_DSCR_NORMAL_QUEUE].dscr_min_thres =
                ALG_TRAFFIC_CTL_RX_DSCR_RESTORE_THRES;
        }
    }
}
#endif
...
#ifdef CONFIG_MIDDLEWARE_SUPPORT_NV
    alg_traffic_ctl_customize_dscr_min_thres(traffic_ctl_sync_data);
#endif
    return OAL_SUCC;
}
