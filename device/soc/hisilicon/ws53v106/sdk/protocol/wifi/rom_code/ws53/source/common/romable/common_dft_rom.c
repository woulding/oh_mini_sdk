...
{
    if (frame_mode >= FRAME_MODE_BUTT || track_frame_type >= OAM_USER_TRACK_FRAME_TYPE_BUTT ||
...
osal_void dft_report_signal_addr(const osal_u8 *data, osal_u16 len, osal_u32 msg_id)
{
    ...
    if (memcpy_s(buf, dst_len, data, frame_len) != EOK) {
        oam_warning_log0(0, OAM_SF_DFT, "{dft_report_signal_addr:memcpy_s data error.}");
        osal_kfree(buf);
        return;
    }
    dft_report_to_hso(buf, frame_len, msg_id);
    osal_kfree(buf);
}
    ...
    const osal_u8 *frame_addr, osal_u16 frame_len, adjust_hso_param_stru *adjust_hso_param)
{
    osal_u8 *buf = OSAL_NULL;
    osal_u16 dst_len = DFT_MAX_FRAME_LEN;
    osal_u16 len = frame_len;

    if ((hdr_addr == OSAL_NULL) || (frame_addr == OSAL_NULL) || (adjust_hso_param == OSAL_NULL) || (len < hdr_len)) {
        oam_warning_log2(0, OAM_SF_DFT,
            "{dft_report_two_addr:hdr_add or frame_addr or adjust_hso_param = NULL or hdr_len[%d] len[%d] error.}",
            hdr_len, len);
        return;
    }
    ...
    if (adjust_hso_param->is_probe_rsp_or_beacon != OSAL_FALSE) {
        buf[2] = (osal_u8)adjust_hso_param->rssi; /* mac头第2个字节填充rssi */
        buf[3] = adjust_hso_param->channel; /* mac头第3个字节填充channel */
    }
    if (memcpy_s(buf + hdr_len, dst_len - hdr_len, frame_addr, len - hdr_len) != EOK) {
        oam_warning_log0(0, OAM_SF_DFT, "{dft_report_two_addr:memcpy_s hdr_addr error.}");
        osal_kfree(buf);
        return;
    }
    dft_report_to_hso(buf, len, adjust_hso_param->msg_id);
    osal_kfree(buf);
}
...
osal_u8 dft_get_frame_mode(osal_u8 *hdr)
{
    ...
    if ((is_tods == OSAL_FALSE) && (is_fromds == OSAL_FALSE)) {
        /* IBSS */
        address = mac_hdr->address1;
    } else if (is_tods != OSAL_FALSE) {
        /* to AP */
        address = mac_hdr->address3;
    } else if (is_fromds != OSAL_FALSE) {
        /* from AP */
        address = mac_hdr->address1;
    }
