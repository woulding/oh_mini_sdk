...
typedef struct _mac_sta_pm_handler {
    ...
    osal_u16 max_skip_bcn_cnt;        /* 最大允许跳过beacon次数 */

    osal_u16 tbtt_cnt_since_full_bcn; /* 距离上次接收完整beacon的tbtt cnt计数 */
...
typedef enum {
    ...
    PF_TX_AFTER_HCC,    /* TX HCC之后的性能 */
    PF_RX_REPORT,       /* RX流量统计上报 */
    PF_RX_BEFORE_HCC,   /* RX在HCC之前的性能 */
    PF_DELAY_START,     /* 延时统计起始点 */
    PF_DELAY_END,       /* 延时统计结束点 */
    PF_FIX_AMPDU,       /* 固定聚合个数 */
    PF_BYPASS_ALG,      /* bypass算法 */
    PF_RESV11,
    PF_RESV12,
    PF_RESV13,
    PF_RESV14,
