...
#ifndef _PRE_BUILD_ALG_NEW_ARCH
typedef struct {
    /* tx dscr中算法填写的参数 */
...
/* PPDU公共参数 */
typedef struct {
    uint8_t bit_mc_flag : 1;
    uint8_t resv : 7;
    uint8_t resv3[3];
} hal_common_alg_param_stru;

/* PPDU每个User的ALG参数信息 */
typedef struct {
    uint8_t bit_ampdu_enable : 1;   /* ampdu聚合使能 */
    uint8_t bit_tid : 4;
    uint8_t resv1 : 3;
    uint8_t bit_user_id : 6;
    uint8_t resv2 : 2;
    uint8_t resv3[2];
} hal_user_alg_param_stru;

typedef struct {
    uint8_t bit_alg_pktnum;         /* 算法用到的字段，唯一标示该报文 */
    uint8_t en_frame_type;
    uint8_t bit_probe_type : 4;
    uint8_t uc_hal_device_id : 1;
    uint8_t resv1 : 3;
    uint8_t resv2;

    hal_common_alg_param_stru st_common_param;                   /* PPDU公共参数 */
    hal_rank_alg_param_stru ast_rank_param[HAL_TX_RATE_MAX_NUM];
    hal_user_alg_param_stru ast_user_param[HAL_MU_MAX_USER_NUM];

    hal_tx_phy_mode_one_stru   phy_info;
    hal_tx_txop_tpc_stru       tpc;
    uint8_t                    rts_rate[HAL_TX_RATE_MAX_NUM];
    uint32_t                   self_cts_protect : 1;
    uint32_t                   rts_tpc_rank : 2;
    uint32_t                   res_ : 29;
    const void *cb;
} hal_tx_txop_alg_stru;
typedef hal_tx_txop_alg_stru hal_alg_tx_param_stru;
#endif
