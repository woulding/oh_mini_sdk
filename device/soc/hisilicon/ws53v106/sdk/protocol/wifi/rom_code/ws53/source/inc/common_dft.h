...
typedef struct {
    osal_u32  tot_retry_mpdu_num;
    osal_u32  tot_mpdu_num;
} retry_ratio_report_stru;

typedef enum {
    HAL_PM_NOT_SLEEP_INFO_START = 0,
    HAL_PM_NOT_SLEEP_FLAG = HAL_PM_NOT_SLEEP_INFO_START,
    HAL_PM_NOT_SLEEP_FRW,
    HAL_PM_NOT_SLEEP_HCC,
    HAL_PM_NOT_SLEEP_SCAN,
    HAL_PM_NOT_SLEEP_TX_QUE,
    HAL_PM_NOT_SLEEP_TIME,

    HAL_PM_DISALLOW_HOST_HCC,
    HAL_PM_DISALLOW_HOST_FRW,
    HAL_PM_DISALLOW_HOST_TX_QUE,
    HAL_PM_DISALLOW_HOST_BCN_TIMEOUT,

    HAL_PM_NOT_SLEEP_WAIT_TX_QUE,
    HAL_PM_VOTE_SLEEP_FLAG,
    HAL_PM_NOT_SLEEP_BCAST,
    HAL_PM_NOT_SLEEP_UCAST,
    HAL_PM_NOT_SLEEP_WK,
    HAL_PM_NOT_SLEEP_TX,
    HAL_PM_NOT_SLEEP_BCN_TIMEOUT,

    HAL_PM_NOT_SLEEP_INFO_BUTT
} hal_pm_sleep_type;
...
typedef struct {
    osal_u8 negetive_cnt;
    osal_u8 positive_cnt;
    osal_u16 cali_cnt;
    osal_u16 negetive_ratio;
    osal_u16 positive_ratio;
} pm_tsf_cali_res_stru;

typedef struct {
    osal_u32 tbtt_cnt;
    osal_u16 sleep_debug_cnt[HAL_PM_NOT_SLEEP_INFO_BUTT];
    osal_u16 tsf_diff[HAL_PM_TSF_END * 2];
    pm_tsf_cali_res_stru tsf_cali;
} dft_pm_debug_info_stru;

/* 定义获取PM DEBUG INFO形式 */
#define PM_DEBUG_GET_DEBUG_INFO BIT0
#define PM_DEBUG_CLEAR_BCN_DEBUG_INFO BIT1
#define PM_DEBUG_CLEAR_ALL_DEBUG_INFO BIT2
