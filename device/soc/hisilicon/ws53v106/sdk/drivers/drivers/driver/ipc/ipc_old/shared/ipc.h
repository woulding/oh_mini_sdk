...
typedef struct {
    ...
    uint8_t priority;  // !< Priority to use when processing the message internally.
    uint8_t src;  // !< Sending core.
    uint8_t dst;  // !< Destination core.

    // User fields.
    uint16_t action;    // !< Type of IPC message being sent. Used to route message on the destination.
    ipc_payload payload;  // !< Union of user data types. The action field determines which union member to use.

    uint8_t request_ack;  // !< Request that a delivery confirmation be sent to the sender. UNUSED!
