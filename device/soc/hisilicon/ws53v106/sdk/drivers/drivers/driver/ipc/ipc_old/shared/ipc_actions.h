...
typedef enum {
    ...
    IPC_ACTION_TEST = 73,
#endif
    IPC_ACTION_BT_AT_ADAPTER = 74,  // !< Used by App core to send at command to BT core,
                                    //            BT core send at log to App Core.
    IPC_ACTION_BT_AP_ACCE_ECDH = 75,  // !< Used by BT core to call App core to calc ecdh
    IPC_ACTION_AP_BT_GET_ECDH = 76,   // !< Used by App core to return ecdh result to BT core
...
#define IPC_TEST_SUITE_STRING_LENTGH 120
...
#define IPC_GNSS_PAYLOAD_SIZE (IPC_RPC_PAYLOAD_SIZE - 4)

typedef struct {
    uint8_t data[IPC_GNSS_PAYLOAD_SIZE];
    uint32_t length;
} ipc_payload_gnss_msg_info_t;
...
typedef union {
    ...
    ipc_payload_gnss_msg_info_t gnss_msg_info;
} ipc_payload;
