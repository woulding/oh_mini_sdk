...
static td_u32 drv_get_hash_len(const drv_pke_hash_type hash_type)
{
    ...
    if (i >= sizeof(g_pke_hash_properties) / sizeof(pke_hash_properties_type)) {
        return 0;
    }
    ...
    rsa_pkcs1_hash_info *hash_info)
{
    ...
    for (; i < sizeof(g_pke_pkcs1_hash) / sizeof(pkcs1_hash_t); i++) {
    ...
    if (i >= sizeof(g_pke_pkcs1_hash) / sizeof(pkcs1_hash_t)) {
    ...
    const drv_pke_data *sign)
{
    ...
    if (memcmp(in.data, sign->data, klen) == 0) {
    ...
    const drv_pke_data *output)
{
    ...
    if (memcmp(em_data.data, output->data, klen) == 0) {
    ...
    drv_pke_data *output)
{
    ...
    if (memcmp(input->data, em_data.data, klen) == 0) {
        return PKE_COMPAT_ERRNO(ERROR_PKE_RSA_SAME_DATA);
    }
    ...
    drv_pke_data *output)
{
    ...
    if (scheme == DRV_PKE_RSA_SCHEME_PKCS1_V21) {
        hash_len = drv_get_hash_len(hash_type);
        crypto_chk_return(hash_len == 0, PKE_COMPAT_ERRNO(ERROR_INVALID_PARAM), "hash_type is Invalid\n");
    }
