...
static td_bool g_kapi_pke_init = TD_FALSE;
...
td_s32 kapi_cipher_pke_env_init(td_void)
{
    ...
    if (g_kapi_pke_init == TD_TRUE) {
        return TD_SUCCESS;
    }

    ret = drv_cipher_pke_init();
    if (ret != TD_SUCCESS) {
        crypto_log_err("drv_cipher_pke_init failed\n");
        return ret;
    }
    ret = crypto_mutex_init(&g_pke_mutex);
    if (ret != TD_SUCCESS) {
        crypto_log_err("crypto_mutex_init failed\n");
        goto error_pke_deinit;
    }
    g_kapi_pke_init = TD_TRUE;
    return ret;
...
{
    td_s32 ret = TD_SUCCESS;
    if (g_kapi_pke_init == TD_FALSE) {
        return TD_SUCCESS;
    }

    crypto_mutex_destroy(&g_pke_mutex);
    drv_cipher_pke_deinit();
    g_kapi_pke_init = TD_FALSE;
    return ret;
}
