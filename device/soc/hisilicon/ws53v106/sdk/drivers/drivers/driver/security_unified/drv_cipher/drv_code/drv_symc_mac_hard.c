...
td_s32 drv_cipher_mac_finish(td_handle symc_handle, td_u8 *mac, td_u32 *mac_length)
{
    ...
    if (symc_ctx->work_mode == CRYPTO_SYMC_WORK_MODE_CBC_MAC) {
        crypto_chk_goto_with_ret(ret, (symc_ctx->tail_length % CRYPTO_AES_BLOCK_SIZE_IN_BYTES) != 0, error_exit,
        ...
        if (symc_ctx->tail_length != 0) {
            ...
            crypto_chk_goto_with_ret(ret, ret != EOK, error_exit, SYMC_COMPAT_ERRNO(ERROR_MEMCPY_S),
                "memcpy_s failed\n");

            ret = inner_mac_process(chn_num, symc_ctx, crypto_get_phys_addr(symc_ctx->dma_addr), symc_ctx->tail_length);
            crypto_chk_goto(ret != TD_SUCCESS, error_exit, "inner_mac_process failed, ret is 0x%x\n", ret);
        }
    } else {    /* For CMAC */
        crypto_chk_goto_with_ret(ret, symc_ctx->tail_length == 0, error_exit, SYMC_COMPAT_ERRNO(ERROR_INVALID_PARAM),
            "the last block size is zero for CMAC!\n");
        ret = inner_cmac_finish(chn_num, symc_ctx);
        crypto_chk_goto(ret != TD_SUCCESS, error_exit, "inner_cmac_finish failed, ret is 0x%x\n", ret);
    }

    ret = hal_cipher_symc_get_iv(chn_num, mac, *mac_length);
    crypto_chk_goto(ret != TD_SUCCESS, error_exit, "hal_cipher_symc_get_iv failed, ret is 0x%x\n", ret);
