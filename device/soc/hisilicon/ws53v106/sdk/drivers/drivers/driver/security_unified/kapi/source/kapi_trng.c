...
static td_bool g_kapi_trng_init = TD_FALSE;
...
td_s32 kapi_cipher_trng_env_init(td_void)
{
    ...
    g_kapi_trng_init = TD_TRUE;
    return ret;

error_trng_deinit:
    (void)drv_cipher_trng_deinit();
    return ret;
}

td_s32 kapi_cipher_trng_env_deinit(td_void)
{
    if (g_kapi_trng_init == TD_FALSE) {
        return TD_SUCCESS;
    }

    crypto_mutex_destroy(&g_trng_mutex);
    drv_cipher_trng_deinit();
    g_kapi_trng_init = TD_FALSE;
    return TD_SUCCESS;
}
