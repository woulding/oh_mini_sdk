...
static td_s32 drv_hash_get_state(crypto_hash_type hash_type, td_u32 *state, td_u32 state_size)
{
    ...
    if (i >= sizeof(g_hash_state) / sizeof(hash_state_t)) {
...
td_s32 drv_cipher_hash_deinit(td_void)
{
    ...
    for (chn_info_index = 0; chn_info_index < CRYPTO_HASH_HARD_CHN_CNT; chn_info_index++) {
        ctx = &g_hash_channel_ctx_list[chn_info_index];
        if (ctx->open == TD_FALSE) {
            continue;
        }
        (td_void)drv_cipher_hash_destroy(chn_info_index + 1);   /* 1: chn num is chn_index + 1. */
    }
...
static td_s32 drv_hash_lock_chn(td_u32 *chn_num)
{
    ...
    *chn_num = CRYPTO_INVALID_CHN_NUM;
    ...
    for (chn_info_index = 0; chn_info_index < CRYPTO_HASH_HARD_CHN_CNT; chn_info_index++) {
        if (g_hash_channel_ctx_list[chn_info_index].open == TD_TRUE) {
            continue;
        }

        ret = hal_hash_lock(chn_info_index + 1);
        if (ret == TD_SUCCESS) {
            *chn_num = chn_info_index + 1;
            break;
        }
    }
...
td_s32 drv_cipher_hash_start(td_handle *drv_hash_handle, const crypto_hash_attr *hash_attr)
{
    ...
    chn_info_index = chn_num - 1;
    ctx = &g_hash_channel_ctx_list[chn_info_index];
    (td_void)memset_s(ctx, sizeof(hal_hash_channel_context), 0, sizeof(hal_hash_channel_context));

    /* Alloc data_buffer. */
    ctx->hash_ctx.data_buffer = g_drv_dma_buf + CRYPTO_HASH_DRV_BUFFER_SIZE * chn_info_index;
...
#ifdef CRYPTO_HASH_SOFT_ADDR_NOT_ALIGN_SUPPORT
td_s32 drv_cipher_hash_update(td_handle drv_hash_handle,  const crypto_buf_attr *src_buf, const td_u32 len)
{
    ...
    td_u32 chn_info_index = chn_num - 1;
    crypto_drv_func_enter();
    crypto_chk_return(g_drv_hash_init == TD_FALSE, HASH_COMPAT_ERRNO(ERROR_NOT_INIT), "call init first\n");
...
td_s32 drv_cipher_hash_update(td_handle drv_hash_handle,  const crypto_buf_attr *src_buf, const td_u32 len)
{
    ...
    /* Process one block_size(tail + some of src_buf->virt_addr). */
    processing_len = block_size - tail_len;
    ret = memcpy_s(ctx->hash_ctx.tail + tail_len, sizeof(ctx->hash_ctx.tail) - tail_len,
        src_buf->virt_addr, processing_len);
    crypto_chk_return(ret != EOK, HASH_COMPAT_ERRNO(ERROR_MEMCPY_S), "memcpy_s failed\n");

    crypto_cache_flush((uintptr_t)ctx->hash_ctx.tail, block_size);
    ret = hal_cipher_hash_add_in_node(chn_num, crypto_get_phys_addr(ctx->hash_ctx.tail), block_size, node_type);
    crypto_chk_return(ret != TD_SUCCESS, ret, "hal_cipher_hash_add_in_node failed\n");
    ...
    /* Process left src_buf->virt_addr, except not aligned part. */
    if (left >= block_size) {
        processing_len = left - left % block_size;

        crypto_cache_flush((uintptr_t)((td_u8 *)src_buf->virt_addr + processed_len), processing_len);
        ret = hal_cipher_hash_add_in_node(chn_num, crypto_get_phys_addr((td_u8 *)src_buf->virt_addr + processed_len),
            processing_len, node_type);
        crypto_chk_return(ret != TD_SUCCESS, ret, "hal_cipher_hash_add_in_node failed\n");
    ...
    /* copy not aligned part of src_buf->virt_addr */
    if (left != 0) {
        ret = memcpy_s(ctx->hash_ctx.tail, sizeof(ctx->hash_ctx.tail),
        (td_u8 *)((uintptr_t)(src_buf->virt_addr) + processed_len), left);
        crypto_chk_return(ret != EOK, HASH_COMPAT_ERRNO(ERROR_MEMCPY_S), "memcpy_s failed\n");
...
static td_s32 inner_hash_finish_common(td_handle drv_hash_handle, td_u8 *out, td_u32 *out_len, td_bool is_destroy)
{
    ...
    crypto_chk_goto(ret != EOK, exit_hash_unlock, "memcpy_s failed\n");
...
td_s32 inner_hash_drv_handle_chk(td_handle hash_handle)
{
    ...
    crypto_chk_return(chn_num - 1 >= CRYPTO_HASH_HARD_CHN_CNT, HASH_COMPAT_ERRNO(ERROR_INVALID_HANDLE),
