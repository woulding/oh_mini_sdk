...
static crypto_atomic g_init_counter;
...
static td_bool g_kapi_km_init = TD_FALSE;
...
td_s32 kapi_km_env_deinit(td_void)
{
    ...
    g_kapi_km_init = TD_FALSE;
    return TD_SUCCESS;
}

int kapi_km_init(void)
{
    crypto_atomic_inc(&g_init_counter);
...
td_s32 kapi_km_deinit(td_void)
{
    ...
    if (crypto_atomic_dec_return(&g_init_counter) != 0) {
        return CRYPTO_SUCCESS;
    }
