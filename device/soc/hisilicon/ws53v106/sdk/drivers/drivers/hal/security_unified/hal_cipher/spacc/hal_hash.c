...
td_s32 hal_hash_lock(td_u32 chn_num)
{
    ...
    if ((chn_info_index >= CRYPTO_HASH_HARD_CHN_CNT) || (chn_num == 0)) {
        return TD_FAILURE;
    }

    hard_ctx = &g_hash_hard_ctx[chn_info_index];
...
td_s32 hal_cipher_hash_config(td_u32 chn_num, crypto_hash_type hash_type, const td_u32 *state)
{
    ...
    crypto_param_require(chn_num - 1 < CRYPTO_HASH_HARD_CHN_CNT);
    ...
    in_node_type_e in_node_type)
{
    ...
    hard_ctx = &g_hash_hard_ctx[chn_num - 1];
...
td_void hal_hash_debug(td_void)
{
    ...
    for (i = 0; i < CRYPTO_HASH_HARD_CHN_CNT + 1; i++) {
...
td_void hal_hash_debug_chn(td_u32 chn_num)
{
    ...
    if (chn_info_index >= CRYPTO_HASH_HARD_CHN_CNT) {
