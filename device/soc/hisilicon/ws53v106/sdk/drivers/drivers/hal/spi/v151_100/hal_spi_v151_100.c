...
{
    hal_spi_frame_format_t spi_frame_format = (hal_spi_frame_format_t)hal_spi_v151_100_ctrlr0_get_spi_frf(bus);
...
static bool hal_spi_is_busy(spi_bus_t bus, uint32_t timeout)
{
    ...
    while (hal_spi_v151_100_sr_get_tfe(bus) != 1 || hal_spi_v151_100_sr_get_rfne(bus) != 0 ||
           hal_spi_v151_100_sr_get_busy(bus) != 0) {
...
{
    hal_spi_v151_100_ssienr_set_ssi_en(bus, 0);
    hal_spi_v151_100_spi_ctrlr0_set_trans_type(bus, qspi_param->trans_type);
    hal_spi_v151_100_spi_ctrlr0_set_inst_l(bus, qspi_param->inst_len);
    hal_spi_v151_100_spi_ctrlr0_set_addr_l(bus, qspi_param->addr_len);

    uint32_t wait_cyc = qspi_param->wait_cycles;
    if (wait_cyc > QSPI_WAIT_CYCLE_MAX) {
        wait_cyc = QSPI_WAIT_CYCLE_MAX;
    }
    hal_spi_v151_100_spi_ctrlr0_set_wait_cycles(bus, wait_cyc);
    hal_spi_v151_100_ssienr_set_ssi_en(bus, 1);
}
...
static void hal_spi_config_sspi(spi_bus_t bus, const hal_spi_xfer_sspi_param_t *sspi_param)
{
    ...
    hal_spi_v151_100_rsvd_set(bus, sspi_param->wait_cycles);
}
#endif
...
static void hal_spi_init_config_spi_frame_format(spi_bus_t bus, hal_spi_frame_format_t spi_frame_format)
{
    if (spi_frame_format == HAL_SPI_FRAME_FORMAT_STANDARD ||
        ...
        hal_spi_v151_100_ctrlr0_set_spi_frf(bus, (uint32_t)spi_frame_format);
    } else {
        if (spi_frame_format == HAL_SPI_FRAME_FORMAT_OCTAL ||
            spi_frame_format == HAL_SPI_FRAME_FORMAT_DOUBLE_OCTAL ||
            spi_frame_format == HAL_SPI_FRAME_FORMAT_SIXT) {
            hal_spi_v151_100_ctrlr0_set_spi_frf(bus, (uint32_t)HAL_SPI_FRAME_FORMAT_OCTAL);
        }
    }
}
...
static void hal_spi_init_config_freq(spi_bus_t bus, uint32_t bus_clk, uint32_t clk_in_mhz)
{
    ...
    hal_spi_v151_100_baudr_set_sckdv(bus, clk_div);
}

static void hal_spi_init_config_master(spi_bus_t bus, const hal_spi_attr_t *attr,
                                       const hal_spi_extra_attr_t *extra_attr)
{
    hal_spi_v151_100_ser_set_ser(bus, (attr->slave_num == 0) ? 0 : bit(attr->slave_num - 1));
    ...
    if (attr->ndf > 0) {
        hal_spi_v151_100_ctrlr1_set_ndf(bus, attr->ndf - 1);
    } else {
        hal_spi_v151_100_ctrlr1_set_ndf(bus, 0);
    }
}
...
static bool hal_spi_init_config(spi_bus_t bus, const hal_spi_attr_t *attr, const hal_spi_extra_attr_t *extra_attr)
{
    ...
    if (bus <= SPI_BUS_MAX && attr->frame_format == SPI_CFG_FRAME_FORMAT_TEXAS_SSP) {
        return false;
    }
#endif
    ...
    hal_spi_v151_100_ssienr_set_ssi_en(bus, 0);
    hal_spi_v151_100_ctrlr0_set(bus, 0);
    hal_spi_v151_100_spi_ctrlr0_set(bus, 0);

    hal_spi_init_config_spi_frame_format(bus, (hal_spi_frame_format_t)attr->spi_frame_format);
    hal_spi_v151_100_ctrlr0_set_frf(bus, (hal_spi_cfg_frame_format_t)attr->frame_format);

    /* Config the clock phase and polarity. */
    hal_spi_v151_100_ctrlr0_set_scph(bus, attr->clk_phase);
    hal_spi_v151_100_ctrlr0_set_scpol(bus, attr->clk_polarity);

    /* Config the frame size. */
    hal_spi_v151_100_ctrlr0_set_dfs_32(bus, attr->frame_size);
    hal_spi_v151_100_ctrlr0_set_cfs(bus, attr->frame_size);

    /* Config the transfer mode and FIFO threshold. */
    hal_spi_v151_100_ctrlr0_set_tmod(bus, attr->tmod);
    if (attr->frame_format == SPI_CFG_FRAME_FORMAT_NS_MICROWIRE && attr->tmod == HAL_SPI_TRANS_MODE_TX) {
        hal_spi_v151_100_mwcr_set_mdd(bus, 1);
    }

    hal_spi_v151_100_txftlr_set_tft(bus, CONFIG_SPI_TXFTLR);
    hal_spi_v151_100_rxftlr_set_rft(bus, CONFIG_SPI_RXFTLR);
    ...
    hal_spi_v151_100_ctrlr0_set_sste(bus, attr->sste);

    /* Enable the SPI and all of the interrupts. */
    hal_spi_v151_100_ssienr_set_ssi_en(bus, 1);
    hal_spi_v151_100_int_set(bus, SPI_INMAR_REG, 0);

    (void)memcpy_s(&g_hal_spi_attrs[bus], sizeof(hal_spi_attr_t), attr, sizeof(hal_spi_attr_t));
    if (extra_attr != NULL) {
        (void)memcpy_s(&g_hal_spi_extra_attrs[bus], sizeof(hal_spi_extra_attr_t), extra_attr,
            sizeof(hal_spi_extra_attr_t));
    }

    return true;
}

#pragma weak hal_spi_init = hal_spi_v151_100_init
errcode_t hal_spi_v151_100_init(spi_bus_t bus, const hal_spi_attr_t *attr,
...
#pragma weak hal_spi_deinit = hal_spi_v151_100_deinit
errcode_t hal_spi_v151_100_deinit(spi_bus_t bus)
{
                                               ...
                                               hal_spi_frame_format_t spi_frame_format)
{
    if (frame_bytes == SPI_FRAME_BYTES_1) {
        hal_spi_v151_100_dr_set_dr(bus, (uint8_t)(*data));
    } else {    /* SPI_FRAME_BYTES_4 */
        if (spi_frame_format == HAL_SPI_FRAME_FORMAT_STANDARD) {
            hal_spi_v151_100_dr_set_dr(bus, hal_spi_trans_bytes_to_word(data));
        } else {    /* SPI_FRAME_FORMAT_QUAD */
            hal_spi_v151_100_dr_set_dr(bus, *(uint32_t *)data);
        }
    }
}
...
static errcode_t hal_spi_clear_rx_fifo(spi_bus_t bus, uint32_t trans_frames, uint32_t timeout)
{
    ...
    if (hal_spi_v151_100_ctrlr0_get_tmod(bus) == HAL_SPI_TRANS_MODE_TXRX) {
        while (hal_spi_v151_100_sr_get_tfe(bus) != 1 || hal_spi_v151_100_sr_get_busy(bus) != 0) {
        ...
        trans_time = 0;
        while (trans_frames) {
            if (hal_spi_v151_100_rxflr_get_rxtfl(bus) != 0) {
                hal_spi_v151_100_dr_get_dr(bus);
                trans_frames--;
            }
            if (hal_spi_check_timeout_by_count(&trans_time, timeout)) {
                return ERRCODE_SPI_TIMEOUT;
...
#endif
#pragma weak hal_spi_write = hal_spi_v151_100_write
errcode_t hal_spi_v151_100_write(spi_bus_t bus, hal_spi_xfer_data_t *data, uint32_t timeout)
{
    uint8_t *tx_buff = data->tx_buff;
    uint32_t frame_bytes = hal_spi_frame_size_trans_to_frame_bytes(hal_spi_v151_100_ctrlr0_get_dfs_32(bus));
...
#if defined(CONFIG_SPI_SUPPORT_SINGLE_SPI)
    if (bus <= SPI_BUS_MAX && g_hal_spi_attrs[bus].frame_format == SPI_CFG_FRAME_FORMAT_NS_MICROWIRE) {
        hal_spi_v151_100_bus0_ssienr_set_ssi_en(bus, 0);
        if (data->rx_bytes > 0 && data->rx_buff != NULL) {
            hal_spi_v151_100_bus0_mcr_set_mtrc(bus, 0);
            hal_spi_v151_100_bus0_ctrb_set_nrdf(bus, (uint32_t)(data->rx_bytes / frame_bytes) - 1);
        } else {
            hal_spi_v151_100_bus0_mcr_set_mtrc(bus, 1);
        }
        hal_spi_v151_100_bus0_ssienr_set_ssi_en(bus, 1);
    }
#endif
    ...
    if (unlikely(spi_porting_get_device_mode(bus) == SPI_MODE_MASTER &&
        ...
        if (hal_spi_v151_100_spi_ctrlr0_get_inst_l(bus) != 0) {
            hal_spi_v151_100_dr_set_dr(bus, data->cmd);
        }
        if (hal_spi_v151_100_spi_ctrlr0_get_addr_l(bus) != 0) {
            hal_spi_v151_100_dr_set_dr(bus, data->addr);
        }
    }
    ...
    while (trans_frames > 0) {
        if (hal_spi_v151_100_sr_get_tfnf(bus) != 0) {
...
{
    if (frame_bytes == SPI_FRAME_BYTES_1) {
        *data = (uint8_t)hal_spi_v151_100_dr_get_dr(bus);
    } else {    /* SPI_FRAME_BYTES_4 */
        if (spi_frame_format == HAL_SPI_FRAME_FORMAT_STANDARD) {
            uint32_t data_read = hal_spi_v151_100_dr_get_dr(bus);
            ...
            *(uint32_t *)data = hal_spi_v151_100_dr_get_dr(bus);
        }
    }
}
...
static bool hal_spi_read_rx_mode_prepare(spi_bus_t bus)
{
    ...
    if (spi_porting_get_device_mode(bus) == SPI_MODE_MASTER &&
        hal_spi_v151_100_ctrlr0_get_tmod(bus) == HAL_SPI_TRANS_MODE_RX) {
        hal_spi_v151_100_dr_set_dr(bus, 0); /* 解决主机读场景时，先任意写一个数据拉低CS发出时钟信号。 */
        while (hal_spi_v151_100_rxflr_get_rxtfl(bus) == 0) {
...
#pragma weak hal_spi_read = hal_spi_v151_100_read
errcode_t hal_spi_v151_100_read(spi_bus_t bus, hal_spi_xfer_data_t *data, uint32_t timeout)
{
    ...
    if (hal_spi_v151_100_ctrlr0_get_tmod(bus) == HAL_SPI_TRANS_MODE_TXRX) {
        for (uint32_t i = 0; i < trans_frames; i++) {
            hal_spi_v151_100_dr_set_dr(bus, 0);
        }
    }
#endif

    while (trans_frames > 0) {
        if (hal_spi_v151_100_sr_get_rff(bus) == 1) {
        ...
        uint32_t rx_level = hal_spi_v151_100_rxflr_get_rxtfl(bus);
        while (rx_level > 0) {
            if (timeout == 0) {
                trans_time = 0;
            }
...
static errcode_t hal_spi_ctrl_set_attr(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    if (attr->frame_format == SPI_CFG_FRAME_FORMAT_NS_MICROWIRE) {
        if (attr->tmod == HAL_SPI_TRANS_MODE_TX) {
            ...
            hal_spi_v151_100_mwcr_set_mdd(bus, 0);
        }
    }
    hal_spi_v151_100_ctrlr0_set_tmod(bus, attr->tmod);
...
static errcode_t hal_spi_ctrl_select_slave(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    hal_spi_v151_100_ser_set_ser(bus, bit(slave));
...
static errcode_t hal_spi_ctrl_set_dma_cfg(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    hal_spi_v151_100_dmacr_set_tdmae(bus, (uint32_t)data->is_enable);
    hal_spi_v151_100_dmacr_set_rdmae(bus, (uint32_t)data->is_enable);
    hal_spi_v151_100_dmatdlr_set_dmatdl(bus, data->dma_tx_level);
    hal_spi_v151_100_dmardlr_set_dmardl(bus, data->dma_rx_level);
...
{
    return hal_spi_v151_100_ctrl_get_dma_data_addr(bus, id, param);
}
...
static errcode_t hal_spi_ctrl_en_rxfi_int(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    hal_spi_v151_100_imr_set_rxfi(bus, en);
    return ERRCODE_SUCC;
}
...
static errcode_t hal_spi_ctrl_check_rx_fifo_empty(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    if (hal_spi_v151_100_sr_get_rfne(bus) == 0) {
        *rx_fifo_empty = true;
    } else {
        *rx_fifo_empty = false;
    }
...
static errcode_t hal_spi_ctrl_en_txei_int(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    uint32_t en = (uint32_t)param;
    hal_spi_v151_100_imr_set_txei(bus, en);
    return ERRCODE_SUCC;
}
...
static errcode_t hal_spi_ctrl_resume(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    unused(id);
    unused(param);
    return hal_spi_v151_100_init(bus, &g_hal_spi_attrs[bus], &g_hal_spi_extra_attrs[bus], g_hal_spi_callback);
}
#endif
...
static errcode_t hal_spi_ctrl_set_tmod(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    if (hal_spi_v151_100_sr_get_busy(bus) == 1) {
    ...
    if (attr->tmod >= HAL_SPI_TRANS_MODE_RX) {
        ...
        hal_spi_v151_100_ctrlr1_set_ndf(bus, ndf);
...
#pragma weak hal_spi_ctrl = hal_spi_v151_100_ctrl
errcode_t hal_spi_v151_100_ctrl(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    return g_hal_spi_ctrl_func_array[id](bus, id, param);
}

#if defined(CONFIG_SPI_SUPPORT_INTERRUPT) && (CONFIG_SPI_SUPPORT_INTERRUPT == 1)
void hal_spi_v151_100_irq_handler(spi_bus_t bus)
{
    if (!g_hal_spi_callback) {
        return;
    }

    if (hal_spi_v151_100_isr_get_rxfi(bus, SPI_INSR_REG) == 1) {
        g_hal_spi_callback(bus, SPI_EVT_RX_FULL_ISR, 0);
    }
    if (hal_spi_v151_100_isr_get_rxui(bus, SPI_INSR_REG) == 1) {
        g_hal_spi_callback(bus, SPI_EVT_RX_UNDERFLOW_ISR, 0);
    }
    if (hal_spi_v151_100_isr_get_rxoi(bus, SPI_INSR_REG) == 1) {
        g_hal_spi_callback(bus, SPI_EVT_RX_OVERFLOW_ISR, 0);
    }
    if (hal_spi_v151_100_isr_get_txei(bus, SPI_INSR_REG) == 1) {
        g_hal_spi_callback(bus, SPI_EVT_TX_EMPTY_ISR, 0);
    }
    if (hal_spi_v151_100_isr_get_txoi(bus, SPI_INSR_REG) == 1) {
        g_hal_spi_callback(bus, SPI_EVT_TX_OVERFLOW_ISR, 0);
    }
    if (hal_spi_v151_100_isr_get_msti(bus, SPI_INSR_REG) == 1) {
        g_hal_spi_callback(bus, SPI_EVT_MULTI_MASTER_ISR, 0);
    }
    hal_spi_v151_100_icr_set_any(bus);
}
#endif  /* CONFIG_SPI_SUPPORT_SLAVE */
