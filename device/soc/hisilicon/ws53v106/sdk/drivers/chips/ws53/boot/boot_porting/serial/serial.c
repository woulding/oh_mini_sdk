...
{
    if (check_uart_mute() == true) {
        return;
    }
    uapi_uart_write(g_hiburn_uart, (uint8_t *)&c, sizeof(int8_t), UART_GETC_DELAY_TIME);
}
...
void serial_puts(const char *s)
{
    ...
    uapi_uart_write(g_hiburn_uart, (uint8_t *)s, strlen(s), UART_GETS_DELAY_TIME);
}
...
void serial_put_buf(const char *buffer, int32_t length)
{
    ...
    uapi_uart_write(g_hiburn_uart, (uint8_t *)buffer, length, UART_GETS_DELAY_TIME);
}

uint8_t serial_getc(void)
{
    uint8_t getc = 0;
    uapi_uart_read(g_hiburn_uart, (uint8_t *)&getc, sizeof(getc), UART_GETC_DELAY_TIME);
    return getc;
}
...
uint32_t serial_gets(uint8_t *str, uint32_t str_len)
{
    ...
    if (str_len != (uint32_t)uapi_uart_read(g_hiburn_uart, str, str_len, UART_GETS_DELAY_TIME)) {
        return ERRCODE_FAIL;
    }
    return ERRCODE_SUCC;
}
...
void hiburn_uart_init(uart_param_stru uart_param, uart_bus_t uart_bus)
{
    ...
    uart_line_config.data_bits = uart_param.databit - NUM_5;
    uart_line_config.parity = uart_param.parity;
    if (uart_line_config.parity == NUM_1) {
        uart_line_config.parity = UART_PARITY_ODD;
    } else if (uart_line_config.parity == NUM_2) {
        uart_line_config.parity = UART_PARITY_EVEN;
    } else {
        uart_line_config.parity = UART_PARITY_NONE;
    }
    uart_line_config.stop_bits = uart_param.stopbit - NUM_1;

    uart_buffer_config.rx_buffer_size = HIBURN_UART_RX_BUFFER_SIZE;
    uart_buffer_config.rx_buffer = g_hiburn_rx_buffer;

    (void)uapi_uart_init(g_hiburn_uart, &uart_pins, &uart_line_config, NULL, &uart_buffer_config);
    uart_port_unregister_irq(g_hiburn_uart);
