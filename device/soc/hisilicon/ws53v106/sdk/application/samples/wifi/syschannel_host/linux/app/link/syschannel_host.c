...
typedef struct {
    ...
    syschannel_rx_func rx_func;
} netlink_monitor_s;
/*****************************************************************************
  3 枚举、结构体定义
*****************************************************************************/
static osal_bool            g_channel_terminate = OSAL_FALSE;
static netlink_monitor_s *g_channel_monitor = OSAL_NULL;
/*****************************************************************************
  4 函数实现
*****************************************************************************/
static osal_void *syschannel_host_thread(osal_void *args)
{
    osal_s32 rev_len;
    osal_s32 data_len;
    osal_char msg[SYSTEM_CMD_SIZE];
    ...
    while (!g_channel_terminate) {
        (osal_void)memset_s(&msg[0], sizeof(msg), 0, sizeof(msg));
        ...
        sample_log_print("syschannel_host_thread:%x,%d,%d,%d\n", nlh->nlmsg_type, data_len, rev_len, NLMSG_HDRLEN);
        if (g_channel_monitor->rx_func != OSAL_NULL) {
            g_channel_monitor->rx_func((osal_u8 *)NLMSG_DATA(nlh), data_len);
        }
    }
...
osal_s32 syschannel_register_rx_cb(syschannel_rx_func rx_func)
{
    if ((g_channel_monitor == OSAL_NULL) || (rx_func == OSAL_NULL)) {
        sample_log_print("syschannel_register_rx_cb is fail\n");
        return OSAL_NOK;
    }
...
osal_s32 syschannel_send_to_dev(unsigned char *buf, int len)
{
    int ret;
    struct nlmsghdr *nlh = OSAL_NULL;
    struct sockaddr_nl daddr;

    if ((buf == OSAL_NULL) || (len <= 0) || (len > MAX_SEND_DATA_SIZE)) {
        sample_log_print("sendto sata len:%d\n", len);
        return OSAL_NOK;
    }

    (osal_void)memset_s(&daddr, sizeof(daddr), 0, sizeof(daddr));
    ...
    (osal_void)memset_s(nlh, sizeof(*nlh), 0x00, sizeof(*nlh));
    nlh->nlmsg_len = NLMSG_SPACE(len);
    nlh->nlmsg_pid = NETLINK_SOCKET_PORT_ID;
    (osal_void)memcpy_s(NLMSG_DATA(nlh), NLMSG_SPACE(len), buf, len);
    ...
    sample_log_print("syschannel_send_to_dev finish\n");

    free(nlh);
    return 0;
}

osal_s32 syschannel_init(osal_void)
{
    osal_s32 ret;
    struct sockaddr_nl saddr = {0};

    if (g_channel_monitor != OSAL_NULL) {
    ...
    g_channel_terminate = OSAL_FALSE;
    (osal_void)memset_s(g_channel_monitor, sizeof(netlink_monitor_s), 0, sizeof(netlink_monitor_s));
    ...
    (osal_void)memset_s(&saddr, sizeof(saddr), 0x00, sizeof(saddr));
    ...
    ret = pthread_create(&g_channel_monitor->channel_thread, OSAL_NULL, syschannel_host_thread, OSAL_NULL);
    if (ret != OSAL_OK) {
        goto deinit;
    }
    sample_log_print("syschannel_init finish\n");
...
deinit:
    ...
    if (g_channel_monitor != OSAL_NULL) {
        ...
        g_channel_monitor = OSAL_NULL;
    }
    return OSAL_NOK;
}

osal_s32 syschannel_deinit(osal_void)
{
    if (g_channel_monitor == OSAL_NULL) {
        sample_log_print("syschannel_deinit is fail\n");
        return OSAL_NOK;
    }

    g_channel_terminate = OSAL_TRUE;

    if (g_channel_monitor->channel_thread) {
        pthread_cancel(g_channel_monitor->channel_thread);
        pthread_join(g_channel_monitor->channel_thread, OSAL_NULL);
    }
