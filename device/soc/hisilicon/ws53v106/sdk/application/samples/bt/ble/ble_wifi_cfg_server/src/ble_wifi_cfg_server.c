...
{
    sample_at_log_print("connect state change conn_id: %d, status: %d, pair_status:%d, disc_reason:0x%x\n",
        conn_id, conn_state, pair_state, disc_reason);
    sample_at_log_print("addr:\n");
    for (uint8_t i = 0; i < BD_ADDR_LEN; i++) {
        sample_at_log_print("0x%2x ", addr->addr[i]);
    }
    sample_at_log_print("\n");
...
void ble_wifi_cfg_server_enable_cbk(uint8_t status)
{
...
static errcode_t ble_wifi_cfg_server_register_callbacks(void)
{
    ...
    bts_dev_manager_callbacks_t bts_dev_cbk = { 0 };

    bts_dev_cbk.ble_enable_cb = ble_wifi_cfg_server_enable_cbk;
    ret = bts_dev_manager_register_callbacks(&bts_dev_cbk);
    if (ret != ERRCODE_BT_SUCCESS) {
        sample_at_log_print("[wifi_cfg_server] bts_dev_cbk init failed\r\n");
        return ERRCODE_BT_FAIL;
    }
...
errcode_t wifi_cfg_server_service(void)
{
    ...
    if (ret != ERRCODE_BT_SUCCESS) {
        sample_at_log_print("[wifi_cfg_server_service] reg service failed, ret=0x%x\r\n", ret);
        return ret;
    }
