...
static void sle_chba_user_pair_complete_cb(uint16_t conn_id, const sle_addr_t *addr, errcode_t status)
{
    ...
    if (status != ERRCODE_SUCC) {
        sle_remove_paired_remote_device(addr); // 配对失败删除配对信息
        return;
    }
    sle_chba_user_link_t *link = sle_chba_user_get_linkinfo_by_connid(conn_id);
    if (link == NULL) {
...
#if defined(CONFIG_AT_GLE) && (CONFIG_AT_GLE == 1)
extern bool g_flag_sle_at_register;
static void sle_at_register_weakref(void) __attribute__ ((weakref("sle_at_register")));
#endif
...
#if defined(CONFIG_AT_GLE) && (CONFIG_AT_GLE == 1)
    if (!g_flag_sle_at_register && sle_at_register_weakref != NULL) {
        // sample调试可使用at指令修改连接参数，若不需要将此代码注释可关闭AT
        sle_at_register_weakref();
        g_flag_sle_at_register = true;
    }
#endif
#if CHBA_LWIP_SWITCH
