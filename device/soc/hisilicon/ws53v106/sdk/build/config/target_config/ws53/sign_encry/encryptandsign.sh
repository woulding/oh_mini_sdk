...
# 配置文件
TMP_DIR=$(echo "${OUT_PATH}/tmp" | sed 's,//,/,g')
...
case ${IMG_TYPE} in
    *"ota"*)
        ...
        if [[ $OTA_IMAGES == *"app"* ]]; then
            sign_image liteos_app_bin_ecc_ota.cfg ws53_liteos_app.bin
        fi
    ...
    *"all"*)
        cp ${INPUT_FILE} ${OUT_PATH}/all.zip
        tar -xf ${OUT_PATH}/all.zip -C ${TMP_DIR}
        rm ${OUT_PATH}/all.zip
        # loaderboot
        ...
        # app
        sign_image liteos_app_bin_ecc.cfg ws53_liteos_app.bin
        # mfg
        if [ -f "${TMP_DIR}/ws53_liteos_mfg.bin" ];then
            sign_image liteos_mfg_bin_ecc.cfg ws53_liteos_mfg.bin
        fi
        # efuse
        ...
        # 整包fwpkg镜像加密 & 模块烧写文件
        make_burn_image ws53_liteos_app_all.fwpkg ws53_liteos_app_all_encry.fwpkg
        ;;
    *)
