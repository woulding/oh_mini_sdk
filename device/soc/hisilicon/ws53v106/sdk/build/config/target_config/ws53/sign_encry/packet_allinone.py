...
def make_all_in_one_packet(argv):
    ...
    params_bx = params_bin + f"|0x400000|{hex(get_file_size(params_bin))}|1"
    # ssb
    ssb_bin = os.path.join(fwpkg_dir, "ssb_sign.bin")
    ssb_bx = ssb_bin + f"|0x402000|{hex(get_file_size(ssb_bin))}|1"
    # flash boot
    flashboot_bin = os.path.join(fwpkg_dir, "flashboot_sign.bin")
    flashboot_bx = flashboot_bin + f"|0x420000|{hex(get_file_size(flashboot_bin))}|1"
    # flash boot backup
    flashboot_backup_bin = os.path.join(fwpkg_dir, "flashboot_backup_sign.bin")
    flashboot_backup_bx = flashboot_backup_bin + f"|0x410000|{hex(get_file_size(flashboot_backup_bin))}|1"
    # nv
    nv_bin = os.path.join(fwpkg_dir, "ws53_all_nv.bin")
    nv_bx = nv_bin + f"|0x7FC000|0x4000|1"
    # nv backup
    nv_backup_bin = os.path.join(fwpkg_dir, "ws53_all_nv_factory.bin")
    nv_backup_bx = nv_backup_bin + f"|0x40C000|0x4000|1"
    # app
    app_bin = os.path.join(fwpkg_dir, "ws53_liteos_app_sign.bin")
    app_bx = app_bin + f"|0x430000|{hex(get_file_size(app_bin))}|1"
    # mfg
    mfg_bin = os.path.join(fwpkg_dir, "ws53_liteos_mfg_sign.bin")
    ...
    if os.path.exists(mfg_bin):
        mfg_bx = mfg_bin + f"|0x670000|{hex(0x183000)}|1"
        packet_post_agvs.append(mfg_bx)
    packet_post_agvs.append(efuse_bx)
    fpga_fwpkg = os.path.join(fwpkg_dir, f"ws53_liteos_app_all.fwpkg")
    packet_bin(fpga_fwpkg, packet_post_agvs)

    packet_post_agvs = list()
    packet_post_agvs.append(loadboot_bx)
    packet_post_agvs.append(app_bx)
    fpga_loadapp_only_fwpkg = os.path.join(fwpkg_dir, f"ws53_liteos_app_all_only.fwpkg")
