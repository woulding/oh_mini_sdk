/*
 * Copyright (c) 2020 HiSilicon (Shanghai) Technologies Co., Ltd. 2019-2020. All rights reserved.
...
#define TASK_COMMON_APP_DELAY_MS 100
#endif
...
at_ret_t at_syschannel_get_stat(void)
{
#ifdef _PRE_SYSCHANNEL_FEATURE
    if (uapi_syschannel_get_trx_stat() == OSAL_NULL) {
        return AT_RET_SYNTAX_ERROR;
    }
#endif
    return AT_RET_OK;
...
at_ret_t at_syschannel_clean_stat(void)
{
#ifdef _PRE_SYSCHANNEL_FEATURE
    return uapi_syschannel_clean_trx_stat();
#else
    return AT_RET_OK;
#endif
}
        ...
        if ((ret == OSAL_OK) && (filter != TD_NULL) && (args->para3 == WIFI_FILTER_TYPE_IPV4)) {
            ...
            for (int i = 0; i < num; i++) {
                uapi_at_print("local_port=%d; remote_port=%d; packet_type=%d; match_mask=%d; config_type=%d\r\n", ipv4_filter[i].local_port,
                     ipv4_filter[i].remote_port,ipv4_filter[i].packet_type, ipv4_filter[i].match_mask, ipv4_filter[i].config_type);
            }
            ...
            for (int i = 0; i < num; i++) {
                uapi_at_print("local_port=%d; remote_port=%d; packet_type=%d; match_mask=%d; config_type=%d\r\n", ipv6_filter[i].local_port,
                    ipv6_filter[i].remote_port, ipv6_filter[i].packet_type, ipv6_filter[i].match_mask, ipv6_filter[i].config_type);
            }
        }
    }
