...
#define ALG_SET_FIX_PARAMS_TYPE_DATA 2
#define MAX_DSCP_VALUE_NUM 64
...
at_ret_t cmd_softap_show_sta(void)
{
    ...
    (void)memset_s(result, sizeof(wifi_sta_info_stru), 0, sizeof(wifi_sta_info_stru));
...
{
    if (uapi_wifi_set_phy_txrate_by_frame_type(args->rate_mode, args->cfg_frame_type, args->rate_val) != EXT_WIFI_OK) {
        return EXT_WIFI_FAIL;
    }
...
{
    if (uapi_wifi_set_tx_count(args->cfg_frame_type, args->tx_count0, args->tx_count1, args->tx_count2,
        args->tx_count3) != EXT_WIFI_OK) {
        return EXT_WIFI_FAIL;
    }
...
at_ret_t cmd_set_sta_pm(const ps_args_t *args)
{
    ...
    ret = uapi_wifi_set_pm_switch((td_u8)args->ps_switch, (td_u32)args->sleep_time);
    if (ret != ERRCODE_SUCC) {
        return AT_RET_SYNTAX_ERROR;
    }
...
at_ret_t cmd_set_sta_psm_debug(const ps_debug_args_t *args)
{
    ...
    ret = uapi_wifi_set_psm_debug_switch((td_u8)args->psm_debug_cmd, (td_u8)args->psm_debug_switch);
    if (ret != ERRCODE_SUCC) {
        return AT_RET_SYNTAX_ERROR;
    }
...
at_ret_t cmd_get_wifi_tsf(const wifi_tsf_args_t *args)
{
    ...
    ret = uapi_wifi_get_vap_tsf((td_u8)args->net_type, &tsf);
    if (ret != ERRCODE_SUCC) {
        return AT_RET_SYNTAX_ERROR;
    }

    uapi_at_printf("TSF %08x %08x\r\n", (td_u32)(tsf >> 32), (td_u32)tsf); // 32 : 右移32位
    return AT_RET_OK;
}
...
{
    td_s32 ret;
    td_u8 dscp_to_tid_value[MAX_DSCP_VALUE_NUM];
    td_char *pos;
    td_char *tmp;
    td_u32 len = strlen((td_char *)args->dscp2tid_map);
    td_s32 idx;

    tmp = at_parse_string((td_char *)args->dscp2tid_map, &len);
    if (tmp == NULL) {
        return AT_RET_SYNTAX_ERROR;
    }

    if (len != MAX_DSCP_VALUE_NUM) {
        osal_kfree(tmp);
        return AT_RET_SYNTAX_ERROR;
    }

    pos = tmp;
    for (idx = 0; idx < MAX_DSCP_VALUE_NUM; idx++) {
        if (*pos == 'f' || *pos == 'F') {
            dscp_to_tid_value[idx] = 0xA5; /* 0xA5:默认值 */
            pos++;
            continue;
        }
        if (*pos < '0' || *pos > '7') {
        ...
        dscp_to_tid_value[idx] = *pos - '0';
        pos++;
    }
    osal_kfree(tmp);
    ret = uapi_wifi_set_dscp_tid_map((td_u8)args->net_type, &dscp_to_tid_value, MAX_DSCP_VALUE_NUM);
    if (ret != ERRCODE_SUCC) {
        return AT_RET_SYNTAX_ERROR;
    }
