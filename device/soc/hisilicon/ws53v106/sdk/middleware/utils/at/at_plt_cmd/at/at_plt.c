...
#include "nv.h"
#if (defined(CHIP_WS53) && CHIP_WS53)
...
{
    uapi_at_print("los_at_plt_cmd_register EXCUTE\r\n");
...
*****************************************************************************/
static td_u32 set_mac_addr_with_type(td_s32 mac_type, td_uchar *mac_addr)
{
    td_u32 ret;
    switch (mac_type) {
        case MAC_ADDR_EFUSE:
...
#if defined(CONFIG_MIDDLEWARE_SUPPORT_NV)
            ret = uapi_nv_write(NV_ID_SYSTEM_FACTORY_SLE_MAC, mac_addr, WIFI_MAC_LEN);
            if (ret != ERRCODE_SUCC) {
...
at_ret_t set_efuse_mac_addr(const efusemac_args_t *args)
{
    ...
    if (set_mac_addr_with_type(args->mac_type, mac_addr) != ERRCODE_SUCC) {
        return AT_RET_CMD_PARA_ERROR;
    }
...
at_ret_t get_efuse_mac_addr(void)
{
    ...
    uapi_nv_read(NV_ID_SYSTEM_FACTORY_MAC, WIFI_MAC_LEN, &nv_mac_length, mac_addr);
    ret = mac_addr_nv_check(mac_addr);
    if (ret != ERRCODE_SUCC) {
        /* 获取NV中的MAC为非法值时，尝试获取NV工厂区中的MAC */
        kv_read_factory(NV_ID_SYSTEM_FACTORY_MAC, WIFI_MAC_LEN, &nv_mac_length, mac_addr);
        ret = mac_addr_nv_check(mac_addr);
    }
    if (ret != ERRCODE_SUCC) {
        uapi_at_print("GET SLAVE NV MAC ERROR, ret : 0x%x\r\n", ret);
    }
...
#if defined(CONFIG_MIDDLEWARE_SUPPORT_NV)
    ret = uapi_nv_read(NV_ID_SYSTEM_FACTORY_SLE_MAC, WIFI_MAC_LEN, &nv_mac_length, mac_addr);
    ...
    if (mac_addr_is_zero(mac_addr)) {
        /* 获取NV中的MAC为非法值时，尝试获取NV工厂区中的MAC */
        ret = kv_read_factory(NV_ID_SYSTEM_FACTORY_SLE_MAC, WIFI_MAC_LEN, &nv_mac_length, mac_addr);
        if (ret != ERRCODE_SUCC || nv_mac_length != WIFI_MAC_LEN) {
...
#if defined(ENABLE_LOW_POWER) && (ENABLE_LOW_POWER == YES)
    pm_lpc_type type = (g_sleep_mode == 0) ? PM_NO_SLEEP : PM_DEEP_SLEEP;
    uapi_lpc_set_type(type);
#endif
    return ERRCODE_SUCC;
}
...
#if defined(ENABLE_LOW_POWER) && (ENABLE_LOW_POWER == YES)
errcode_t __attribute__((weak)) pm_config_gpio_switch_udsleep(pin_t pin)
{
    unused(pin);
...
{
#if defined(ENABLE_LOW_POWER) && (ENABLE_LOW_POWER == YES)
    if (pm_config_gpio_switch_udsleep(args->para1) != ERRCODE_SUCC) {
        return AT_RET_SYNTAX_ERROR;
    }
#else
    unused(args);
...
{
#if defined(ENABLE_LOW_POWER) && (ENABLE_LOW_POWER == YES) && defined(BOARD_ASIC)
    uapi_at_print("+at_set_freq_mode :%d\r\n", args->para1);
    system_set_pll_clock_flag((uint8_t)args->para1);
#else
    unused(args);
#endif
    return AT_RET_OK;
...
at_ret_t cmd_set_pm(const pm_args_t *args)
{
    unused(args);
#if defined(ENABLE_LOW_POWER) && (ENABLE_LOW_POWER == YES)
    pm_lpc_type type = (args->para1 == 0) ? PM_NO_SLEEP : PM_DEEP_SLEEP;
    uapi_lpc_set_type(type);
#endif
    return AT_RET_OK;
}
...
__attribute__((weak)) void reboot_port_soft_reboot_chip(void)
{
    return;
}
...
at_ret_t cmd_set_customer_rsvd2_efuse(const customer_rsvd_efuse_args_t *args)
{
    ...
    td_s32 argc = at_plt_convert_bin_to_dec((td_s32)args->para_map);
    errcode_t ret;
    td_u8 key[CUSTOM_RESVED2_EFUSE_BYTE_LEN];
    td_u32 len = 0;
    td_u8 tmp_data;
    ...
    len = strlen((char *)(args->para1 + 2)); /* 2:偏移2个字符 */
    if (len != CUSTOM_RESVED2_EFUSE_BYTE_LEN * 2) { /* 2:乘2 */
        return AT_RET_CMD_PARA_ERROR;
    }
    memset_s(key, CUSTOM_RESVED2_EFUSE_BYTE_LEN, 0, CUSTOM_RESVED2_EFUSE_BYTE_LEN);
    at_str_to_hex((char *)(args->para1 + 2), len, key); /* 2:偏移2个字符 */
    for (td_u8 index = 0; index < (CUSTOM_RESVED2_EFUSE_BYTE_LEN >> 1); index++) {
        tmp_data = key[index];
        key[index] = key[CUSTOM_RESVED2_EFUSE_BYTE_LEN - 1 - index];
        key[CUSTOM_RESVED2_EFUSE_BYTE_LEN - 1 - index] = tmp_data;
    }
    ret = efuse_write_customer_rsvd2_efuse(key, CUSTOM_RESVED2_EFUSE_BYTE_LEN);
    if (ret != ERRCODE_SUCC) {
        uapi_at_print("SET CUSTOMER RSVD2 EFUSE ERROR, ret : 0x%x\r\n", ret);
        return AT_RET_CMD_PARA_ERROR;
    }
    uapi_at_print("OK\r\n");
#else
...
#ifdef _PRE_WLAN_FEATURE_MFG_TEST
    errcode_t ret;
    td_u8 data[CUSTOM_RESVED2_EFUSE_BYTE_LEN];
    td_u32 index;

    memset_s(data, CUSTOM_RESVED2_EFUSE_BYTE_LEN, 0, CUSTOM_RESVED2_EFUSE_BYTE_LEN);
    ret = efuse_read_customer_rsvd2_efuse(data, sizeof(data));
    if (ret != EXT_ERR_SUCCESS) {
        uapi_at_print("READ EFUSE CUSTOM RESVED2 ERROR, ret : 0x%x\r\n", ret);
        return AT_RET_SYNTAX_ERROR;
    }

    uapi_at_print("RESERVED2 EFUSE:0x");
    for (index = sizeof(data); index > 0; index--) {
        uapi_at_print("%02x", data[index - 1]);
    }
    uapi_at_print("\r\n");
#endif
    return AT_RET_OK;
...
#define SIZE_12_BITS 12
#define BIT_TO_BYTE 8
#define WIFI_RSSI_1_PG15 831
#define WIFI_RSSI_2_PG19 835
#define WIFI_RSSI_3_PG23 839
#define WIFI_POWER_1_PG12 828
#define WIFI_POWER_2_PG16 832
#define WIFI_POWER_3_PG20 836
#define WIFI_MAC_1_PG24 840
#define WIFI_MAC_2_PG25 841
#define WIFI_MAC_3_PG26 842
#define BLSE_POWER_1_LOCKFLAG 1300
#define BLSE_POWER_2_LOCKFLAG 1301
#define BLSE_POWER_3_LOCKFLAG 1302
...
typedef struct {
    ...
    uint16_t id_size;      /* 以bit为单位 */
} efuse_data_stru;

const efuse_data_stru g_efuse_mfg_cfg[] = {
    {289, 15}, /* xo_trim 第一组 15--fine:1~4 coarse 5-10 temp:12~15 */
    {305, 15}, /* xo_trim 第二组 15--fine:1~4 coarse 5-10 temp:12~15 */
    {320, 15}, /* xo_trim 第三组 15--fine:1~4 coarse 5-10 temp:12~15 */

    {1392, 32}, /* wifi power  11b offset 第一组 */
    {1424, 32}, /* wifi power  ofdm 20M offset 第一组 */
    {1456, 32}, /* wifi power  ofdm 40M offset 第一组 */
    {1488, 15}, /* rssi 第一组 15--band1:0~4 band2:5~9 band3:10~14 */

    {1504, 32}, /* wifi power  11b offset 第二组 */
    {1536, 32}, /* wifi power  ofdm 20M offset 第二组 */
    {1568, 32}, /* wifi power  ofdm 40M offset 第二组 */
    {1600, 15}, /* rssi 第二组 15--band1:0~4 band2:5~9 band3:10~14 */

    {1616, 32}, /* wifi power  11b offset 第三组 */
    {1648, 32}, /* wifi power  ofdm 20M offset 第三组 */
    {1680, 32}, /* wifi power  ofdm 40M offset 第三组 */
    {1712, 15}, /* rssi 第三组 15--band1:0~4 band2:5~9 band3:10~14 */

    {1728, 48}, /* wifi mac 第一组 */
    {1776, 48}, /* wifi mac 第二组 */
    {1824, 48}, /* wifi mac 第三组 */

    {1264, 12}, /* bsle power offset 第一组 */
    {1276, 12}, /* bsle power offset 第二组 */
    {1288, 12}, /* bsle power offset 第三组 */
};

typedef struct {
    uint8_t xo_trim_cnt;
    uint8_t wifi_power_cnt;
    uint8_t wifi_rssi_cnt;
...
td_u32 cmd_get_efuse_times(uint16_t lock_bit[], uint8_t *efuse_times_cnt)
{
    uint8_t i, value = 0;
    td_u32 ret = 0;
...
td_void cmd_efuse_print_wifi_calidata(efuse_mfg_cali_data_stru mfg_data)
{
    ...
    uint16_t lock_wifi_rssi[] = {WIFI_RSSI_1_PG15, WIFI_RSSI_2_PG19, WIFI_RSSI_3_PG23};
    uint16_t lock_wifi_power[] = {WIFI_POWER_1_PG12, WIFI_POWER_2_PG16, WIFI_POWER_3_PG20};
    uint16_t lock_wifi_mac[] = {WIFI_MAC_1_PG24, WIFI_MAC_2_PG25, WIFI_MAC_3_PG26};

    if (cmd_get_efuse_times(lock_wifi_power, &g_efuse_times_left.wifi_power_cnt) != EXT_ERR_SUCCESS) {
        return;
    }
    uapi_at_print("WiFi Power Param:  times left: %d\r\n",
                  EFUSE_GROUP_NUM - g_efuse_times_left.wifi_power_cnt);
    for (i = 0; i < EFUSE_GROUP_NUM; i++) {
    ...
    if (cmd_get_efuse_times(lock_wifi_rssi, &g_efuse_times_left.wifi_rssi_cnt) != EXT_ERR_SUCCESS) {
        return;
    }
    uapi_at_print("WiFi Rssi Param:  times left: %d\r\n", EFUSE_GROUP_NUM - g_efuse_times_left.wifi_rssi_cnt);
    for (i = 0; i < EFUSE_GROUP_NUM; i++) {
        ...
        uapi_at_print("    [%d] %4d %4d %4d\r\n", i,
                      ...
                      (rssi_offset_2 & 0x10 ? (td_s8)(rssi_offset_2 | 0xE0) : (td_s8)rssi_offset_2)); /* 5bit无符号数转换为有符号数 */
    }

    if (cmd_get_efuse_times(lock_wifi_mac, &g_efuse_times_left.wifi_mac_cnt) != EXT_ERR_SUCCESS) {
        return;
    }
    uapi_at_print("WiFi Mac Addr:  times left: %d\r\n", EFUSE_GROUP_NUM - g_efuse_times_left.wifi_mac_cnt);
    for (i = 0; i < EFUSE_GROUP_NUM; i++) {
        uapi_at_print("    [%d] %02x:%02x:%02x:%02x:%02x:%02x\r\n", i, mfg_data.wifi_mac[i].mac_addr[0],
            mfg_data.wifi_mac[i].mac_addr[1], mfg_data.wifi_mac[i].mac_addr[2], mfg_data.wifi_mac[i].mac_addr[3], /* mac 1/2/3位 */
            mfg_data.wifi_mac[i].mac_addr[4], mfg_data.wifi_mac[i].mac_addr[5]); /* mac 4/5位 */
    }
}

td_void  cmd_efuse_print_all_calidata(efuse_mfg_cali_data_stru mfg_data)
{
    uint8_t i;
    uint16_t xo_trim = 0;
    uint8_t coarse[EFUSE_GROUP_NUM] = {0};
    uint8_t fine[EFUSE_GROUP_NUM] = {0};
    uint8_t temper[EFUSE_GROUP_NUM] = {0};
    uint16_t bsle_c_offset[EFUSE_GROUP_NUM] = {0};
    uint16_t lock_xo_trim[] = {XO_TRIM_1_PG1, XO_TRIM_2_PG2, XO_TRIM_3_PG3};
    uint16_t lock_bsle_power[] = {BLSE_POWER_1_LOCKFLAG, BLSE_POWER_2_LOCKFLAG, BLSE_POWER_3_LOCKFLAG};

    (td_void)memset_s(&g_efuse_times_left, sizeof(g_efuse_times_left), 0, sizeof(g_efuse_times_left));

    if (cmd_get_efuse_times(lock_xo_trim, &g_efuse_times_left.xo_trim_cnt) != EXT_ERR_SUCCESS) {
        return;
    }
    uapi_at_print("Freq Param:  times left: %d\r\n", EFUSE_GROUP_NUM - g_efuse_times_left.xo_trim_cnt);
    for (i = 0; i < EFUSE_GROUP_NUM; i++) {
        xo_trim = ((mfg_data.xo_trim[i] >> SIZE_1_BITS) & 0x7FF);
        coarse[i] = ((xo_trim >> SIZE_4_BITS) & 0x3F);
        fine[i] = (xo_trim & 0xF);
        temper[i] = ((mfg_data.xo_trim[i] >> SIZE_12_BITS) & 0xF);
        uapi_at_print("    [%d] %4d %4d %4d\r\n", i, coarse[i], fine[i], temper[i]);
    }

    cmd_efuse_print_wifi_calidata(mfg_data);

    if (cmd_get_efuse_times(lock_bsle_power, &g_efuse_times_left.bsle_power_cnt) != EXT_ERR_SUCCESS) {
        return;
    }
    uapi_at_print("BSLE Power Param:  times left: %d\r\n", EFUSE_GROUP_NUM - g_efuse_times_left.bsle_power_cnt);
    bsle_c_offset[0] = mfg_data.bsle_c_offset[0] & 0xFFF;
    bsle_c_offset[1] = (mfg_data.bsle_c_offset[1] & 0xFFF0) >> SIZE_4_BITS;
    bsle_c_offset[2] = mfg_data.bsle_c_offset[2] & 0xFFF; /* 2:第三组bsle power校准参数 */
    for (i = 0; i < EFUSE_GROUP_NUM; i++) {
        uapi_at_print("    [%d] 0x%x\r\n", i,
            (bsle_c_offset[i] & 0x800 ? (td_s16)(bsle_c_offset[i] | 0xF000) : (td_s16)bsle_c_offset[i])); /* 12bit无符号数转换为有符号数 */
    }
    ...
    if (mac_addr_is_zero(mac_addr)) { /* 获取NV中的sle MAC为非法值时，尝试获取“从NV区”中的sle MAC */
        ret = kv_read_factory(NV_ID_SYSTEM_FACTORY_SLE_MAC, WIFI_MAC_LEN, &nv_mac_length, mac_addr);
        if (ret != ERRCODE_SUCC || nv_mac_length != WIFI_MAC_LEN) {
            uapi_at_print("GET SLAVE NV SLE MAC ERROR, ret : 0x%x\r\n", ret);
        }
    }
    uapi_at_print("NV SLE MAC: " EXT_AT_MACSTR "\r\n", ext_at_mac2str(mac_addr));
#endif
}
...
at_ret_t cmd_efuse_read_cali_info(void)
{
    ...
    efuse_mfg_cali_data_status ptr[] = {
        {(td_u8 *)&mfg_data.xo_trim[0], sizeof(mfg_data.xo_trim[0])},
        {(td_u8 *)&mfg_data.xo_trim[1], sizeof(mfg_data.xo_trim[1])},
        {(td_u8 *)&mfg_data.xo_trim[2], sizeof(mfg_data.xo_trim[2])},
    ...
    cmd_efuse_print_all_calidata(mfg_data);
