...
int uapi_wifi_sta_get_connect_info(ext_wifi_status *connect_status)
{
    ...
    (void)os_event_clear(g_wpa_event, ~(unsigned int)WPA_EVENT_STA_STATUS_FLAG);
    (void)wpa_cli_sta_status(wpa_s);
    (void)os_event_read(g_wpa_event, WPA_EVENT_STA_STATUS_FLAG, &ret_val,
                        WIFI_WAITMODE_OR | WIFI_WAITMODE_CLR, WIFI_EVENT_DELAY_5S);

    g_sta_status = NULL;
    if (ret_val != WPA_EVENT_STA_STATUS_OK) {
        wpa_error_log1(MSG_ERROR, "os_event_read WPA_EVENT_STA_STATUS_FLAG failed ret_val = %x.", ret_val);
        return EXT_WIFI_FAIL;
    }
    return EXT_WIFI_OK;
}
