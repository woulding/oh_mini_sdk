...
#ifdef SW21_PRODUCT_FPGA
#define SERVICE_MIN_STACK 11008 /* sw21临时规避方案 */
#else
...
#define WIFI_STA_SEND_PROBE_REQ_MAX 33 /* linkloss之前最多可以发送33次probe request,对应场景--低功耗DBAC场景 */
...
errcode_t wifi_set_channel(wifi_if_type_enum iftype, int32_t channel)
{
    ...
    if (iftype == IFTYPE_STA && g_sta_enble_flag == 1) {
        ...
        service_error_log1(SERVICE_ERROR, "Srv:vap not enabled or iftype[%d] invalid", iftype);
        return ERRCODE_FAIL;
    }

    if (uapi_wifi_set_channel(ifname, strlen(ifname), channel) != 0) {
        service_error_log1(SERVICE_ERROR, "Srv:set_channel[%d] failed", channel);
        return ERRCODE_FAIL;
    }
...
errcode_t wifi_get_channel(wifi_if_type_enum iftype, int32_t *channel)
{
    ...
    if (iftype == IFTYPE_STA && g_sta_enble_flag == 1) {
        ...
        service_error_log1(SERVICE_ERROR, "Srv:vap is not enabled or iftype[%d] is invalid", iftype);
        return ERRCODE_FAIL;
    }

    *channel = uapi_wifi_get_channel(ifname, strlen(ifname));
    return ERRCODE_SUCC;
}
...
errcode_t wifi_sta_wnm_bss_query(int32_t reason_code, int32_t candidate_list)
{
    ...
    return uapi_wifi_wnm_bss_query(reason_code, candidate_list);
}
...
errcode_t wifi_sta_wnm_notify(const char *param, uint32_t len)
{
    ...
    return uapi_wifi_wnm_notify(param, len);
}
#endif
...
OSAL_STATIC void wifi_sta_connect_fill_security_type(ext_wifi_assoc_request *req)
{
    ...
    if (ap_list == NULL) {
        service_error_log1(SERVICE_ERROR, "Srv:%d malloc err.", __LINE__);
        return;
    }
...
OAL_STATIC errcode_t wifi_sta_scan_not_found_process(event_wifi_disconnected *wifi_disconnected)
{
    /* 未扫到对应AP的情况下,上报错误码 */
...
OAL_STATIC errcode_t wifi_sta_connect_part2(void)
{
    ...
    if (req.auth == WIFI_SEC_TYPE_INVALID || wifi_sta_pre_check_key(&g_sta_conn_config) == OSAL_FALSE) {
...
static void wifi_sta_connect_fill_scan_param(const wifi_sta_config_stru *config, wifi_scan_params_stru *scan_param)
{
    ...
    if (strlen(config->ssid) > 0) {
        ...
        scan_param->ssid_len = strlen(config->ssid);
    ...
    service_error_log1(SERVICE_ERROR, "scan type[%u]", scan_param->scan_type);
}
    ...
    uint32_t period, uint32_t max_try_count)
{
    uint32_t ret = service_wifi_and_sta_check();
    if (ret != ERRCODE_SUCC) {
        service_error_log1(SERVICE_ERROR, "Srv:%d:check failed", __LINE__);
        return ret;
    }
...
errcode_t wifi_sta_fast_connect(const wifi_fast_connect_stru *fast_request)
{
    ...
    if (ret != ERRCODE_SUCC) {
        service_error_log1(SERVICE_ERROR, "Srv:%d:check_device failed", __LINE__);
        return ret;
    }
...
errcode_t wifi_softap_enable(const softap_config_stru *config)
{
    ...
    if (register_callback() != 0) {
        service_error_log1(SERVICE_ERROR, "Srv:%d:register cb fail", __LINE__);
        return ERRCODE_FAIL;
    }
...
errcode_t wifi_set_psd_mode(ext_psd_option_param *psd_option)
{
    ...
    if (ret != WIFI_SUCCESS) {
        service_error_log2(SERVICE_ERROR, "Srv:%d: fail[%d]", __LINE__, ret);
        return ERROR_WIFI_UNKNOWN;
    }
    return ERRCODE_SUCC;
}
...
errcode_t wifi_set_fixed_tx_rate(unsigned char auto_rate, alg_param_stru *alg_param)
{
    ...
    ret = uapi_wifi_set_phy_txrate(auto_rate, alg_param->rate_value);
    if (ret == 0) {
        return ERRCODE_SUCC;
    } else {
...
errcode_t wifi_set_apf_list(const int8_t *apf_filter_cmd, uint16_t cmd_len, uint8_t flag)
{
    uint32_t ret;

    /* 512:apf十六进制过滤规则最长512 */
    if (cmd_len >= 512 || cmd_len == 0 || flag > 1) {
        service_error_log1(SERVICE_ERROR, "Srv:%d:invalid len", __LINE__);
        return ERROR_WIFI_INVALID_ARGS;
    }

    ret = uapi_wifi_apf_set_list(apf_filter_cmd, cmd_len, flag);
    if (ret != 0) {
        service_error_log1(SERVICE_ERROR, "Srv:%d:set_apf_list failed", __LINE__);
        return ERRCODE_FAIL;
    }
...
errcode_t wifi_set_arp_offload_apf_setting(const char *ifname, uint8_t en, uint32_t ip)
{
    uint32_t ret;

    ret = uapi_wifi_arp_offload_apf_setting(ifname, en, ip);
    if (ret != 0) {
        service_error_log2(SERVICE_ERROR, "Srv:%d:wifi_set_arp_offload_apf_setting failed, ret=[%d]", __LINE__, ret);
        return ERRCODE_FAIL;
    }
...
errcode_t wifi_sta_config_probe_req_max_times(uint8_t max_times)
{
    ...
    if ((max_times == 0) || (max_times > WIFI_STA_SEND_PROBE_REQ_MAX)) {
        service_error_log2(SERVICE_ERROR, "Srv:%d,max_times[%u] out of range", __LINE__, max_times);
        return ERRCODE_FAIL;
    }

    ret = uapi_wifi_sta_config_probe_req_max_times(max_times);
    if (ret != 0) {
