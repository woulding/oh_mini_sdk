...
uint32_t uapi_flash_write_data(flash_id_t id, uint32_t addr, const uint8_t *src, uint32_t length)
{
    ...
    do {
        ...
        write_addr += trans_len;
        send_data_len += trans_len;
    } while (write_len > 0);
...
#if defined(CONFIG_FLASH_USE_SPI_SINGLE_LINE_MODE_ONLY) && (CONFIG_FLASH_USE_SPI_SINGLE_LINE_MODE_ONLY == 1)
static errcode_t flash_switch_deeppower(flash_id_t id, bool en)
{
    spi_xfer_data_t xfer_data = { 0 };
    spi_attr_t *attr = &g_flash_config[id].attr;
    uint8_t cmd = en ? FLASH_DP_CMD : FLASH_RDP_CMD;
...
{
    if (unlikely(id >= FLASH_MAX) || unlikely(flash_info == NULL)) {
        return;
    }
