...
static bool g_efuse_inited = false;

errcode_t uapi_efuse_init(void)
{
    if (g_efuse_inited == true) {
        return ERRCODE_SUCC;
    }
...
static errcode_t efuse_write_with_check(uint32_t byte_number, const uint8_t *buffer, uint16_t length)
{
    hal_efuse_funcs_t *hal_funcs = hal_efuse_get_funcs();

    for (uint32_t i = 0; i < length; i++) {
        uint8_t read_buff = 0;
        uapi_efuse_read_buffer(&read_buff, byte_number + i, 0x1);
        if ((buffer[i] & ~read_buff) == 0) { continue; }
        uint32_t irq_sts = osal_irq_lock();
        if (hal_funcs->write_buffer_op(byte_number + i, &buffer[i], 0x1) != ERRCODE_SUCC) {
            osal_irq_restore(irq_sts);
            return ERRCODE_FAIL;
        }
        osal_irq_restore(irq_sts);
...
errcode_t uapi_efuse_write_buffer(uint32_t byte_number, const uint8_t *buffer, uint16_t length)
{
    ...
    return efuse_write_with_check(byte_number, buffer, length);
}
