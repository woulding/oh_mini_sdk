...
#define HAL_PIN_BS2X_MODE_CONTROL_CONFIG_BITS 7U
#define HAL_PIN_BS2X_DS_CFG_BITS              2U

#define HAL_PIN_BS2X_PULL_CFG_BITS            2U
#define HAL_PIN_BS2X_PULL_START_BITS          4
#define HAL_PIN_BS2X_IE_CFG_BITS              1U
#define HAL_PIN_BS2X_INPUT_ENABLE_BIT         6
/**
...
static hal_pin_pad_ctl_map_t g_pin_pull_map[] = {
    { S_MGPIO0,  S_MGPIO31, HAL_PIN_PULL_ADDR, HAL_PIN_BS2X_PULL_START_BITS }
};

#if defined(CONFIG_PINCTRL_SUPPORT_IE)
static hal_pin_pad_ctl_map_t g_pin_ie_map[] = {
    { S_MGPIO0,  S_MGPIO31, HAL_PIN_PULL_ADDR, HAL_PIN_BS2X_INPUT_ENABLE_BIT }
};
...
static hal_pin_mode_map_t *hal_pin_bs2x_get_aon_mode_addr(pin_t pin)
{
...
static errcode_t hal_pin_bs2x_set_aon_mode(pin_t pin, pin_mode_t mode)
{
    uint32_t reg_addr;
    hal_pin_mode_map_t *pin_map = NULL;

    pin_map = hal_pin_bs2x_get_aon_mode_addr(pin);
    if ((pin_map == NULL) || (pin_map->per_num == 0)) {
        return ERRCODE_FAIL;
    }
    reg_addr = (uintptr_t)pin_map->reg_addr + ((uint32_t)(pin - pin_map->begin) / pin_map->per_num) *
               (sizeof(uint32_t));
    reg16_setbits(reg_addr, 0, HAL_PIN_BS2X_MODE_CONTROL_CONFIG_BITS, mode);
    return ERRCODE_SUCC;
}

static pin_mode_t hal_pin_bs2x_get_aon_mode(pin_t pin)
{
    uint32_t reg_addr;
    hal_pin_mode_map_t *pin_map = NULL;
    ...
    reg_addr = (uintptr_t)pin_map->reg_addr + ((uint32_t)(pin - pin_map->begin) / pin_map->per_num) *
               (sizeof(uint32_t));
    return (pin_mode_t)reg16_getbits(reg_addr, 0, HAL_PIN_BS2X_MODE_CONTROL_CONFIG_BITS);
}

static hal_pin_ds_map_t *hal_pin_bs2x_get_aon_ds_addr(pin_t pin)
{
...
static errcode_t hal_pin_bs2x_set_aon_ds(pin_t pin, pin_drive_strength_t ds)
{
    uint32_t config_pos = 0;
    uint32_t reg_addr;
    hal_pin_ds_map_t *pin_map = NULL;

    pin_map = hal_pin_bs2x_get_aon_ds_addr(pin);
    if (pin_map == NULL) {
        return ERRCODE_FAIL;
    }
    reg_addr = (uintptr_t)pin_map->addr + ((uint32_t)pin - (uint32_t)(pin_map->begin)) * (sizeof(uint32_t));
    config_pos = pin_map->reg_bit;
    reg16_setbits(reg_addr, config_pos, HAL_PIN_BS2X_DS_CFG_BITS, ds);
    return ERRCODE_SUCC;
}

static pin_drive_strength_t hal_pin_bs2x_get_aon_ds(pin_t pin)
{
    uint32_t config_pos = 0;
    uint32_t reg_addr;
    hal_pin_ds_map_t *pin_map = NULL;
    ...
    return (pin_drive_strength_t)reg16_getbits(reg_addr, config_pos, HAL_PIN_BS2X_DS_CFG_BITS);
}

static hal_pin_pad_ctl_map_t *hal_pin_bs2x_get_pull_addr(pin_t pin)
{
...
static errcode_t hal_pin_bs2x_set_aon_pull(pin_t pin, pin_pull_t pull_type)
{
    uint32_t reg_addr;
    uint32_t config_pos;
    hal_pin_pad_ctl_map_t *pin_map = NULL;

    pin_map = hal_pin_bs2x_get_pull_addr(pin);
    if (pin_map == NULL) {
        return ERRCODE_FAIL;
    }
    ...
    reg16_setbits(reg_addr, config_pos, HAL_PIN_BS2X_PULL_CFG_BITS, pull_type);
    return ERRCODE_SUCC;
}

static pin_pull_t hal_pin_bs2x_get_aon_pull(pin_t pin)
{
    uint32_t reg_addr;
    uint32_t config_pos;
    hal_pin_pad_ctl_map_t *pin_map = NULL;
    ...
    reg_addr = (uintptr_t)pin_map->addr + ((uint32_t)pin - (uint32_t)pin_map->begin) *
            (uint32_t)(sizeof(uint32_t));
    config_pos = pin_map->config_bit;
    return (pin_pull_t)reg16_getbits(reg_addr, config_pos, HAL_PIN_BS2X_PULL_CFG_BITS);
}

static errcode_t hal_pin_bs2x_set_mode(pin_t pin, pin_mode_t mode)
{
    ...
    return hal_pin_bs2x_set_aon_mode(pin, mode);
}

static pin_mode_t hal_pin_bs2x_get_mode(pin_t pin)
{
    return hal_pin_bs2x_get_aon_mode(pin);
}

static errcode_t hal_pin_bs2x_set_ds(pin_t pin, pin_drive_strength_t ds)
{
    return hal_pin_bs2x_set_aon_ds(pin, ds);
}

static pin_drive_strength_t hal_pin_bs2x_get_ds(pin_t pin)
{
    return hal_pin_bs2x_get_aon_ds(pin);
}

static errcode_t hal_pin_bs2x_set_pull(pin_t pin, pin_pull_t pull_type)
{
    return hal_pin_bs2x_set_aon_pull(pin, pull_type);
}

static pin_pull_t hal_pin_bs2x_get_pull(pin_t pin)
{
    return hal_pin_bs2x_get_aon_pull(pin);
}

#if defined(CONFIG_PINCTRL_SUPPORT_IE)
static hal_pin_pad_ctl_map_t *hal_pin_bs2x_get_ie_addr(pin_t pin)
{
    hal_pin_pad_ctl_map_t *pin_map = NULL;
    uint32_t map_size = 0;
...
static errcode_t hal_pin_bs2x_set_ie(pin_t pin, pin_input_enable_t ie_type)
{
    uint32_t reg_addr;
    uint32_t config_pos;
    hal_pin_pad_ctl_map_t *pin_map = NULL;

    pin_map = hal_pin_bs2x_get_ie_addr(pin);
    if (pin_map == NULL) {
        return ERRCODE_FAIL;
    }
    reg_addr = (uintptr_t)pin_map->addr + ((uint32_t)pin - (uint32_t)pin_map->begin) *
            (uint32_t)(sizeof(uint32_t));
    config_pos = pin_map->config_bit;
    reg16_setbits(reg_addr, config_pos, HAL_PIN_BS2X_IE_CFG_BITS, ie_type);
    return ERRCODE_SUCC;
}

static pin_input_enable_t hal_pin_bs2x_get_ie(pin_t pin)
{
    uint32_t reg_addr;
    uint32_t config_pos;
    hal_pin_pad_ctl_map_t *pin_map = NULL;
    ...
    reg_addr = (uintptr_t)pin_map->addr + ((uint32_t)pin - (uint32_t)pin_map->begin) *
            (uint32_t)(sizeof(uint32_t));
    config_pos = pin_map->config_bit;
    return (pin_input_enable_t)reg16_getbits(reg_addr, config_pos, HAL_PIN_BS2X_IE_CFG_BITS);
}
#endif /* CONFIG_PINCTRL_SUPPORT_IE */

static hal_pin_funcs_t g_hal_pin_bs2x_funcs = {
    .set_mode = hal_pin_bs2x_set_mode,
    .get_mode = hal_pin_bs2x_get_mode,
    .set_ds = hal_pin_bs2x_set_ds,
    .get_ds = hal_pin_bs2x_get_ds,
    .set_pull = hal_pin_bs2x_set_pull,
    .get_pull = hal_pin_bs2x_get_pull,
#if defined(CONFIG_PINCTRL_SUPPORT_IE)
    .set_ie = hal_pin_bs2x_set_ie,
    .get_ie = hal_pin_bs2x_get_ie,
#endif /* CONFIG_PINCTRL_SUPPORT_IE */
};

hal_pin_funcs_t *hal_pin_bs2x_funcs_get(void)
{
    return &g_hal_pin_bs2x_funcs;
}
