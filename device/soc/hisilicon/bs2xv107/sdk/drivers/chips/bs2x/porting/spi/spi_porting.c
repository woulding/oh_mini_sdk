...
{
#if defined(BUILD_FLASHBOOT)
    if (on) {
        writew(0x5200055C, 0x3); // sel XO32M
        reg16_setbit(0x52000044, ((uint16_t)bus + 0x1)); // enable clk
    } else {
        reg16_clrbit(0x52000044, ((uint16_t)bus + 0x1)); // disable clk
    }
#else
