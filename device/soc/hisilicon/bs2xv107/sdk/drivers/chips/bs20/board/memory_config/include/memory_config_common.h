...
#define APP_PROGRAM_ORIGIN      (SFC_FLASH_START + APPLICATION_IMAGE_OFFSET + CODE_INFO_OFFSET)
#define APP_PROGRAM_LENGTH      (APPLICATION_IMAGE_LEN - CODE_INFO_OFFSET)
...
#define APP_STACK_TOTAL_LEN     (APP_NMI_STACK_LEN + APP_EXCP_STACK_LEN + APP_IRQ_STACK_LEN + APP_USER_STACK_LEN)
#else
/* stack for normal 1.5k */
...
#ifdef CONFIG_SUPPORT_USER_PRESERVE_DATA
#define PRESERVED_REGION_LENGTH (0x100 + CONFIG_USER_PRESERVE_DATA_LEN)
#else
...
#if defined(SUPPORT_LOG_BUF_IN_ITCM) && defined(SUPPORT_STACK_IN_ITCM)
/* LOG BUF和启动栈放ITCM */
#define LOGGING_REGION_START  (APP_USER_STACK_BASEADDR - APP_LOGGING_LENGTH)
#elif defined(SUPPORT_LOG_BUF_IN_ITCM)
/* LOG BUF放ITCM，启动栈放DTCM */
#define LOGGING_REGION_START  APP_DTCM_ORIGIN
#elif defined(SUPPORT_STACK_IN_ITCM)
/* LOG BUF放DTCM，启动栈放ITCM */
#define LOGGING_REGION_START  (APP_ITCM_ORIGIN + APP_ITCM_LENGTH - APP_LOGGING_LENGTH)
#else
/* LOG BUF放DTCM，启动栈放DTCM */
