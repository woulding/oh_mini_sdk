...
tiot_pm_retcode tiot_pm_slave_request_sleep_handle(tiot_pm *pm)
{
    ...
    const uint8_t request_sleep_command = SYS_INF_REQUEST_SLEEP;
    tiot_xfer_tx_param tx_param = { SUBSYS_SYS };

    tiot_print_info("[TIoT][pm]request sleep enter.\n");

    ret = tiot_xfer_send(xfer, &request_sleep_command, sizeof(request_sleep_command), &tx_param);
    if (ret != sizeof(request_sleep_command)) {
        tiot_print_err("[TIoT][pm]send request sleep msg fail.\r\n");
        return TIOT_PM_EVTRET_FAIL;
    }
...
tiot_pm_retcode tiot_pm_slave_sleep_ack_handle(tiot_pm *pm)
{
    ...
    if (pm->work_vote == 1) {
        tiot_print_info("[TIoT][pm]dev no agree sleep.\n");
        if (tiot_pm_send_sys_msg(pm, SYS_INF_DEV_NOAGREE_HOST_SLP) != 0) {
            return TIOT_PM_EVTRET_FAIL;
        }
        return TIOT_PM_EVTRET_KEEP_STATE;
    }
