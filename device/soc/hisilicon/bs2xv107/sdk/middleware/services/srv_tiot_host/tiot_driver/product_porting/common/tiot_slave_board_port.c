...
int32_t tiot_slave_board_init(void *param)
{
    ...
    uint32_t wakein_pin = pins[TIOT_SLAVE_PIN_HOST_WAKEUP_DEVICE];
    ...
    if ((wakein_pin != TIOT_PIN_NONE) && (wakein_pin != TIOT_PIN_UART_WAKEUP_NUM)) {
        (void)uapi_pin_set_mode(wakein_pin, (pin_mode_t)HAL_PIO_FUNC_GPIO);
        (void)uapi_gpio_set_dir(wakein_pin, GPIO_DIRECTION_INPUT);
        (void)uapi_gpio_set_val(wakein_pin, GPIO_LEVEL_LOW);
    }
