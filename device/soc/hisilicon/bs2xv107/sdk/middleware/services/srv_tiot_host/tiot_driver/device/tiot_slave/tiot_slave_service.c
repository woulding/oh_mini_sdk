...
static inline void slave_xfer_set_initial_config(tiot_xfer_manager *xfer)
{
    ...
    (void)tiot_xfer_set_config(xfer, &uart_config);
}

static inline int32_t slave_set_initial_pm_state(tiot_pm *pm, uint32_t power_ctrl_pin)
{
    int ret = 0;
    ...
    if (power_ctrl_pin == TIOT_PIN_NONE) {
        ...
        if (ret != 0) {
            tiot_print_err("slave set default state sleep fail.\r\n");
        }
    } else {
...
static int32_t tiot_slave_service_open(tiot_controller *ctrl, void *param)
{
    ...
    (void)slave_xfer_set_initial_config(&ctrl->transfer);
    ret = tiot_slave_service_install_packet(ctrl, dev_id);
    if (ret != 0) {
        goto pkt_install_fail;
    }
    ret = slave_set_initial_pm_state(&ctrl->pm, power_ctrl_pin);
    if (ret != 0) {
...
pkt_install_fail:
    tiot_pm_set_event(&ctrl->pm, TIOT_PM_EVENT_POWEROFF);
power_on_fail:
                                        ...
                                        uint32_t len, const tiot_xfer_tx_param *param)
{
    ...
    if (ret <= 0) {
        tiot_print_err("[TIoT]wakeup host fail, pm state is 0x%x.\n", ctrl->pm.state);
        goto vote_down;
    }
...
static int32_t slave_pm_suspend_prepare(tiot_controller *ctrl)
{
    int32_t ret;
    const int allow_sleep_wait_ms = 100;

    ret = tiot_pm_set_event(&ctrl->pm, TIOT_PM_EVENT_REQUEST_SLEEP);
    if (ret != 0) {
        tiot_print_err("[TIoT]suspend prepare, request sleep fail.\n");
        return -1;
    }

    ret = tiot_pm_wait_state(&ctrl->pm, TIOT_PM_STATE_SLEEP, allow_sleep_wait_ms);
    if (ret <= 0) {
        tiot_print_err("[TIoT]request sleep is ignored by host or dev no agree sleep.\n");
        tiot_pm_set_event(&ctrl->pm, TIOT_PM_EVENT_BACK_TO_WORK);
        return -1;
    }
    return 0;
}

static int32_t tiot_slave_service_pm_ctrl(tiot_controller *ctrl, uint8_t cmd)
{
    uint8_t dev_id;
    if (tiot_slave_get_dev_id(ctrl, &dev_id) != 0) {
        return -1;
    }
    if (cmd >= TIOT_PM_CTRL_BUTT) {
        tiot_print_err("[TIoT]invalid pm ctrl cmd: %d\n", cmd);
        return -1;
    }
    if (cmd == TIOT_PM_CTRL_SUSPEND_PREPARE) {
        return slave_pm_suspend_prepare(ctrl);
    }
    tiot_print_warning("[TIoT]not support pm ctrl cmd: %d\n", cmd);
    return -1;
}
...
int32_t tiot_slave_service_init(void)
{
    ...
    for (i = 0; i < CONFIG_TIOT_SLAVE_DEV_NUM; i++) {
        ...
        g_tiot_slave_controllers[i].ops.pm_ctrl = tiot_slave_service_pm_ctrl;
    }
    return 0;
deinit:
