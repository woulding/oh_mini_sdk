...
#define UPLOAD_BUFF_LEN  133
#define UPLOAD_CMD_NUM   2
#define YMODEM_HEAD_LEN  3
#define CAN_COUNT    3
...
typedef enum {
    ...
    TIOT_FW_EXTCMD_GET_VERSION_UPLOAD = 0x5,
    TIOT_FW_EXTCMD_RECV_ACK_FRAME = 0x6,
    TIOT_FW_EXTCMD_RECV_STRING = 0x7,
    TIOT_FW_EXTCMD_NUM
...
static int32_t tiot_fw_get_version_upload_handle_hiburn(tiot_fw *fw, const uint8_t *cmd, uint16_t len);
static int32_t tiot_fw_recv_ack_frame_handle_hiburn(tiot_fw *fw, const uint8_t *cmd, uint16_t len);
static int32_t tiot_fw_recv_string_handle_hiburn(tiot_fw *fw, const uint8_t *cmd, uint16_t len);
...
static const hiburn_ext_cmd_handle_func tiot_fw_hiburn_ext_cmd_table[TIOT_FW_EXTCMD_NUM] = {
    ...
    tiot_fw_get_version_upload_handle_hiburn,
    tiot_fw_recv_ack_frame_handle_hiburn,
    tiot_fw_recv_string_handle_hiburn
};

static int32_t tiot_fw_read_until(tiot_fw *fw, uint8_t *buff, uint32_t len, uint32_t timeout_ms)
{
    int32_t ret = 0;
    uint32_t read_len = len;
    uint32_t read_cnt = 0;
    while (read_len > 0) {
        ret = tiot_firmware_read_timeout(fw, &buff[read_cnt], read_len, timeout_ms);
        if (ret <= 0) {
            return -1;
        }
...
static int32_t tiot_fw_ack_frame_receive(tiot_fw *fw, tiot_fw_ack_frame_hiburn *frame,
                                         uint16_t header_received,
                                         uint32_t timeout_ms)
{
    int32_t ret = 0;
    ...
    /* Header */
    ret = tiot_fw_read_until(fw, &frame->header[header_received], receive_cnt, timeout_ms);
    if (ret != (int32_t)receive_cnt) {
    ...
    ret = tiot_fw_read_until(fw, frame->data, receive_cnt, timeout_ms);
    if (ret != (int32_t)receive_cnt) {
        return TIOT_FW_LOAD_RET_ERR;
    }
...
static int32_t tiot_fw_ack_handle_hiburn(tiot_fw *fw, const uint8_t *cmd, uint16_t len)
{
    ...
    uint8_t ack = 0;
    /* 第一次需要等待擦除flash */
    const uint32_t recv_ack_timeout = 5000;

    tiot_unused(cmd);
    tiot_print_dbg("[TIoT:hiburn]ack handle...\r\n");
    ret = tiot_firmware_read_timeout(fw, &ack, sizeof(uint8_t), recv_ack_timeout);
...
static int32_t tiot_fw_init_handle_hiburn(tiot_fw *fw, const uint8_t *cmd, uint16_t len)
{
    ...
    ret = tiot_fw_read_until(fw, init_str_buff, sizeof(boot_str) - 1, READ_TIMEOUT_MS);
...
static int32_t tiot_fw_handshake_handle_hiburn(tiot_fw *fw, const uint8_t *cmd, uint16_t len)
{
    ...
    for (i = 0; i < handshake_ack_count; i++) {
        ret = tiot_fw_ack_frame_receive(fw, &frame, 0, READ_TIMEOUT_MS);
...
static int32_t tiot_fw_ack_frame_handle_hiburn(tiot_fw *fw, const uint8_t *cmd, uint16_t len)
{
    ...
    tiot_xfer_rx_param rx_param = { READ_TIMEOUT_MS, 0 };
    tiot_controller *controller = tiot_container_of(fw, tiot_controller, firmware);
    tiot_fw_ack_frame_hiburn frame = { 0 };
    ...
    while (1) {
        ...
        if (ack == TIOT_FW_HIBURN_ACK_START_FIRST) {
            frame.header[0] = ack;
            ret = tiot_fw_ack_frame_receive(fw, &frame, 1, READ_TIMEOUT_MS);
            if (ret != TIOT_FW_LOAD_RET_OK) {
                return ret;
            }
            ret = tiot_fw_ack_frame_check_ack(&frame);
...
static int32_t tiot_fw_recv_ack_frame_handle_hiburn(tiot_fw *fw, const uint8_t *cmd, uint16_t len)
{
    ...
    while (1) {
        ...
        if (ack == TIOT_FW_HIBURN_ACK_START_FIRST) {
            frame.header[0] = ack;
            ret = tiot_fw_ack_frame_receive(fw, &frame, 1, recv_ack_timeout);
            if (ret != TIOT_FW_LOAD_RET_OK) {
                return ret;
            }
            ret = tiot_fw_ack_frame_check_ack(&frame);
...
static int32_t tiot_fw_recv_string_handle_hiburn(tiot_fw *fw, const uint8_t *cmd, uint16_t len)
{
    ...
    tiot_unused(len);
    while (1) {
        ret = tiot_firmware_read_timeout(fw, &byte, sizeof(uint8_t), last_byte_timeout);
        if (ret == 0) {
            break;
        }
        if (ret != sizeof(uint8_t)) {
...
static int32_t tiot_fw_get_version_handle_hiburn(tiot_fw *fw, const uint8_t *cmd, uint16_t len)
{
    ...
    ret = tiot_fw_read_until(fw, (uint8_t *)device_version, VERSION_STR_LEN, READ_TIMEOUT_MS);
...
static int32_t get_version_str_upload(tiot_fw *fw, uint8_t *buff, uint32_t buff_size)
{
    int32_t ret;
    const uint8_t start = TIOT_FW_FILE_STA;
    const uint8_t ack = TIOT_FW_FILE_ACK;
    const uint8_t cancel[CAN_COUNT] = { TIOT_FW_FILE_CAN, TIOT_FW_FILE_CAN, TIOT_FW_FILE_CAN };
    /* 读start frame. */
    ret = tiot_firmware_write(fw, &start, sizeof(uint8_t));
    if (ret != sizeof(uint8_t)) {
        return TIOT_FW_LOAD_RET_ERR;
    }
    ret = tiot_fw_read_until(fw, buff, buff_size, READ_TIMEOUT_MS);
    if (ret != UPLOAD_BUFF_LEN) {
        return TIOT_FW_LOAD_RET_ERR;
    }
    ret = tiot_firmware_write(fw, &ack, sizeof(uint8_t));
    if (ret != sizeof(uint8_t)) {
        return TIOT_FW_LOAD_RET_ERR;
    }
    /* 读版本号. */
    ...
    ret = tiot_firmware_write(fw, cancel, sizeof(cancel));
    if (ret != sizeof(cancel)) {
        return TIOT_FW_LOAD_RET_ERR;
    }
    return TIOT_FW_LOAD_RET_OK;
}
...
static int32_t tiot_fw_get_version_upload_handle_hiburn(tiot_fw *fw, const uint8_t *cmd, uint16_t len)
{
    int32_t ret;
    const char *fwpkg_version;
    char upload_data[UPLOAD_BUFF_LEN];
    tiot_fw_ack_frame_hiburn frame = { 0 };
    /* Hiburn upload命令只需要两个参数，这里为了方便处理，在命令的参数位置加上了`fwpkg version`
     * (notes: 对端不会校验多余参数) */
    if (len < HIBURN_FRAME_HEAD_LEN + sizeof(uint32_t) * UPLOAD_CMD_NUM + VERSION_STR_LEN) {
        tiot_print_err("[TIoT:hibrun]cmd len not enough: %d\r\n", len);
        return TIOT_FW_LOAD_RET_ERR;
    }
    fwpkg_version = (const char *)cmd + HIBURN_FRAME_HEAD_LEN + sizeof(uint32_t) * UPLOAD_CMD_NUM;
    /* Hiburn cmd ACK. */
    ret = tiot_fw_ack_frame_receive(fw, &frame, 0, READ_TIMEOUT_MS);
    if (ret != TIOT_FW_LOAD_RET_OK) {
        return ret;
    ...
    if (get_version_str_upload(fw, (uint8_t *)upload_data, UPLOAD_BUFF_LEN) != TIOT_FW_LOAD_RET_OK) {
        return TIOT_FW_LOAD_RET_ERR;
    }
    /* ignore ACK failure. */
    ret = tiot_fw_ack_frame_receive(fw, &frame, 0, READ_TIMEOUT_MS);
    ...
    tiot_print_info("[TIoT:hibrun]old ver: %s;new ver: %s\r\n", upload_data + YMODEM_HEAD_LEN, fwpkg_version);
    /* 版本号不匹配，就继续加载 */
    if (memcmp(fwpkg_version, upload_data + YMODEM_HEAD_LEN, VERSION_STR_LEN) != 0) {
        return TIOT_FW_LOAD_RET_OK;
    }
    /* 版本号匹配，软复位device，然后退出加载流程 */
    ret = tiot_fw_send_reset_cmd(fw);
    if (ret != TIOT_FW_LOAD_RET_OK) {
        return ret;
    }
    /* 复位时打印Reset Device及ACK frame，忽略. */
    return TIOT_FW_LOAD_RET_QUIT;
}
