...
errcode_t uapi_upg_boot_image_update(const upg_image_header_t *image)
{
    ...
    uint32_t app_address = 0;
    uint32_t app_size = 0;
    uint32_t index = UPG_BOOT_JUMP_FLAG;
    uint32_t index_len = sizeof(index);
    errcode_t ret_val = upg_get_partition_info(image->image_id, &app_address, &app_size);
    if (ret_val != ERRCODE_SUCC) {
        return ret_val;
    }
    ...
    upg_free(buffer);
    ret = upg_write_new_image_data(UPG_BOOT_JUMP_FLAG_OFFSET, (uint8_t *)&index, &index_len, image->image_id, false);
    if (ret != ERRCODE_SUCC) {
        return ret;
    }
    buffer = NULL;
