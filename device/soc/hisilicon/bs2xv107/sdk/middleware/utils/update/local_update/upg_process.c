...
{
    errcode_t ret;
    if (img_header->image_id != UPG_IMAGE_ID_RES_INDEX && img_header->image_id != UPG_IMAGE_ID_RES_DATA && img_header->image_id != FLASHBOOT_IMAGE_ID) {
...
#else
    } else if (img_header->image_id == FLASHBOOT_IMAGE_ID) {
        upg_partition_id_ref_t image_refs = { PARTITION_FLASHBOOT_IMAGE, PARTITION_FLASHBOOT_BACKUP };
        ret = upg_backup_update(&image_refs);
        if (ret != ERRCODE_SUCC) { return ret; }
        ret = uapi_upg_boot_image_update(img_header);
        if (ret != ERRCODE_SUCC) { return ret; }
    } else {
