...
#define UPG_BOOT_FLAG         0x10000
#define UPG_BOOT_FLAG_OFFSET  0x4
...
errcode_t upg_backup_update(upg_partition_id_ref_t *ids)
{
    ...
    ret = upg_flash_read(primary.part_info.addr_info.addr + UPG_BOOT_FLAG_OFFSET, sizeof(flag), (uint8_t *)&flag);
    if (ret != ERRCODE_SUCC) {
        return ret;
    }

    if (flag != UPG_BOOT_FLAG) {
        return ERRCODE_SUCC;
    }
