...
class BuildArgsResolver(ArgsResolverInterface):
    ...
    def resolve_build_xts(target_arg: Arg, build_module: BuildModuleInterface):
        ...
        """
        loader = build_module.loader
        product_arg = build_module.args_dict.get('product_name')
        product_name = getattr(product_arg, 'arg_value', product_arg)
        build_xts = bool(target_arg.arg_value) or str(product_name).endswith('_xts')
        loader.regist_arg("build_xts", build_xts)
        if build_xts:
            build_module.target_generator.regist_arg("build_xts", True)
        for gn_arg in build_module.args_dict['gn_args'].arg_value:
            ...
            if 'build_xts' in gn_arg:
                ...
                build_module.target_generator.regist_arg(variable, value)
