...
int GattServerService::impl::DeregisterApplicationImpl(int appId)
{
    ...
    if (server.has_value()) {
        ...
        LOG_ERROR("%{public}s:%{public}d:%{public}s() %{public}s",
            __FILE__, __LINE__, __FUNCTION__, "Invalid application Id");
    }
    ...
    uint16_t connectionHandle, uint16_t valueHandle, bool isUsingUuid)
{
    ...
    if (appId == attHandleMap_.end()) {
        LOG_ERROR("%{public}s:%{public}d:%{public}s() %{public}s",
            __FILE__, __LINE__, __FUNCTION__, "Attribute map error!");
        return;
    }
    ...
    if (server == servers_.end()) {
        LOG_ERROR("%{public}s:%{public}d:%{public}s() %{public}s",
            __FILE__, __LINE__, __FUNCTION__, "Server app map error!");
        return;
    }
    ...
    if (remote != remotes_.end()) {
        ...
        auto ccc = profile_->GetCharacteristic(valueHandle);
        if (ccc == nullptr) {
            LOG_ERROR("%{public}s:%{public}d:%{public}s Gatt database Error: Can not find "
                "Characteristic by value handle.", __FILE__, __LINE__, __FUNCTION__);
            return;
        }
...
void GattServerService::impl::OnIndicationEvent(uint16_t connectionHandle, uint16_t valueHandle, int ret)
{
    ...
    if (remote != remotes_.end()) {
        ...
        auto ccc = profile_->GetCharacteristic(valueHandle);
        if (ccc == nullptr) {
            LOG_ERROR("%{public}s:%{public}d:%{public}s Gatt database Error: Can not "
                "find Characteristic by value handle.", __FILE__, __LINE__, __FUNCTION__);
            return;
        }
        if (server->second.callback_ != nullptr) {
