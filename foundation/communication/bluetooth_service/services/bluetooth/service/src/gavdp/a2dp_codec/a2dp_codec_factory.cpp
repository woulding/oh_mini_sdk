    ...
    const A2dpCodecCapability &userConfig, A2dpCodecConfig *a2dpCodecConfig, bool *restartConfig)
{
    ...
    do {
        ...
        if (newPriority <= lastPriority) {
            LOG_INFO("[CodecFactory] %{public}s the new priority is lower than before, but not the current codec",
                __func__);
            break;
        }
        if (newPriority >= lastCodecConfig_->GetCodecPriority()) {
            LOG_INFO("[CodecFactory] %{public}s the new priority is higher than before, update current codec",
                __func__);
            currentCodecConfig_ = a2dpCodecConfig;
