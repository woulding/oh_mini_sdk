...
bool HfpHfDisconnected::Dispatch(const utility::Message &msg)
{
    ...
    LOG_DEBUG("[HFP HF]%{public}s():[Disconnected][%{public}s]",
        __FUNCTION__, HfpHfStateMachine::GetEventName(event.what_).c_str());
    switch (event.what_) {
...
{
    HfpHfMessage &event = (HfpHfMessage &)msg;
    LOG_DEBUG("[HFP HF]%{public}s():[Connecting][%{public}s]",
        __FUNCTION__, HfpHfStateMachine::GetEventName(event.what_).c_str());
    switch (event.what_) {
...
{
    HfpHfMessage &event = (HfpHfMessage &)msg;
    LOG_DEBUG("[HFP HF]%{public}s():[Connected][%{public}s]",
        __FUNCTION__, HfpHfStateMachine::GetEventName(event.what_).c_str());
    switch (event.what_) {
