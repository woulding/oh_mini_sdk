...
void Socket::impl::SockRfcDisconnectFail(Socket &socket, DataTransport *transport)
{
    ...
    if (socket.IsServer()) {
        if (socket.socketMap_.find(transport) != socket.socketMap_.end()) {
            LOG_DEBUG("SockRfcDisconnectFail closefd : fd:%{public}d",
                socket.socketMap_.at(transport).get()->transportFd_);
...
int Socket::ReceiveSdpResult(uint8_t scn)
{
    ...
    serviceId_ = AssignServiceId();
    scn_ = scn;
    LOG_INFO("[sock]%{public}s securityFlag:%{public}d serviceId_:%{public}d scn:%hhu",
        __func__, securityFlag_, serviceId_, scn_);
...
bool Socket::SendAppConnectScn(int fd, int scn)
{
