...
int HfpAgProfile::PhoneStateChange(const HfpAgPhoneState &phoneState)
{
    ...
    LOG_INFO("[HFP AG]%{public}s():preCallState_[%{public}s], preNumActiveCalls_[%{public}d], "
        "preNumHeldCalls_[%{public}d], currentCallState[%{public}s], activeNum[%{public}d], heldNum[%{public}d]",
        __FUNCTION__, GetCallState(preCallState_).c_str(), preNumActiveCalls_, preNumHeldCalls_,
        GetCallState(phoneState.callState).c_str(), phoneState.activeNum, phoneState.heldNum);
