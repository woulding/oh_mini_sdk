...
void HfpAgSystemEventProcesser::ProcessDialOutCallEvent(const std::string &number, int dialType)
{
    ...
    LOG_INFO("[HFP AG]%{public}s():: dialNumber length[%{public}d], dialType[%{public}d]", __FUNCTION__,
        static_cast<int>(dialNumber.length()), dialType);
...
void HfpAgSystemEventProcesser::GetAgIndicator()
{
    ...
    LOG_INFO("[HFP AG]%{public}s():activeNum[%{public}d], heldNum[%{public}d], callState[%{public}d]!",
        __FUNCTION__, activeNum, heldNum, callState);
