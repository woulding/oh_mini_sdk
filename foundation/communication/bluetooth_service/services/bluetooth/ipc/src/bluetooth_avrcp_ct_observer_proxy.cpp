...
void BluetoothAvrcpCtObserverProxy::OnPlayerAppSettingChanged(const RawAddress &rawAddr,
    const std::vector<uint8_t> &attributes, const std::vector<uint8_t> &values, int result)
{
    MessageParcel data;
    if (!data.WriteInterfaceToken(BluetoothAvrcpCtObserverProxy::GetDescriptor())) {
...
void BluetoothAvrcpCtObserverProxy::OnGetFolderItems(const RawAddress &rawAddr, uint16_t uidCounter,
    std::vector<BluetoothAvrcpMeItem> &items, int result, int detail)
{
    MessageParcel data;
    if (!data.WriteInterfaceToken(BluetoothAvrcpCtObserverProxy::GetDescriptor())) {
