    ...
    switch (transTable->type) {
        ...
        case AVDT_CH_TYPE_STREAM: {
            ...
            if (streamCtrl != NULL) {
                ...
                /* Trace: No resources */
                LOG_ERROR("[AVDT]%{public}s:AvdtGetStreamCtrlByHandle(%hu) Failed!",
                    __func__, transTable->streamHandle);
            }
            break;
        }
        default:
...
void AvdtRevSecurityCheck(uint16_t result, GapServiceSecurityInfo security, void *context)
{
    ...
    LOG_DEBUG("[AVDT]%{public}s:result(%hu),lcid(0x%x), psm(0x%x)",
        __func__, result, transTable->lcid, security.channelId.l2capPsm);
    if (result == AVDT_SUCCESS) {
