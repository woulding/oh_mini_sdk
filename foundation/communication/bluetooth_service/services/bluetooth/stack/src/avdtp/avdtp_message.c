...
void AvdtSendSig(AvdtSigCtrl *sigCtrl)
{
    ...
    LOG_DEBUG("[AVDT]%{public}s: packetLen(%hu) peermtu(%hu) lcid (%hu)",
        __func__, Len, transTbl->peerMtu, transTbl->lcid);
...
{
    LOG_DEBUG("[AVDT]%{public}s: number(%hhu) signal(0x%x) msgType(0x%x) label(0x%x)",
        __func__, number, signal, msgType, label);
...
void AvdtParseMsg(AvdtSigCtrl *sigCtrl, Packet *pkt)
{
    ...
    LOG_DEBUG("[AVDT]%{public}s: messageType(%hhu) transLabel(%hhu) pktType(%hhu)",
        __func__, messageType, transLabel, pktType);
...
void AvdtParseSignalRsp(AvdtSigCtrl *sigCtrl, uint8_t sigId, const Packet *pkt)
{
    ...
    LOG_DEBUG("[AVDT]%{public}s:sendLabel(%hhu) ReceiveLabel(%hhu),sigId(%hhu)",
        __func__, sigCtrl->label, sigCtrl->rcvLabel, sigId);
...
void AvdtParseDiscoverRsp(AvdtSigCtrl *sigCtrl, AvdtCtrlData *data, const Packet *pkt)
{
    ...
    do {
        ...
        context = 0;
        maxNumSeps--;
        LOG_DEBUG("[AVDT]%{public}s: sigCtrl->role(%hhu) seps[index].sepType(%hhu) acpid(%hhu) "
            "mediaType(%hhu) index(%hhu) maxNumSeps(%hhu)",
            __func__,
            sigCtrl->role,
