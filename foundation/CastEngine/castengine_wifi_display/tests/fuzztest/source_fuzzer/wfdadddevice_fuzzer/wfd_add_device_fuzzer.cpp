...
#include "wfd_add_device_fuzzer.h"

namespace OHOS {
namespace Sharing {

    bool AddAndRemoveDevice()
    {
        ...
        deviceInfo.deviceId = "AA:BB:CC:DD:EE:FF";
        int32_t ret = client->AddDevice(0, deviceInfo);
        if (ret == 0) {
            client->RemoveDevice("AA:BB:CC:DD:EE:FF");
        }
...
{
    OHOS::Sharing::AddAndRemoveDevice();
    return 0;
}
