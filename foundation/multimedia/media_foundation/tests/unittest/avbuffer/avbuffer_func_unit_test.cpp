...
/**
 * @tc.name: AVBuffer_VirtualMemory_SetSize_SizePlusOffset_001
 * @tc.desc: SetSize accepted when size+offset equals capacity (boundary)
 * @tc.type: FUNC
 */
HWTEST_F(AVBufferInnerUnitTest, AVBuffer_VirtualMemory_SetSize_SizePlusOffset_001, TestSize.Level1)
{
    capacity_ = MEMSIZE;
    align_ = 0;
    CreateLocalVirtualMem();
    ASSERT_FALSE((buffer_ == nullptr) || (buffer_->memory_ == nullptr));
    auto &mem = *buffer_->memory_;
    EXPECT_EQ(Status::OK, mem.SetOffset(capacity_ / 2)); // 2: half capacity
    EXPECT_EQ(mem.offset_, capacity_ / 2);
    EXPECT_EQ(Status::OK, mem.SetSize(capacity_ / 2)); // 2: size+offset == capacity
    EXPECT_EQ(mem.size_, capacity_ / 2);
}

/**
 * @tc.name: AVBuffer_VirtualMemory_SetSize_SizePlusOffset_002
 * @tc.desc: SetSize rejected when size+offset exceeds capacity
 * @tc.type: FUNC
 */
HWTEST_F(AVBufferInnerUnitTest, AVBuffer_VirtualMemory_SetSize_SizePlusOffset_002, TestSize.Level1)
{
    capacity_ = MEMSIZE;
    align_ = 0;
    CreateLocalVirtualMem();
    ASSERT_FALSE((buffer_ == nullptr) || (buffer_->memory_ == nullptr));
    auto &mem = *buffer_->memory_;
    ASSERT_EQ(Status::OK, mem.SetOffset(capacity_ / 2)); // 2: half capacity
    int32_t oldSize = mem.size_;
    EXPECT_EQ(Status::ERROR_INVALID_PARAMETER, mem.SetSize(capacity_ / 2 + 1)); // 2: sum > capacity
    EXPECT_EQ(mem.size_, oldSize);                                              // unchanged on rejection
}

/**
 * @tc.name: AVBuffer_VirtualMemory_SetSize_SizePlusOffset_003
 * @tc.desc: SetSize rejected when size+offset would overflow int32
 * @tc.type: FUNC
 */
HWTEST_F(AVBufferInnerUnitTest, AVBuffer_VirtualMemory_SetSize_SizePlusOffset_003, TestSize.Level1)
{
    capacity_ = MEMSIZE;
    align_ = 0;
    CreateLocalVirtualMem();
    ASSERT_FALSE((buffer_ == nullptr) || (buffer_->memory_ == nullptr));
    auto &mem = *buffer_->memory_;
    mem.offset_ = INT32_MAX; // force offset_ to INT32_MAX; size=1 makes size+offset overflow int32
    int32_t oldSize = mem.size_;
    EXPECT_EQ(Status::ERROR_INVALID_PARAMETER, mem.SetSize(1));
    EXPECT_EQ(mem.size_, oldSize); // unchanged on rejection
}

/**
 * @tc.name: AVBuffer_VirtualMemory_SetOffset_SizePlusOffset_001
 * @tc.desc: SetOffset accepted when size+offset equals capacity (boundary)
 * @tc.type: FUNC
 */
HWTEST_F(AVBufferInnerUnitTest, AVBuffer_VirtualMemory_SetOffset_SizePlusOffset_001, TestSize.Level1)
{
    capacity_ = MEMSIZE;
    align_ = 0;
    CreateLocalVirtualMem();
    ASSERT_FALSE((buffer_ == nullptr) || (buffer_->memory_ == nullptr));
    auto &mem = *buffer_->memory_;
    EXPECT_EQ(Status::OK, mem.SetSize(capacity_ / 2)); // 2: half capacity
    EXPECT_EQ(mem.size_, capacity_ / 2);
    EXPECT_EQ(Status::OK, mem.SetOffset(capacity_ / 2)); // 2: size+offset == capacity
    EXPECT_EQ(mem.offset_, capacity_ / 2);
}

/**
 * @tc.name: AVBuffer_VirtualMemory_SetOffset_SizePlusOffset_002
 * @tc.desc: SetOffset rejected when size+offset exceeds capacity
 * @tc.type: FUNC
 */
HWTEST_F(AVBufferInnerUnitTest, AVBuffer_VirtualMemory_SetOffset_SizePlusOffset_002, TestSize.Level1)
{
    capacity_ = MEMSIZE;
    align_ = 0;
    CreateLocalVirtualMem();
    ASSERT_FALSE((buffer_ == nullptr) || (buffer_->memory_ == nullptr));
    auto &mem = *buffer_->memory_;
    ASSERT_EQ(Status::OK, mem.SetSize(capacity_ / 2)); // 2: half capacity
    int32_t oldOffset = mem.offset_;
    EXPECT_EQ(Status::ERROR_INVALID_PARAMETER, mem.SetOffset(capacity_ / 2 + 1)); // 2: sum > capacity
    EXPECT_EQ(mem.offset_, oldOffset);                                            // unchanged on rejection
}

/**
 * @tc.name: AVBuffer_VirtualMemory_SetOffset_SizePlusOffset_003
 * @tc.desc: SetOffset rejected when size+offset would overflow int32
 * @tc.type: FUNC
 */
HWTEST_F(AVBufferInnerUnitTest, AVBuffer_VirtualMemory_SetOffset_SizePlusOffset_003, TestSize.Level1)
{
    capacity_ = MEMSIZE;
    align_ = 0;
    CreateLocalVirtualMem();
    ASSERT_FALSE((buffer_ == nullptr) || (buffer_->memory_ == nullptr));
    auto &mem = *buffer_->memory_;
    mem.size_ = INT32_MAX; // force size_ to INT32_MAX; offset=1 makes size+offset overflow int32
    int32_t oldOffset = mem.offset_;
    EXPECT_EQ(Status::ERROR_INVALID_PARAMETER, mem.SetOffset(1));
    EXPECT_EQ(mem.offset_, oldOffset); // unchanged on rejection
}

/**
 * @tc.name: AVBuffer_VirtualMemory_ReadCommon_SizePlusOffset_001
 * @tc.desc: ReadCommonFromMessageParcel accepted when size+offset equals capacity
 * @tc.type: FUNC
 */
HWTEST_F(AVBufferInnerUnitTest, AVBuffer_VirtualMemory_ReadCommon_SizePlusOffset_001, TestSize.Level1)
{
    capacity_ = MEMSIZE;
    align_ = 0;
    CreateLocalVirtualMem();
    ...
    auto &mem = *buffer_->memory_;
    MessageParcel parcel;
    (void)parcel.WriteInt32(MEMSIZE);     // capacity
    (void)parcel.WriteInt32(0);           // align
    (void)parcel.WriteInt32(MEMSIZE / 2); // 2: offset, half capacity
    (void)parcel.WriteInt32(MEMSIZE / 2); // 2: size, sum == capacity
    EXPECT_TRUE(mem.ReadCommonFromMessageParcel(parcel));
    EXPECT_EQ(mem.capacity_, MEMSIZE);
    EXPECT_EQ(mem.offset_, MEMSIZE / 2);
    EXPECT_EQ(mem.size_, MEMSIZE / 2);
}

/**
 * @tc.name: AVBuffer_VirtualMemory_ReadCommon_SizePlusOffset_002
 * @tc.desc: ReadCommonFromMessageParcel rejected when size+offset exceeds capacity
 * @tc.type: FUNC
 */
HWTEST_F(AVBufferInnerUnitTest, AVBuffer_VirtualMemory_ReadCommon_SizePlusOffset_002, TestSize.Level1)
{
    capacity_ = MEMSIZE;
    align_ = 0;
    CreateLocalVirtualMem();
    ASSERT_FALSE((buffer_ == nullptr) || (buffer_->memory_ == nullptr));
    auto &mem = *buffer_->memory_;
    int32_t oldCap = mem.capacity_;
    MessageParcel parcel;
    (void)parcel.WriteInt32(MEMSIZE);         // capacity
    (void)parcel.WriteInt32(0);               // align
    (void)parcel.WriteInt32(MEMSIZE / 2);     // 2: offset, half capacity
    (void)parcel.WriteInt32(MEMSIZE / 2 + 1); // 2: size, sum > capacity
    EXPECT_FALSE(mem.ReadCommonFromMessageParcel(parcel));
    EXPECT_EQ(mem.capacity_, oldCap); // unchanged on rejection
}

/**
 * @tc.name: AVBuffer_VirtualMemory_ReadCommon_SizePlusOffset_003
 * @tc.desc: ReadCommonFromMessageParcel rejected when size+offset would overflow int32
 * @tc.type: FUNC
 */
HWTEST_F(AVBufferInnerUnitTest, AVBuffer_VirtualMemory_ReadCommon_SizePlusOffset_003, TestSize.Level1)
{
    capacity_ = MEMSIZE;
    align_ = 0;
    CreateLocalVirtualMem();
    ...
    int32_t oldCap = mem.capacity_;
    MessageParcel parcel;
    (void)parcel.WriteInt32(INT32_MAX); // capacity
    (void)parcel.WriteInt32(0);         // align
    (void)parcel.WriteInt32(INT32_MAX); // offset
    (void)parcel.WriteInt32(1);         // size, INT32_MAX + 1 overflows int32
    EXPECT_FALSE(mem.ReadCommonFromMessageParcel(parcel));
    EXPECT_EQ(mem.capacity_, oldCap); // unchanged on rejection
}

/**
 * @tc.name: AVBuffer_VirtualMemory_SkipCommon_SizePlusOffset_001
 * @tc.desc: SkipCommonFromMessageParcel accepted when size+offset equals capacity
 * @tc.type: FUNC
 */
HWTEST_F(AVBufferInnerUnitTest, AVBuffer_VirtualMemory_SkipCommon_SizePlusOffset_001, TestSize.Level1)
{
    capacity_ = MEMSIZE;
    align_ = 0;
    CreateLocalVirtualMem();
    ...
    auto &mem = *buffer_->memory_;
    MessageParcel parcel;
    (void)parcel.WriteInt32(MEMSIZE);     // capacity (skipped)
    (void)parcel.WriteInt32(0);           // align (skipped)
    (void)parcel.WriteInt32(MEMSIZE / 2); // 2: offset
    (void)parcel.WriteInt32(MEMSIZE / 2); // 2: size, sum == capacity_
    EXPECT_TRUE(mem.SkipCommonFromMessageParcel(parcel));
    EXPECT_EQ(mem.offset_, MEMSIZE / 2);
    EXPECT_EQ(mem.size_, MEMSIZE / 2);
}

/**
 * @tc.name: AVBuffer_VirtualMemory_SkipCommon_SizePlusOffset_002
 * @tc.desc: SkipCommonFromMessageParcel rejected when size+offset exceeds capacity
 * @tc.type: FUNC
 */
HWTEST_F(AVBufferInnerUnitTest, AVBuffer_VirtualMemory_SkipCommon_SizePlusOffset_002, TestSize.Level1)
{
    capacity_ = MEMSIZE;
    align_ = 0;
    CreateLocalVirtualMem();
    ...
    (void)parcel.WriteInt32(0);               // align (skipped)
    (void)parcel.WriteInt32(MEMSIZE / 2);     // 2: offset
    (void)parcel.WriteInt32(MEMSIZE / 2 + 1); // 2: size, sum > capacity_
    EXPECT_FALSE(mem.SkipCommonFromMessageParcel(parcel));
    EXPECT_EQ(mem.size_, oldSize);     // unchanged on rejection
    EXPECT_EQ(mem.offset_, oldOffset); // unchanged on rejection
}

/**
 * @tc.name: AVBuffer_VirtualMemory_SkipCommon_SizePlusOffset_003
 * @tc.desc: SkipCommonFromMessageParcel rejected when size+offset would overflow int32
 * @tc.type: FUNC
 */
HWTEST_F(AVBufferInnerUnitTest, AVBuffer_VirtualMemory_SkipCommon_SizePlusOffset_003, TestSize.Level1)
{
    capacity_ = MEMSIZE;
    align_ = 0;
    CreateLocalVirtualMem();
    ASSERT_FALSE((buffer_ == nullptr) || (buffer_->memory_ == nullptr));
    auto &mem = *buffer_->memory_;
    mem.capacity_ = INT32_MAX; // enlarge capacity_ so prior capacity_ >= offset/size pass
    int32_t oldSize = mem.size_;
    int32_t oldOffset = mem.offset_;
    MessageParcel parcel;
    (void)parcel.WriteInt32(0);         // capacity (skipped)
    (void)parcel.WriteInt32(0);         // align (skipped)
    (void)parcel.WriteInt32(INT32_MAX); // offset
    (void)parcel.WriteInt32(1);         // size, INT32_MAX + 1 overflows int32
    EXPECT_FALSE(mem.SkipCommonFromMessageParcel(parcel));
