...
#ifndef OHOS_DM_SOFTBUS_LISTENER_REFACTOR_H
#define OHOS_DM_SOFTBUS_LISTENER_REFACTOR_H
...
class SoftbusListenerRefactor {
public:
    SoftbusListenerRefactor();
    ~SoftbusListenerRefactor();
    ...
    static void OnCredentialAuthStatus(const char *deviceList, uint32_t deviceListLen,
    ...
    int32_t SetForegroundUserIdsToDSoftBus(const std::string &remoteUserId,
...
private:
...
#if !(defined(__LITEOS_M__) || defined(LITE_DEVICE))
    void ConvertAclToDeviceInfo(DistributedDeviceProfile::AccessControlProfile &profile,
    ...
    static void SaveDeviceOnlineCache(NodeBasicInfo *info, DmSoftbusEvent &dmSoftbusEventInfo);
    static void ReportDeviceOnlineRadar(NodeBasicInfo *info, const std::string &peerUdid);
    static void HandleDeviceOnlineEvent(DmSoftbusEvent &dmSoftbusEventInfo, NodeBasicInfo *info,
                                        const std::string &peerUdid);
    static void DeleteDeviceOfflineCache(NodeBasicInfo *info, DmSoftbusEvent &dmSoftbusEventInfo);
    static void ReportDeviceOfflineRadar(NodeBasicInfo *info, const std::string &peerUdid);
    static void HandleDeviceOfflineEvent(DmSoftbusEvent &dmSoftbusEventInfo, const std::string &peerUdid);
    static void ExtractEventQueue(std::string deviceId, std::queue<DmSoftbusEvent> &eventQueue);
    static void ProcessEventQueueItem(DmSoftbusEvent &dmSoftbusEventInfo, const std::string &deviceId);
    static void CheckQueueRemain(std::string deviceId);
#endif
    ...
    static int32_t ValidateTrustChangeMsg(const char *msg, uint32_t msgLen);
    static void DispatchTrustChangeEvent(TrustChangeType type, const std::string &softbusMsg);
...
#endif // OHOS_DM_SOFTBUS_LISTENER_REFACTOR_H