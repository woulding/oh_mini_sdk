...
#ifndef OHOS_DM_AUTH_STATE_MACHINE_CRED_H
#define OHOS_DM_AUTH_STATE_MACHINE_CRED_H
...
public:
    DmAuthStateMachineCred(std::shared_ptr<DmAuthCredContext> context);
    ~DmAuthStateMachineCred();
    ...
    int32_t TransitionTo(std::shared_ptr<DmAuthStateCred> state);
    ...
    DmEventType WaitExpectEvents(const std::set<DmEventType>& expectedEvents);
...
private:
    ...
    void Run(std::shared_ptr<DmAuthCredContext> context);
    void InsertSrcTransTable();
    void InsertSinkTransTable();

    // Fetch the current state and execute it
    std::optional<std::shared_ptr<DmAuthStateCred>> FetchAndSetCurState();
    ...
    DmAuthStateType preState_{DmAuthStateType::CRED_AUTH_IDLE_STATE};  // Previous push state
    ...
    std::queue<std::shared_ptr<DmAuthStateCred>> statesQueue_;
    ...
    DmAuthCredDirection direction_;
...
#endif // OHOS_DM_AUTH_STATE_MACHINE_CRED_H
