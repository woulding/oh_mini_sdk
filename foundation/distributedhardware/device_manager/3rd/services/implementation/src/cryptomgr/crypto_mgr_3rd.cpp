...
CryptoMgr3rd::CryptoMgr3rd()
{
    LOGI("CryptoMgr3rd ctor");
}

CryptoMgr3rd::~CryptoMgr3rd()
{
    LOGI("CryptoMgr3rd dtor");
    ClearSessionKey();
}

//LCOV_EXCL_START
int32_t CryptoMgr3rd::EncryptMessage(const std::string &inputMsg, std::string &outputMsg)
{
    ...
    Crypto3rd::ConvertBytesToHexString(hexStrTemp, hexStrLen, encData, encDataLen);
    outputMsg.clear();
...
int32_t CryptoMgr3rd::DoEncryptData(AesGcmCipherKey *cipherKey, const unsigned char *input, uint32_t inLen,
...
int32_t CryptoMgr3rd::MbedAesGcmEncrypt(const AesGcmCipherKey *cipherKey, const unsigned char *plainText,
...
int32_t CryptoMgr3rd::GenerateRandomArray(unsigned char *randStr, uint32_t len)
{
...
//LCOV_EXCL_START
int32_t CryptoMgr3rd::DecryptMessage(const std::string &inputMsg, std::string &outputMsg)
{
    ...
    Crypto3rd::ConvertHexStringToBytes(inputMsgBytsTemp, inputMsgBytesLen,
...
int32_t CryptoMgr3rd::MbedAesGcmDecrypt(const AesGcmCipherKey *cipherKey, const unsigned char *cipherText,
...
int32_t CryptoMgr3rd::DoDecryptData(AesGcmCipherKey *cipherKey, const unsigned char *input, uint32_t inLen,
...
std::vector<unsigned char> CryptoMgr3rd::GetSessionKey()
{
    std::lock_guard<ffrt::mutex> lock(sessionKeyMtx_);
    return std::vector<unsigned char>(sessionKey_.key, sessionKey_.key + sessionKey_.keyLen);
}

void CryptoMgr3rd::ClearSessionKey()
{
    std::lock_guard<ffrt::mutex> lock(sessionKeyMtx_);
...
int32_t CryptoMgr3rd::ProcessSessionKey(const uint8_t *sessionKey, const uint32_t keyLen)
{
